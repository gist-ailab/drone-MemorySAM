...
def main(cfg, gpu, save_dir):
    ...
    # mask_tokens 초기화 (Xavier/Glorot)
    torch.nn.init.xavier_uniform_(model.sam.sam_mask_decoder.mask_tokens.weight)
    
    # output_hypernetworks_mlps 초기화
    for mlp in model.sam.sam_mask_decoder.output_hypernetworks_mlps:
    ...
    if loss_cfg.get('CLS_WEIGHTS', False):
        ...
        freq_weights = torch.tensor([
            ...
            1.5,   # Fence
            1.0,   # Other
            2.0,   # Pedestrian (low freq)
            1.5,   # Pole
            1.0,   # RoadLine
            0.3,   # Road (very high freq)
            0.5,   # SideWalk (high freq)
            0.4,   # Vegetation (high freq)
            1.0,   # Cars
            1.0,   # Wall
            1.5,   # TrafficSign
            0.5,   # Sky (high freq)
            1.0,   # Ground
            2.0,   # Bridge (low freq)
            2.0,   # RailTrack (low freq)
            2.0,   # GroundRail (low freq)
            1.5,   # TrafficLight
            1.5,   # Static
            2.0,   # Dynamic (low freq)
            2.0,   # Water (low freq)
            1.0,   # Terrain
            2.0,   # TwoWheeler (low freq)
            2.0,   # Bus (low freq)
            2.0,   # Truck (low freq)
        ], device=device)
        cls_weights = freq_weights
        logger.info(f"Using class weights: {cls_weights}")
    
    loss_fn = get_loss(loss_cfg['NAME'], trainset.ignore_label, cls_weights)
    start_epoch = 0
