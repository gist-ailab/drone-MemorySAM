...
In this work, we build upon these advances and present our solution for the \textbf{4th Workshop on Maritime Computer Vision (MaCVi)} USV Semantic Segmentation Challenge at CVPR 2026.
The challenge employs the MULTIAQUA dataset~\cite{muhovic2025multiaqua}, which provides synchronized RGB, thermal, and LiDAR data captured from USVs across daytime and nighttime conditions, with four semantic classes: static obstacle, dynamic obstacle, water, and sky.
A key difficulty is that models are trained on daytime data but must generalize to a nighttime test set where RGB sensors become severely degraded.
...
To enable the gating to respond to per-image quality variation rather than converging to static weights, we enrich the feature representation with quality-sensitive statistics (global average pooling, global max pooling, and channel-wise standard deviation) and use modality-specific compression networks.
Crucially, the segmentation objective alone provides insufficient gradient signal to drive adaptive gating, since a fixed weighting can already minimize this loss.
We therefore introduce per-modality auxiliary segmentation heads that produce independent per-image quality estimates; oracle gating weights are derived from these estimates and used to supervise the fusion head via KL divergence, providing a direct training signal for adaptivity.
The learned weights are applied through two distinct paths: max-normalized for memory modulation (preserving the best modality at full strength) and as raw softmax for output fusion (weighting contributions proportionally).
...
We empirically validate each component through controlled experiments and report our challenge results on the MULTIAQUA benchmark.
