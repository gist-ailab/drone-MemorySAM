...
idx = 0
num_images = len(image_files)
win_name = 'MinMaxNorm (NonZero) Thermal Viewer'

def minmax_normalize_nonzero(img):
    """
    0이 아닌 값에 대해서만 min-max 정규화 적용. (thermal padding 값 무시)
    """
    if img is None:
        return None
    ...
    if max_val - min_val < 1e-5:
        ...
        norm_img[mask] = (norm_img[mask] * 255.0).clip(0,255)
    return norm_img.astype(np.uint8)

def show_img(idx):
    img = cv2.imread(image_files[idx], cv2.IMREAD_GRAYSCALE)
    if img is not None:
        minmax_img = minmax_normalize_nonzero(img)
        display = cv2.hconcat([
            cv2.cvtColor(img, cv2.COLOR_GRAY2BGR),
            cv2.cvtColor(minmax_img, cv2.COLOR_GRAY2BGR)
        ])
        cv2.putText(display, f'{os.path.basename(image_files[idx])} ({idx+1}/{num_images})',
                    (10, 25), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0,255,0), 2)
        cv2.imshow(win_name, display)
