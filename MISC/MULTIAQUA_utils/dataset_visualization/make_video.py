#!/usr/bin/env python3
"""
MULTIAQUA concat_test_modalities.py 로 저장한 이미지를 영상으로 인코딩.
...
def main():
    ap = argparse.ArgumentParser(description="MULTIAQUA concat frames -> video")
    ap.add_argument("--frames-dir", type=str, default=None, help="프레임 폴더 (000000.png 또는 *_concat.png 등)")
    ap.add_argument("--out", type=str, default="multiaqua_test_modalities.mp4", help="출력 영상 파일명")
    ...
    if first.exists():
        ...
        # 이름순 정렬된 PNG 목록으로 concat demuxer 사용
        with tempfile.NamedTemporaryFile(mode="w", suffix=".txt", delete=False) as f:
            for p in pngs:
                # ffmpeg concat: 경로에 특수문자 있으면 이스케이프
                path = p.resolve()
                f.write(f"file '{path}'\n")
            list_path = f.name
        try:
