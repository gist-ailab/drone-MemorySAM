...
| `P32_정량검증_실패분석_20260708.md` | **학습 완료 후 최종 검증 리포트** — 4축 독립 재계산(멀티에이전트), 실패 유형화, 원인 지도 | **최신. 여기부터 읽어라** → 후속 설계는 [[00_P33_CGMoD_index\|P33_CGMoD]] |
...
- 사후 검증: ✅ **4축 독립 재계산 완료** ([[P32_정량검증_실패분석_20260708]]) — 핵심: CoRB attn-bias는 **유의한 순손해**(ΔmIoU −0.013, p=4.5e-22, flip 0.046%)로 P32 이득을 모듈에 귀속 불가. "신호는 유효, pre-softmax 주입은 무효" → 후속 처방 = [[architecture/P33_v2_설계개정_20260708|P33-v2]] (attn-bias 제거, corr_veto를 gate 입력으로 승격)
