...
| H16 | spatial×modality routing이 DINOv3에서 SAM2-SoftMoE가 못 잡은 여지를 가진다 (user 재개방: H1 반증은 SAM2 계보라 spatial 축 DINOv3 미검증) | Spatial-Modality Oracle 프로브(2026-08-19 실측) → **실현성 통제 대기** | ⏳ **잠정 재개방** — 스크린 미폐쇄 | 오라클 Δ **val +8.66/test +8.29**(게이트 ≥1.0의 8배) → 내 사전확률(Δ<0.3) **반증**. 오류 = "전역 drop≈0 ⟹ 픽셀 라우팅 0"의 함의 거짓(전역 평탄 ≠ 국소 상보성 부재). **단 상계 과대**(union-over-15 팽창 + zero-fill OOD) → 팽창/실현 분리 통제(#14) 선행. 통제 전 라우터 착수·H1 폐쇄 단정 금지 | [experiments/analysis/2026-08-19-spatial-modality-oracle-verdict.md](../experiments/analysis/2026-08-19-spatial-modality-oracle-verdict.md) |
