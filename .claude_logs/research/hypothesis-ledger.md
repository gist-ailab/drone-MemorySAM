...
| H16 | spatial×modality routing이 DINOv3에서 SAM2-SoftMoE가 못 잡은 여지를 가진다 (user 재개방: H1 반증은 SAM2 계보라 spatial 축 DINOv3 미검증) | 오라클 프로브(08-19) + 실현성 통제 #14(08-20) → **입력-실현성 #15 대기** | ⏳ **개방 유지** — GT-회수 여지 확인, 실현성 미검 | 입도 스윕: block16 Δ **+4.1~4.6**·block64 +1.9~2.8(공간응집, 픽셀노이즈 아님). 널 브래킷(완전상관 0 / 독립 +32~44 / 관측 +8.66=27%)=상보성 실재하나 오차 양의상관. **내 게이트 2차 스펙 오류(널 부등호 방향 거꾸로) 인정** — 폐쇄도 라우터정당화도 아님. 남은 선행조건 = no-GT 라우터가 여지 회수하나(#15) | [experiments/analysis/2026-08-20-oracle-realizability-control-verdict.md](../experiments/analysis/2026-08-20-oracle-realizability-control-verdict.md) |
