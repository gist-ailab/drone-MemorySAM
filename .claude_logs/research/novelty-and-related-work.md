...
> **✅ 2026-09-08 편입 업데이트 (전수 스윕 → 비교표, 정본 = 볼트 `09_benchmark_tables_deliver_muses_mcubes.md` U10~U14)**: user 지시 "나중에 논문에서 빼더라도 비교는 하라"에 따라 조사 세션이 제시한 후보 전부를 **채택 또는 보류로 기록**했다(지어낸 수치 없음). 요약 — (1) venue·상태 갱신: StitchFusion **ACM MM 2025**, OmniSegmentor NeurIPS 2025 정식 행, MM-SAM-adapter **IEEE 게재**(Xplore 11162503, 게재본 수치 대조 잔여), **RobustSeg = RMMSS 확장, CVPR 2026 본회의**(DELIVER 결측 +2.40% / full −0.1% 자체 보고, 코드 공개), MemorySAM 여전히 preprint. (2) **결측 강건성 표 신설(U11)** — 두 계열을 분리: (a) 벤치마크 계열(arXiv 2503.18445, 정규화 후 zero-fill) EMM 15조합 평균 MAGIC 44.97 > MAGIC++ 44.85 > StitchFusion 41.98 > CMNeXt 37.90 > GeminiFusion 37.07; (b) MAGIC 계열(결측 브랜치 건너뜀) RobustSeg 49.89 > FunEntropy 48.29 > MAGIC++ 47.74 > AnySeg 46.64 > MAGIC 40.49. 같은 CMNeXt가 37.90 vs 25.25/20.77로 갈리므로 **혼용 금지**, 우리 EMM/RMM은 (a)에 맞춘다. (3) MUSES: URVIS 2026 워크숍 챌린지 리포트(2604.16984)를 리더보드 각주 출처로 채택, GtA 82.39 재확인 실패 유지. (4) 보류: RobustSeg 절대 EMM 값, BiXFormer "+2.75", Any2Seg 절대치, CHARM, MCubeS MAGIC++/StitchFusion 원문 대조, RGB-T 보조표(SHIFNet FMB 67.8/PST900 89.8, Frequency-Guided MFNet 61.73/PST900 86.24 — CVPR 2026 PBVS 워크숍, RSGMamba 61.1/88.9, Sigma 불일치, SARTM 미확인)는 우리 벤치가 아니라 본표 밖(U13).
