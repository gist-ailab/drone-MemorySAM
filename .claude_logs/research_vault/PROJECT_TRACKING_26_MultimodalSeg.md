...
| Status | Item | Output |
|---|---|---|
| ✅ | P32(CoRB) 학습 완료 + 4축 독립 검증 (멀티에이전트 9-agent) | [[P32_CoRB/P32_정량검증_실패분석_20260708]] — val 64.12/test 55.00, CoRB attn-bias 순손해 확정(p=4.5e-22), 지배 원인=class-transfer(상한 +7.9pt), NEW sun 하드씬 |
| ✅ | P33-v2 설계 개정 (원안 CG-MoD 적대적 비판 + 딥리서치 3축 R1/R2/R3) | [[P33_CGMoD/P33_v2_설계개정_20260708]] — ablation 순서 역전(class-transfer 1순위), top-k 삭제, dropout+distillation 필수, unfreeze 삭제, kill criteria/escape 명시 |
| ✅ | 차세대 백본 브레인스토밍 (Cowork) + 검증 | [[material/brainstorm_next_arch_20260708]] — 내부수치 13/14 정확, 외부인용 검증(수정 3건), 카드 A(DINOv3)/B(SAM2 v2) 릴레이 |
