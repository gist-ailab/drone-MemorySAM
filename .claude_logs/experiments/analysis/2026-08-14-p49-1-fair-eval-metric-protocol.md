...
## 5. 후속 실측 (2026-08-15, GPU7 배치) — 프로토콜 완결 + seed 함대 통일

**Job1 — P46 본run ep70, CMNeXt-프로토콜(1024-리사이즈-GT) test = 56.99** — native-GT(56.99)와 소수 2자리까지 동일. **1042→1024 리사이즈의 프로토콜 델타 ≈ 0** (768 케이스의 +2.02와 대조 — 왜곡은 큰 다운샘플에서만 발생). **함의: 우리 수치는 MM-SA(native)와도 DGFusion 계열(1024-GT)과도 비교 유효 — "vs DGFusion +0.28" 각주가 양 프로토콜에서 성립.** 공개표 상향 가설은 기각(상승분 ≈0), 대신 비교 가능성이 완전히 닫힘.

**seed 함대 통일 프로토콜 표 (val.py native test@1024, val-best ckpt):**

| 런 | 학습 해상도 | val-best(로그) | val.py test@1024 |
|---|---|---|---|
| 본run | **@768** | 67.79@70 | **56.99** |
| seedB(진짜시드) | @1024 | 70.58@98 | 54.85 |
| seedC(진짜시드) | @1024 | 70.73@54 | 54.55 |
| λ0.05 | @1024 | 69.10@96 | 53.71 |

**판정**: ① **@1024 학습의 test 유해가 N=3으로 확정**(54.55~54.85 vs @768학습 56.99 — H7의 "학습해상도 이득"은 P39.1 계열 한정, P46-C3 계열에선 역효과. val↑/test↓ 역발산의 통일 프로토콜 재확인). ② **진짜-시드 분산은 작다**: seedB vs seedC Δ=0.30(test)/0.15(val) — 재현성 주장에 유리. ③ 🔴 **대표 구성(@768 학습)의 진짜-시드 복제가 없다** — 시드런들이 전부 @1024로 돌아서, 논문 mean±std를 대표 구성으로 내려면 @768 시드 2런이 필요(코디네이터 결정 대기). ④ λ0.05 트레이너로그(55.43) vs val.py(53.71) −1.72 미해명 잔차 존재 — 판정은 val.py 단일 측정기로만(재확인된 원칙).
