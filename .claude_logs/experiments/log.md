---
legacy_id: 03
legacy_file: 03_experiment_log.md
moved: 2026-07-08
---
...
### P33.1 (CG-MoD stage-1: Calibration + Competence-Weighted Fusion) — DELIVER, 학습 시작 2026-07-09

- **계보**: `LoRA_Sam_P33(LoRA_Sam_P32)` — config-gated 3기구(M1 competence fusion / M2 asymmetric modality dropout / M3 calibration), 전부 OFF→P32 byte-identical. branch `worktree-p33-impl`.
- **P33.1 config** (`configs/b200-deliver_rgbdel_P33_1_physaug.yaml`): RBMA_CALIB on(M3) + COMPETENCE_FUSION on(M1, TAU 0.25, soft) + **CORROBORATION.ENABLE false** + MODAL_DROPOUT off.
  - **M1 신호 = calibrated self-entropy(`_comp_rel`), corr_veto 아님** — corr_veto는 죽은 lidar를 0.85로 오상향(합의 동조)하므로 융합 신호로 부적합. reliability는 depth를 0.94로 정확히 1위(competence 43.7 일치).
  - **CoRB attn-bias OFF (P33-v2 M4 반영)**: P32 4축 독립검증에서 corroboration bias = 유의한 순손해(p=4.5e-22, vault issues/P32_정량검증_실패분석_20260708) → bias는 P31식 calibrated self-entropy로 복귀. seam은 ablation용 보존.
  - ⚠️ 첫 launch(04:04)는 corroboration ON 상태로 30분 진행 후 중단·재시작(04:10, outputs `_v1corrbON_aborted`로 격리) — v2 개정 발견 즉시 교정.
- **검증**: py_compile ×3 / CPU 수학 스모크(competence softmax+top-k 정규화, depth top-weight 획득, dropout이 event·lidar 미선택, all-OFF→P32 위임) / B200 GPU 스모크(빌드+iter, loss 감소) 전부 PASS.
- **학습**: B200 GPU 2-5 4-GPU DDP, 200ep, tmux `jemo:p33_1`, log `logs/b200-deliver_rgbdel_P33_1_physaug/P33_1_20260709_041048.log`.
- **비교 기준**: P32 final Test 55.01@ep158/Val 64.12@ep98 · P28 55.27 · P31 54.75. 게이트(간이): misalloc↓ + test ≥ P32.
- **후속 스테이지**: P33.2 = +MODAL_DROPOUT(P 0.3, targets img/depth, warmup 20ep) — config 준비됨. P33-v2 개정안(M1 class-transfer 승격, RCS+text-anchor+MIC)은 별도 승인·구현 대기.
...
- **Phase 0 무학습 게이트 PASS**: self-entropy [.77,.62,.30,.22] → corroboration event/LiDAR .54/.81 반전. 상세 [analysis/p32-phase0-results.md](analysis/p32-phase0-results.md).
