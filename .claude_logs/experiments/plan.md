...
| 서버 | 우리 점유 | 타 사용자 | 비고 |
|---|---|---|---|
| **jarvis** (4090×8) | GPU0,1 **E-LoRA arm A**(센서별 독립 LoRA rank16)·**arm B**(전 센서 완전공유 LoRA rank16) **시드20260902** legal v2(nearest-exact) 재채점 진행 중 | 이번 갱신 미확인(GPU2-7) | 학습은 p38 체크아웃, 채점은 develop 코드 + 체크포인트 절대경로 |
| **yeon** (3090×8) | GPU4,5 **E-LoRA arm C**(공유 LoRA rank8 + 센서별 잔차 LoRA rank8) **시드902** legal v2 재채점 진행 중(로드 `missing=0 unexpected=0`) · GPU6,7 **arm A·arm B 시드903** legal v2 재채점 진행 중 | 이번 갱신 미확인(GPU0-3) | arm C 시드902 **학습 자체는 완주**(Total Training Time 16:59:24, 트레이너 val 최고 68.69@ep60). 참고: 같은 GPU4,5에서 돌던 **4탭+센서별 프로토타입 시드4(E13)**(4탭 읽기 = 중간층 블록 6/12/18/24 보조 감독, 센서별 prototype = 센서마다 클래스 prototype 유지)는 09-20 05:09 완주(트레이너 val 최고 68.35@110), legal v2 재채점도 같은 날 끝났다: **test 55.67 / val 69.40** |
| **hpca100** (A100×4) | GPU0 **Q1**(품질 헤드 프로브 — 예측 신뢰도를 직접 회귀하는 보조 헤드, 5ep) · GPU1 **R1**(경계 prior refinement — 클래스 경계 부근 예측을 사전 지식으로 다듬는 모듈, 40ep 스크린) · GPU2 **R2**(연결 성분 손실 — 예측 마스크의 연결 요소 개수를 정규화하는 보조 손실, 40ep 스크린) — 전부 **P54(다음 모델 후보) 준비** | 없음(볼륨은 공유) | 이번 갱신 미확인(GPU3). 🔴 공유 볼륨 감시 유지 — 09-15 Errno 28 로 6런 사망 전례 |
| **bengio** (8장) | 없음 | — | 🔴 **GPU 전면 고장(2026-09-20 확인)** — GPU6 이 `nvidia-smi` 목록에서 사라졌고 남은 장도 CUDA 컨텍스트 생성 실패. **배치 불가**. 이전에 여기서 돌던 E-LoRA arm A·B 시드902·903 학습(4쌍)의 생존 여부는 이번 갱신에서 미확인. 리부트는 user 판단 대기 |
...
|---|---|---|---|---|---|---|
| **Q1 = 품질 헤드 프로브**(예측 신뢰도를 직접 회귀하는 보조 헤드, 5ep 짧은 프로브) | hpca100 0 | DELIVER 4모달 | 5 | 진행 중 — 수치 이 세션에서 미확인 | 미확인 | P54 후보 재료 — 품질 헤드가 학습 가능한 신호를 배우는지 초단기 확인 |
| **R1 = 경계 prior refinement**(클래스 경계 부근 예측을 사전 지식으로 다듬는 모듈, 40ep 스크린) | hpca100 1 | DELIVER 4모달 | 40 | 진행 중 — 수치 이 세션에서 미확인 | 미확인 | P54 후보 재료 — 스크린 게이트 판정용 |
| **R2 = 연결 성분 손실**(예측 마스크의 연결 요소 개수를 정규화하는 보조 손실, 40ep 스크린) | hpca100 2 | DELIVER 4모달 | 40 | 진행 중 — 수치 이 세션에서 미확인 | 미확인 | P54 후보 재료 — 스크린 게이트 판정용 |
