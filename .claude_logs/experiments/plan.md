...
## 🔬 실행 중 (2026-09-09 12:54 KST 실상태 동기화 — 노션 논문 페이지 §6.1과 동일 스냅샷)
...
| ⏸ **카드 E4b** 혼동 쌍 명시 — bengio 중단분 재개 | jarvis 5 → 재채점 | DELIVER 4모달 | 40 | 🔴 **ep36~40 구간에서 Adam ComplexFloat 에러로 사망(09-10 10:4x)**. 트레이너 val ep15 63.54 · ep20 64.28 · ep25 64.94 · ep30 65.49 · **ep35 66.07(val-best)** | ep35 로 legal 재채점 중 | `_multi_tensor_adam` 에서 `result type ComplexFloat can't be cast to Float` — 수치 불안정으로 `exp_avg_sq` 가 음수가 되어 sqrt 에서 복소수. **E4(auto)는 완주했는데 E4b(명시 쌍)만 죽었다 → confusion margin 항의 불안정 의심.** ⚠️ 규약은 40ep 완주 후 판정이나 **val-best 가 이미 ep35 이고 E4 도 val-best 가 ep15**(ep40 은 64.99 로 더 낮음)여서 ep35 로 재채점했다 — **판정에 「35ep 기준」 명시할 것**. 후반에 E4 역전(ep35 66.07 vs 65.70) |
...
| ⏸ **카드 E3s2** E3 시드2(20260902) 매칭 페어 | ~~hpca100 0~~ **GPU0 상실(09-10 02:2x)** | DELIVER 4모달 | 40 | 🔴 **CUDA OOM 사망(ep10 직후)** — 트레이너 val ep5 62.90 · **ep10 63.33**. ckpt 보존(`epoch10_63.33_top1`, NAS md5 일치) | 재개 대기 | **원인은 타 사용자**: GPU0 에 남의 프로세스가 34,104MiB 를 잡아 20MiB 도 할당 못 함. 저희 설정 문제 아님. ⚠️ 다만 E3s2 는 평가 구간에서 **38.2GB(A100 의 93%)**를 써 여유가 1.8GB 뿐이었다 — 재개 시 학습 중 test 평가를 끄는 편이 안전. 자리는 E2s2(13:50)·E12(14:20) 완주 후 |
...
| ⏸ **DGFusion Swin-T 재학습**(공식 설정 bs8·LR1e-4·200k) | jarvis — **GPU5·7 상실(09-09)** | DELIVER CLDE | 200k iter | 🔴 **NaN 발산 4회**(iter 86,572 · 87,370 · 87,842 · **86,572 재발**). 80k ckpt 정본 확정: **val 66.5427**(공개값 66.51 대비 +0.03) / **test 55.6807**(56.71 대비 **−1.03**) | — | **200k 완주 포기, 80k 를 정본으로 기록**(생각정리 세션 판정). 발산은 네 번 모두 `loss_ce`·`loss_contrastive` 만 NaN 이고 `loss_mask`·`loss_dice` 는 0.0, `loss_condition`·`loss_depth` 는 정상 — 디코더 경로만 오염. 4차가 1차와 같은 iter 이고 80k 재개는 데이터 순서를 재현하므로 원인은 특정 배치일 가능성이 높다. **test 클래스별에서 RailTrack 50.38 vs 우리 B0 32.0 = 격차 18.4** 로, 카드의 'RailTrack 이 진짜 격차' 진단이 실측으로 뒷받침됨(E2 가 47.75 로 대부분 회복). 반대로 Wall 은 우리가 앞섬(12.17 vs 2.21), Bridge 0.00 은 공통 붕괴. ⚠️ 평가 스플릿 교체는 `DATASETS.TEST_SEMANTIC`. 복원 킷 `third_party/dgfusion_train_restore/` |
