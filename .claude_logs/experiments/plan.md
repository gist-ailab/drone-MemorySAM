...
## 🖥 GPU 예약·점유 현황 (2026-09-20 22:20 KST 실측)

> 🔴 **2026-09-20 전면 교체**: 아래는 이 세션이 직접 확인한 것만 싣는다. 이 갱신에서 확인하지 못한 GPU 는 "이번 갱신 미확인"으로 표시하고 예전 상태를 그대로 베끼지 않는다.

| 서버 | 우리 점유 | 타 사용자 | 비고 |
|---|---|---|---|
| **jarvis** (4090×8) | GPU0,1 **E-LoRA arm A**(센서별 독립 LoRA rank16)·**arm B**(전 센서 완전공유 LoRA rank16) **시드20260902** legal v2(nearest-exact) 재채점 진행 중 | GPU2-7 = **"dgfusion deliver training" 세션이 사용 중**(22:00 실측 6.2~18.3GiB 점유) | 재채점 체크포인트는 bengio 에서 md5 대조 복사해 온 것이다(`rescore_elora/ckpt/`) |
| **yeon** (3090×8) | GPU4,5 **E-LoRA arm C**(공유 LoRA rank8 + 센서별 잔차 LoRA rank8) **시드902** legal v2 재채점 진행 중(로드 `missing=0 unexpected=0`) · GPU6,7 **arm A·arm B 시드903** legal v2 재채점 진행 중 | GPU0,1 = **4탭 읽기 확정 시드4(E1) 학습**(22:00 실측 ep188/200, 트레이너 val 최고 68.33@164, 완주 ETA 09-21 05:50) · GPU2,3 = **프로토타입 손실만 쓴 기준선 시드4(C3-only) 학습**(ep196/200, 67.02@74, 완주 ETA 09-21 00:50) — 둘 다 우리 런이다 | arm C 시드902 **학습 자체는 완주**(Total Training Time 16:59:24, 트레이너 val 최고 68.69@ep60). 참고: 같은 GPU4,5에서 돌던 **4탭+센서별 프로토타입 시드4(E13)**(4탭 읽기 = 중간층 블록 6/12/18/24 보조 감독, 센서별 prototype = 센서마다 클래스 prototype 유지)는 09-20 05:09 완주(트레이너 val 최고 68.35@110), legal v2 재채점도 같은 날 끝났다: **test 55.67 / val 69.40** |
| **hpca100** (A100×4) | GPU0 **Q1**(품질 헤드 프로브 — 예측 신뢰도를 직접 회귀하는 보조 헤드, 5ep) · GPU1 **R1**(경계 prior refinement — 클래스 경계 부근 예측을 사전 지식으로 다듬는 모듈, 40ep 스크린) · GPU2 **R2**(연결 성분 손실 — 예측 마스크의 연결 요소 개수를 정규화하는 보조 손실, 40ep 스크린) — 전부 **P54(다음 모델 후보) 준비** | 없음(볼륨은 공유) | GPU3 = 유휴(22:00 실측 0MiB) — 다음 배치 후보. 🔴 공유 볼륨 감시 유지 — 09-15 Errno 28 로 6런 사망 전례 |
...
## 🔬 실행 중 (2026-09-20 22:20 KST 실측 갱신)
...
> 🔴 **2026-09-20 전면 교체**: **E17 확정 200에폭 2런(4탭 읽기+고해상도 세부 가지, 시드20260821 hpca100 GPU0·시드20260902 jarvis GPU3,7)은 user 지시로 취소**됐다(둘 다 ep32 에서 40에폭 스크린 게이트 수준 미달로 중단; 부분 체크포인트는 NAS `ckpts/E17_confirm200_seed{20260821,20260902}_partial_20260920/`에 md5 대조 보존, registry.md 반영). jarvis·yeon 8장은 전부 학습이 아니라 **E-LoRA 3갈래(arm A 센서별 r16·arm B 완전공유 r16·arm C 공유r8+잔차r8) legal v2 재채점** 중이다 — 위 GPU 예약·점유 현황 표 참조, 이 표에는 EPOCHS 진행이 있는 학습 런만 싣는다. **bengio 는 GPU 전면 고장으로 배치 제외**(GPU6 미인식·CUDA 컨텍스트 생성 실패, 리부트 user 판단 대기). hpca100 3장은 **P54(다음 모델 후보) 준비 스크린 3건**(Q1·R1·R2)으로 교체.

| 실험 | 서버/GPU | 데이터셋 | EPOCHS | 진행(🔴 트레이너 val — legal 아님) | ETA | 목적·게이트 |
|---|---|---|---|---|---|---|
| **Q1 = 품질 헤드 프로브**(예측 신뢰도를 직접 회귀하는 보조 헤드, 5ep 짧은 프로브) | hpca100 0 | DELIVER 4모달 | 5 | ep0/5 완료(22:00 실측, 프로브라 트레이너 val 없음) | 09-21 01:00 | P54 후보 재료 — 품질 헤드가 학습 가능한 신호를 배우는지 초단기 확인 |
| **R1 = 경계 prior refinement**(클래스 경계 부근 예측을 사전 지식으로 다듬는 모듈, 40ep 스크린) | hpca100 1 | DELIVER 4모달 | 40 | ep4/40(22:00 실측, 첫 eval 전) | 09-21 10:10 | P54 후보 재료 — 스크린 게이트 판정용 |
| **R2 = 연결 성분 손실**(예측 마스크의 연결 요소 개수를 정규화하는 보조 손실, 40ep 스크린) | hpca100 2 | DELIVER 4모달 | 40 | ep3/40(22:00 실측, 첫 eval 전) | 09-21 16:30 | P54 후보 재료 — 스크린 게이트 판정용 |
