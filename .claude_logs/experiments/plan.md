...
| **15b** | **confidence 라우터 실현성** (per-subset max-softmax 기반 블록 선택) | **1 GPU 수 시간**(캐시가 argmax만 → forward 재실행) | 🟡 **판정 파생(2026-08-20)** — consensus와 기제적으로 다른 마지막 실현 신호 | 소수-옳은 모달이 고신뢰면 confidence-max가 잡을 수 있음(합의는 못 잡음). **게이트**: block16 Δ≥+1.0 → 학습 게이트 정당화(신 세대·user GO) / <+0.5 → **모든 실현신호 소진 = H16 실용적 폐쇄**(user 최종 판단). 구현=`oracle_spatial_modality.py`에 confidence 저장+`--realizable confidence`, forward 재실행 필요 | 판정 §C |
