...
## 🔬 실행 중 (2026-09-08 16:10 KST 실상태 동기화 — 노션 논문 페이지 §6.1과 동일 스냅샷)
...
|---|---|---|---|---|---|---|
| **카드 E1** 중간층 4탭 읽기 | bengio 6,7 | DELIVER 4모달 | 40 | ep38/40 — ep5~35: 60.98/63.56/66.30/66.57/66.43/66.88/**67.25**(카드 중 최고, 트레이너 val) (B0 58.71/63.17/62.18/63.91/64.03/65.13/64.67/**65.40@40**) | 09-08 20:28 KST 전후 → `val.py` legal 재채점(GPU0 조기 재채점은 타 사용자 점유로 취소) | 카드 첫 통과 후보. 통과선 test ≥ 54.78(B0 53.78 + 1.0) |
| **카드 E3** 센서별 클래스 prototype | bengio 4,5 | DELIVER 4모달 | 40 | ep30/40 — ep5~30: 59.35/63.72/62.23/65.44/65.97/65.51 (best 65.97@25) | 09-08 23:45 KST | user 가설 직접 검증 |
| **카드 E4** 혼동 쌍 margin(auto k5) | bengio 1,2,3 | DELIVER 4모달 | 40 | ep30/40 — ep5~25: 59.77/63.17/65.92/65.70/65.79 (best 65.92@15) | 09-08 22:30 KST | auto 쌍에 RailTrack 누락 → 명시 쌍판 **E4b** config 추가(RailTrack→Sky/Static/Terrain, Wall→Building, Water→Terrain), 미기동 |
| **카드 E2** 전 선형층 LoRA r32·α64 | hpca100 1,3 | DELIVER 4모달 | 40 | ✅ **완주(09-08 17:24 KST, 18:26:55)** — 트레이너 val ep5~40: 62.54/63.44/60.09/62.40/65.25/66.91/67.85/**68.65**(val-best ckpt `epoch40_68.65_top1`). 🔴 로그의 `Best Test 55.57 (epoch 30)`은 test-best라 무효 | ✅ **legal 재채점 완결(09-08 20:3x KST)**: **test 54.50**(mAcc 63.31) / **val 68.38**(mAcc 75.83), 1024 BS1 native GT(`configs/eval/hpca100-deliver_rgbdel_P46_E2_eval1024_legal.yaml`). **B0 대비 Δtest +0.72 / Δval +3.41** — 통과선 +1.0 미달·폐기선 +0.5 초과 = **회색지대, 판정 보류(생각정리 세션)**. 클래스별 test Δ: RailTrack +15.77(31.98→47.75)·TrafficLight +12.49·TrafficSign +2.89·Truck +2.68 / Wall −6.40·Water −5.47·Static −5.44·Ground −1.20. 트레이너 val 68.65 − 공식 68.38 = +0.27(B0 +0.43과 같은 방향) | 구조 사다리 S2(= E-BB A1). 통과선 test ≥ 54.78. hpca100 GPU1 해방 → 다음 카드 대기 |
| **카드 E7c** MUSES PhysAug-on 대조군 | hpca100 2 | MUSES 3모달 | 40 | ✅ **완주(09-08 20:25 KST)** — 트레이너 val ep5~40: 73.18/75.52/75.97/77.68/79.54/79.52/79.96/**80.18**(val-best ep40). 짝 **E7**(off) ep5~40: 73.84/75.39/77.06/77.41/79.16/79.84/79.69/**80.29**. 8지점 Δ(on−off) −0.66/+0.13/−1.09/+0.27/+0.38/−0.32/+0.27/−0.11 = 부호 교대·폭 ±1.1 안(트레이너 val, 공식 아님) | `tools/eval_muses_official.py` 공식 페어 재채점 진행 중(hpca100 GPU2, `mus_pair_eval`, 출력 `~/SSDb/jemo_maeng/muses_official_eval_20260908/{E7,E7c}`) — E7의 **첫 공식 test** | PhysAug 효과 격리 → MUSES 헤드라인 교체 여부 |
| **카드 E1M** MUSES 4탭 이식(E7 PhysAug-off 매칭 페어) | hpca100 3 | MUSES 3모달 | 40 | ep10/40 — 트레이너 val ep5 71.73 · ep10 75.55 vs E7 73.84 · 75.39 (−2.11 → +0.16; warmup 15ep라 ep15·20이 실질 비교점) | 09-09 04시 KST 전후 | 공통 상승 검증: 공식 val Δ(E1M − E7) ≥ +1.0 |
| **미기동(config만)** B0s2 · E1s2(bengio판·hpca100판) · E12(E1+E2) · E4b | — | DELIVER 4모달 | 40 | develop f34c0a8·25eda09 (E1M은 17:37 기동으로 제외) | E1·E2 legal 판정 후 | 시드2 페어·결합·명시 쌍 |
| **P52 RxDINO DELIVER seed1 / seed2** | yeon 2,3 / 4,5 | DELIVER 4모달 | 200 | ep38/200 — val-best 66.25@36 / 66.19@28 | ~09-12 | 감사 후 게이트 재등록 예정(동등성 [−1.0,∞) 3페어 + G5 고정 스윕) |
| **E-LoRA arm A** per-modal r16 재런 | yeon 0,1 | DELIVER 4모달 | 200 | ep38/200 — val-best 66.64@26 | ~09-12 | A0 기준치. arm B(공유)·C(공유+잔차) 슬롯 대기 |
| **P52 MUSES seed1 / seed2** | hpca100 0,2 / 1,3 | MUSES 4모달 | 300 | 🟡 **보류**(ep54 79.63 / ep30 78.81, ckpt 보존) | — | 감사 §1.5(3모달 기준선·PhysAug) 적용 후 재개 |
| **DGFusion Swin-T 재학습**(공식 설정 bs8·LR1e-4·200k) | jarvis 1,2,4,5 | DELIVER CLDE | 200k iter | iter 19,979/200,000(16:08 KST) · iter 10k 평가 mIoU 60.31(depth abs_rel 10.99) | ~1.3일 (09-09~10) | 공개값 val 66.51/test 56.71 재현 → 한계 분석. 복원 킷 `third_party/dgfusion_train_restore/` |
| **CAFuser Swin-T 대조군**(bs6·LR0.75e-4·266,667 iter) | lecun 0,1,2 | DELIVER CLDE | 266,667 iter | iter 11,039/266,667(16:08 KST) · iter 10k 평가 mIoU 55.74 | 로그 ETA 4일 17시간(09-13경) | 공개값 val 68.12/test 55.80 재현 |
...
| **MUSES-PhysAug-off** | 🔴 **공정성 정정(user 2026-09-07)**: MUSES 최선(P39.1 seed2 3모달) 레시피에서 PhysAug만 off, 시드 매칭 3페어 → MUSES 헤드라인·SOTA 표 교체. P52 MUSES 팔도 시드 3부터 PhysAug off | yeon/hpca100 2장 × 3런 | 🟡 등록(2026-09-07, 감사 문서 §3.5-1) — DELIVER는 07-20부터 off였으나 MUSES 계보는 on이었음(P43/P44만 off, 효과 분리 불가) |
