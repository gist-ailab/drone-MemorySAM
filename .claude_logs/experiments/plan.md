...
| **P50-EXT** | P50 사전학습 확장(코퍼스/에폭 스케일업) — 제안서 "통과 시 확장" 조항 | 대규모(수일) | ⏸ 등록 + 슬롯 후보 = hpca100 0,1(현재 타유저 점유, 해방 감시) — 🟢 **user 승인(2026-08-31, 디스커션 B)** — Phase1=생성 파이프라인 확장(편광 AoLP/DoLP proxy: normal 추정→Fresnel 합성 / NIR proxy: RGB 변환) 구현 위임 + 코퍼스 규모 결정 → Phase2=hpca100 해방 시 본 사전학습. 게이트 사전등록 예정(프로브 +0.74 대비 스케일 이득). 확장 시 **편광/NIR proxy 추가 생성**으로 3벤치 공통 init화가 설계 포인트 | H22 이득(+0.74)이 스케일에 비례하는지 |
