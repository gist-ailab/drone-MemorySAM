...
| **15** | **no-GT 라우터 실현성 하한** (캐시 전용 합의/다수결) | 없음(캐시 numpy) | 🔵 **val 음성(2026-08-20), test 진행 중** | majority/consensus **전부 음수(val −1.0~−1.4)** = 값싼 실현신호 부재. 여지가 anti-consensus(소수-옳음)라 합의 라우터가 틀린 쪽 선택. → #15b로 이월. 판정 §B |
| **15b** | **confidence 라우터 실현성** (per-subset max-softmax 기반 블록 선택) | **1 GPU 수 시간**(캐시가 argmax만 → forward 재실행) | 🟢 **user GO(2026-08-20) — 기동 지시(opus-direct)** | 소수-옳은 모달이 고신뢰면 confidence-max가 잡을 수 있음(합의는 못 잡음). **게이트(사전등록)**: block16 Δ≥+1.0 → 학습 게이트 정당화(신 세대·user GO) / 전 granularity <+0.5 → **모든 실현신호 소진 = H16 실용적 폐쇄**. 구현=`oracle_spatial_modality.py`에 max-softmax conf 저장(pred 캐시 불변 가드)+`--realizable confidence`(block{1,8,16,32,64}), forward 재실행=yeon GPU5/6/7(P50 GPU1-4 무충돌). 검수=이 세션. 판정 §C |
