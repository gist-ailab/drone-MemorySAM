...
## 🖥 GPU 예약·점유 현황 (2026-09-22 17:50 KST 실측 — yeon 리부트 사고·중단 런 재개 반영)

> 🔴 **2026-09-20 전면 교체**: 아래는 이 세션이 직접 확인한 것만 싣는다. 이 갱신에서 확인하지 못한 GPU 는 "이번 갱신 미확인"으로 표시하고 예전 상태를 그대로 베끼지 않는다.

| 서버 | 우리 점유 | 타 사용자 | 비고 |
|---|---|---|---|
| **jarvis** (4090×8) | GPU0 **Q2**(P54 증류 대조군, tmux `p54_q2`) · GPU4 **Q3**(QAF 본 카드, tmux `p54_q3`) — cfg `/SSDb/jemo_maeng/qaf/cfg_Q{2,3}_jarvis.yaml`, repo `/SSDb/jemo_maeng/src/drone-MemorySAM` · **GPU1,2 e1scr_s903**(E1 40ep 스크린 시드903 — 09-22 19:57 yeon ckpt md5 이송 후 AUTO_RESUME ep20/40 재개, tmux `e1scr_s903`, 2.5it/s ~00:20 완주) → 종료 시 체인이 **muphys_826** 기동(GPU1,2, `chain_muphys826`) | GPU3,5,6,7 = **minkyoung_chun(dgss)이 09-22 저녁 점유**(~17.7GB×4) | probe_quality_head(Q1b-2 추정) 09-22 저녁 종료 — **결과 회수 필요**. Q2 가 policy ban:0 GPU0 에 있음(user 확인 요청) |
| **yeon** (3090×8) | GPU4,5 **muphys_824**(MUSES PhysAug-off 기준선 시드20260824, 300ep) · GPU6,7 **muphys_825**(시드20260825) — 🔴 09-22 12:55 리부트로 ep75/300 사망 → 17:39 **AUTO_RESUME 재개**(tmux `muphys_824`·`muphys_825`, 로그 `logs/muphys_8{24,25}_resume_20260922.log`, 사망 시 best 80.66@68 / 81.30@74, ckpt 보존 확인) | GPU0-3 = **sangtae_park(dice-rl)이 리부트 직후 점유**(각 8.5GB, 09-22 13:3x) | 🔴 **09-22 리부트 사고**: 12:55 셧다운 → 13:28 부팅, tmux 전소실. 같이 죽은 **e1scr_s903**(E1 40ep 스크린 시드903, ep20/40, `last_checkpoint.pth` 12:35 보존)은 자리 부족으로 **미재개** — jarvis 빈 슬롯에 ckpt 옮겨 재개 가능(아래 비고) |
| **hpca100** (A100×4) | GPU1 **R1 legal v2 재채점**(tmux `r1_rescore`, 19:50 기동 — BREFINE 로드 0/0·가드 OK, val+test ~2.5h) → 종료 시 체인이 **E1-shared s821**(tmux 예약, GPU1,3, cfg `queue_cfg/cfg_E1shared_s821_hpca100.yaml`) 기동 · GPU2 **R2 학습 중**(체인이 종료 감지 → R2 재채점 GPU2 예약, `chain_r2rescore`) · GPU3 E1-shared 대기 | GPU0 = 타 tenant 프로세스(~34.7GB, util 0%) | ⚠️ repo가 오래된 cddc319 위에 **R1/R2·QAF 코드가 미커밋 로컬 수정(+566줄)로 존재** — pull 금지(ISSUE-038). LORA_MODE shared 코드는 있음 확인(E1-shared 가동 가능). 체인 로그 = `queue_cfg/chain_{e1shared,r2rescore}.log` |
| **bengio** (8장) | 없음 | — | 🔴 **GPU 전면 고장(2026-09-20 확인)** — GPU6 이 `nvidia-smi` 목록에서 사라졌고 남은 장도 CUDA 컨텍스트 생성 실패. **배치 불가**. 여기서 돌던 E-LoRA arm A·B 시드902·903 학습 4쌍은 **고장 이전에 전부 200에폭 완주**했다(학습 로그 `Total Training Time` 22:25 실측: arm A 902 = 11:16:46·최고 68.06@70, arm B 902 = 10:38:47·67.39@100, arm A 903 = 11:06:10·68.57@80, arm B 903 = 15:10:32·66.88@80). 체크포인트도 md5 대조로 회수해 잃은 것은 없다. 리부트는 user 판단 대기 |
| **lecun** (24GB×7) | 없음 — user 지시로 추가 작업 배치 금지 | sangmin_park · youngjin_lee | GPU 0~4 유휴. 🔴 lecun 배치 금지(2026-09-17 user 지정) 유지 |
| ~~B200~~ | — | — | 🔴 상실(07-15 마감) |

## 🔬 실행 중 (2026-09-22 17:50 KST 실측 갱신)
...
> 🔴 **2026-09-22 사고·재개**: **yeon 12:55 리부트**로 muphys_824·825(ep75/300)·e1scr_s903(ep20/40) 사망, GPU0-3 을 타인(sangtae_park)이 선점 → 824·825 는 GPU4-7 AUTO_RESUME 재개, **e1scr_s903 만 미재개**(ckpt 보존, jarvis 슬롯 대기). hpca100 R1·R2 는 user가 09-21 새벽에 죽인 세션을 **체크포인트에서 원상 재개**(내 첫 보고 "ckpt 없음"은 오류 — 서브디렉터리 안에 있었음). E1·C3-only 시드4 학습, E-LoRA 재채점, E17, Q1/Q1b 프로브는 전부 완료·종결로 표에서 제거.

| 실험 | 서버/GPU | 데이터셋 | EPOCHS | 진행(🔴 트레이너 val — legal 아님) | ETA | 목적·게이트 |
|---|---|---|---|---|---|---|
| **E1-shared s821**(E1 레시피 + 완전공유 LoRA r16, 40ep 스크린) | hpca100 1,3 | DELIVER 4모달 | 40 | ep33/40(09-23 10:10 실측, best 65.02@15 — 부진) | ~09-23 12:30 | 게이트 = legal v2 Δ24 ≥ −0.3 vs E1 스크린 짝 + RMM depth Δ 절반 이하 |
| **R2 legal v2 재채점**(val+test, ckpt epoch40_67.6_top1) | hpca100 2 | DELIVER | — | val 2%(09-23 10:15 재기동 — 1차 기동이 config YAML 들여쓰기로 즉사했던 것 수복) | ~09-23 13:30 | R2 스크린 판정 재료(짝 = E1 40ep 스크린 v2) |
| **muphys_826 = MUSES PhysAug-off 시드20260826**(셋째, 300ep) | jarvis 1,2 | MUSES 3모달 | 300 | 09-23 01:40 체인 기동(e1scr_s903 완주 직후) | ~09-25 | 공정선 3페어 완결 |
| **muphys_824 = MUSES PhysAug-off 기준선 시드20260824**(풀 런) | yeon 4,5 | MUSES 3모달 | 300 | 재개 후 진행 중(사망 시 best 80.66@68) | ~09-25 오전 | 공정선 3페어(E7 81.88 짝) — user 2026-09-07 공정성 정정 |
| **muphys_825 = 〃 시드20260825** | yeon 6,7 | MUSES 3모달 | 300 | 재개 후 진행 중(사망 시 best 81.30@74) | ~09-25 오전 | 〃 |
| **Q2 = P54 증류 대조군**(동결 교사 + 두 패스, F·Q off) | jarvis 0 🔴 | DELIVER 4모달 | 40 | 생존 확인(09-22 15:0x etime 19h20m, 17.8GiB) — 에폭 미실측 | — | P54 게이트(T′ 대조군) |
| **Q3 = P54 QAF 본 카드**(품질 토큰 + key 마스크 + 두 패스) | jarvis 4 | DELIVER 4모달 | 40 | 생존 확인(etime 21h33m, 5.9GiB+) — 에폭 미실측 | — | P54 게이트(G-clean·G-robust) |

> 🔗 **체인 4건 설치(09-22 19:5x)**: ① hpca100 R1 재채점 끝 → **E1-shared s821**(GPU1,3, 40ep, 게이트 = legal v2 24클래스 Δ ≥ −0.3 vs E1 스크린 짝 그리고 RMM depth Δ 절반 이하) ② hpca100 R2 학습 끝 → **R2 legal v2 재채점**(GPU2, 최신 top1 ckpt 자동 선택, test_ 접두어 제외) ③ jarvis e1scr_s903 끝 → **muphys_826**(MUSES PhysAug-off 셋째 시드, 300ep, GPU1,2) ④ (yeon GPU4-7 muphys_824·825 ~09-25). **여전히 대기**: E1-shared 시드 902·903 · R1/R2 시드 902·903 스크린(s821 판정 후) · DGFusion (b) val-best 사후 스윕 · Q1b-2 probe 결과 회수.
