...
| **P44-BMR (Balanced Modality Rebalance)** | hpca100 GPU2,3 (A100×2) | 200 (ep30 조기게이트) | ~07-29 완주 추정(첫 ep 후 it/ep로 재확정) | **P0** | drop-modality dMIoU(lidar/event)≈0 = 비RGB 미사용(img 과지배) 공략. B-1 MMPareto gradient 통합 + B-2 peer 상호증류(대칭KL+관계형) + B-3 coverage-aware 국소마스킹. config `configs/hpca100-muses_rgbel_P44_bmr.yaml`(develop @678c493, jarvis판에서 경로·배치만 이식), BS2/GPU accum→eff16, tmux `jemo:p44_bmr`, log `logs/hpca_P44_bmr.log`. **기동검증 PASS**(iter 14→17 전진 ~1.5s/it, GPU2,3 100%/~33GB 활성화수준, 에러0, MMPareto active=388params 4groups, loss 11.38→11.21 유한). **판정 게이트(사전등록, ep30)** = dMIoU(lidar)≈0→>1 & p44/cos_lora_lidar 음→양 전환 & CKA(img,lidar)≥0.5 유지 / 완주 val≥82.22 & fog≥68. 미달 시 kill 🔴 사망(2026-07-28 00:36, ep150 외부 preempt — 에러0 프로세스 소멸+GPU 타테넌트 점유, hpca100 공유 pod). 미완주, 사망 직전 best **val 80.59@ep126**(seed2 82.62 −2.03). ep126 ckpt 저장됨(AUTO_RESUME 가능). BMR-MUSES 기제(test 전이) 판정 미확보. hpca100 2,3 회수 불가. → **부활(07-28 재개 성공)**: hpca100 preempt 후 GPU 재해방, AUTO_RESUME이 **ep150 last_checkpoint에서 재개**(백본 HF Hub 로딩 정체는 `HF_HUB_OFFLINE=1`으로 해소). GPU2,3 활성 81-86%/33GB, loss 유한, ep150/200부터 진행. ETA ~오늘 완주. tmux `jemo:p44_resume2`, log `hpca_P44_bmr_resume2.log`. ⚠️ hpca100 공유 pod라 재-preempt 가능(ckpt로 재개 가능). |
...
| **P44-BMR DELIVER** | jarvis GPU4-7(4090×4) | 200 | ~07-30/31 완주 추정 | **P1** | DELIVER 모달 리밸런스 — depth/lidar 잉여(drop-lidar −0.78 실증)가 DELIVER 특화 병목이라 P44(B-1 MMPareto+B-2 상호증류+B-3 국소마스킹)가 가장 이론적합. config `configs/jarvis-deliver_rgbdel_P44_bmr.yaml`(develop be2603c, BS1). tmux `jemo:deliver_p44`, log `_run2.log`. 기동검증 PASS(iter 406/995, GPU4-7 65~100%/~15.4GB, loss 8.56 유한). 게이트: dMIoU(비RGB)>0 전환 & P36 fair 초과 |
| **4-modal P44-BMR+radar** | hpca100 GPU0-3(A100×4) | 200(ep30 게이트) | ~07-29 완주 추정(375it/ep, 첫 ep 후 재확정) | **P0** | **2번째 4-modal 각도**(user 4모달 우선). P44-MUSES(80.71) 완주로 해방된 A100에 배치. base=P44-BMR(MMPareto+상호증류+국소마스킹) + radar 4모달 = **BMR이 가장 안 쓰이는 radar를 surface하는지** 검증(yeon P39.1+radar와 다른 recipe). config `configs/hpca100-muses_rgbelr_P44_bmr_4modal.yaml`(develop 1cf1e66, BS1 — BS2는 A100 40GB서 OOM 36.4GB). tmux `jemo:p44_4modal2`, log `hpca_P44_4modal2.log`. 기동검증 PASS(iter 245/375, GPU0-3 93-100%/~25GB, loss 9.19→9.13 유한, radar 로드 OK MODALS=4, HF_HUB_OFFLINE로 백본정체 없음). ⚠️ hpca100 churn 노드(재-preempt 가능, ckpt 재개). 게이트: ep30 val 3모달궤도 추종 & 붕괴없음 / 완주 val≥82.22(P38) 또는 drop-radar dMIoU>0 @fog  🔴 kill(07-28) — 최초 'BMR 과균형' 진단은 **오진**이었음. 실제 원인 = `HF_HUB_OFFLINE=1`로 DINOv3 백본이 RANDOM INIT돼 학습된 것(P44·P39.1 둘 다 동일 실패). **BMR·radar 무죄.** hpca100 HF 백본 이중고장 → RELIADINO_LOCAL_BACKBONE env fix로 해결(697a10a). 이 자리는 P39.1+radar seed2로 재배치됨. |
...
| **P44-BMR DELIVER (seed2)** | yeon GPU6,7(3090×2) | 200 | ~07-30 완주 추정(1991it/ep) | P1 | jarvis P44-DELIVER의 seed2(variance). config `configs/yeon-deliver_rgbdel_P44_bmr.yaml`(develop 216a286, BS1). seed3 완주(81.89@204)로 해방된 6,7에 배치(⚠️ 자동기동 watch가 pgrep self-match 버그로 실패 → 수동 기동). tmux `jemo:deliver_p44_yeon`. 기동검증 PASS(iter 164/1991, GPU6,7 100%/95%·15.3GB, loss 9.76→9.73 유한). |
...
| **P46-C3only-λ0.3-DELIVER (λ 스윕 상단탐색)** | jarvis GPU4-7(4090×4, λ0.2 완주로 회수) | 200(ep30 조기게이트) | 산정 전(2.6-2.7it/s 추정, ep200까지 base 21h+) | **P0** | λ0.2 완주(200/200, test 57.05@ep108 = DELIVER test SOTA 돌파, DGFusion 56.71 대비 +0.34)로 λ 스윕(0.05/0.1/0.15/0.2) 중 test가 λ0.2에서 최고를 찍음 — λ0.05→0.2 구간 test 단조 증가 경향(56.39→56.78→57.05, 0.15는 초반이라 제외) 관측되어 **상단(λ0.3)에서 더 오르는지 확인**. config `configs/jarvis-deliver_rgbdel_P46_ctr_c3only_lam03.yaml`(develop 207b43a, lam02 대비 SAVE_DIR + C3_PROTO.LAMBDA(0.2→0.3)만 변경, 나머지 동일 — C1_RCS/C2_MCC off·C3_PROTO on·cross_view off). tmux `jemo:p46_c3_lam03`, log `logs/jarvis-deliver_rgbdel_P46_ctr_c3only_lam03/run.log`. env: `PYTHONPATH=/SSDb/jemo_maeng/pylibs_p34:.` + `PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python` + `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`, torchrun=`/home/jemo_maeng/miniconda3/envs/MMSS_SAM/bin/torchrun --nproc_per_node=4 --master_port=29553`. **기동검증 PASS**(2026-08-03 00:39): `Resumed`/`RANDOM INIT` 0건(safetensors 온라인 로드, 진짜 fresh), `[P46-C3] prototype consistency on — lambda=0.3 tau=0.1 ema=0.999 cross_view=False feature=mfeat` 확인·`[P46-C1]`/`[P46-C2]` 로그 0건, iter 16→67/995 전진(~30s, 2회 측정), GPU4,5,6,7 전부 65-100%util·~14.6GiB/rank, loss 13.64→9.86 유한 감소. GPU0(user)·GPU1-3(λ0.05 학습중, 91/87/66%util 무간섭 확인)·yeon·hpca100 무터치. **판정 게이트**: c3only와 동일(완주 test 최적 λ 확정) — λ0.2(57.05) 대비 상승/정체/하락으로 스윕 결론. |
| **P46-C3only-λ0.2-seed2-DELIVER (SOTA 재현성 검증)** | jarvis GPU1,2,3(4090×3, GPU0=user 예약) | 200(ep30 조기게이트) | 산정 전(2.6-2.7it/s 추정, ep200까지 base 21h+) | **P0** | λ0.2가 DELIVER test 57.05로 SOTA(DGFusion 56.71) 돌파했으나 **단일 run** — 논문 주장엔 재현성 필수. config `configs/jarvis-deliver_rgbdel_P46_ctr_c3only_lam02_seed2.yaml`(develop b925c90, lam02 대비 SAVE_DIR + C1_RCS.SEED(0→1, C1 off라 inert)만 변경, LAMBDA=0.2 그대로). ⚠️ 코드에 전역 SEED 키 없음(train_reliadino.py grep 확인, c3only_seed2 때와 동일 논리) — unseeded RNG + SAVE_DIR 분리만으로 독립 2nd run. jarvis 체크아웃이 untracked config 다수로 dirty해 scp로 config 전달(develop은 push 완료). tmux `jemo:p46_c3_lam02_seed2`, log `logs/jarvis-deliver_rgbdel_P46_ctr_c3only_lam02_seed2/run.log`. env: `PYTHONPATH=/SSDb/jemo_maeng/pylibs_p34:.` + `PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python` + `PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True`, torchrun=`/home/jemo_maeng/miniconda3/envs/MMSS_SAM/bin/torchrun --nproc_per_node=3 --master_port=29554`. **기동검증 PASS**(2026-08-03 10:33): `Resumed`/`RANDOM INIT` 0건(safetensors 온라인 로드, 진짜 fresh), `[P46-C3] prototype consistency on — lambda=0.2 tau=0.1 ema=0.999 cross_view=False feature=mfeat` 확인·`[P46-C1]`/`[P46-C2]` 로그 0건, iter 38→71→122/1327 전진(2회+ 측정), GPU1,2,3 전부 100%/81%/84%util·~14.9GiB/rank(활성화 수준), loss 13.64→9.71 유한 감소. GPU0(user, 4062MiB 그대로)·GPU4-7(λ0.3 학습중, 60-100%util 무간섭 확인). **판정 게이트**: test ≥56.71 재현(λ0.2 원본 57.05 재현 여부로 SOTA 주장 확정/철회). |
