...
| **Router-oracle-probe** | 오라클 지도 라우터 프로브: 고정 특징 위 소형 라우터를 H16 오라클 정답 부분집합으로 지도학습, val 정확도 − 우연 ≥ +10%p and 합성 예측 legal val ≥ +0.5 → 학습형 라우팅 재개 검토 / 미달 → 영구 폐쇄(H26) | GPU 1장 수 시간 | 🟡 등록(2026-09-07, 감사 문서 §3.5-3) — user 제안(공유 LoRA + 패치·클래스별 가중)의 재개 가능성 판정 |
