...
| **N4b** | **MCubeS C3-on 페어** (N4와 동일 config + C3_PROTO on λ0.1, 같은 시드) — **N3 진단-구동 검증의 핵심**: plaster 붕괴(0.40)를 N3 검출기가 "C3 필요"로 예측 → C3-on이 plaster 회복시키면 3벤치 진단↔효과 일치 완성(DELIVER有→도움/MUSES無→해로움/MCubeS有→도움) | hpca100 GPU2,3 (off 완주 후) — 데이터 스테이징 선행 | 🟡 **기동 보류 — 예측 재등록 선행(2026-08-25 정정)**: 문헌 대조 결과 plaster=벤치 공통난제(전방법 0~3)라 "C3가 plaster 회복" 예측 폐기. 선행 = N4 per-class 20종 vs published 대조 → **RailTrack형 격차(우리≪published) 유무 판정 → 예측 사전등록 → 기동**. 격차 존재→"C3 회복" 예측 / 부재→"C3 중립~유해(MUSES형)" 예측 — 어느 쪽이든 N3 검증 데이터. 스테이징은 계속 | B-서사 기둥. **plaster는 primary 지표에서 제외**(공통난제→limitation 소재). N3 검출기 정교화 필요: 회복가능(RailTrack형) vs 공통난제(plaster형) 구분 | 판정 [analysis/2026-08-25-n4-mcubes-first-entry-verdict.md](analysis/2026-08-25-n4-mcubes-first-entry-verdict.md) |
