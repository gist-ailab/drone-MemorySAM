...
| **14** | **Oracle 실현성 통제** (선택 입도 스윕 + 독립성 널 — 캐시 재사용, GPU 불요) | **없음**(numpy 후처리, 수 분) | 🟡 **판정에서 파생(2026-08-19)** — 즉시 실행 가능 | +8.5를 팽창 바닥 vs 실현 가능 구조로 분리. **게이트(사전등록)**: 블록16 Δ<1.0 & 관측≈독립성널 → **H16 최종 반증(SoftMoE 폐쇄)** / 블록16 Δ≥2.0 & 관측≫널 → **진짜 공간응집 → 라우터 설계 정당화(별도 제안)**. 구현=`oracle_spatial_modality.py`에 `--select-granularity`+`--null-shuffle` 추가(합성 단계만, forward 0회), labcode 위임+검수. 캐시=yeon `oracle_probe_out/deliver_c3only_lam01_ep70/`. 판정 §3 |
