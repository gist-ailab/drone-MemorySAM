...
| ⏸ **DGFusion Swin-T 재학습**(공식 설정 bs8·LR1e-4·200k) | jarvis (중단, 7장 유휴) | DELIVER CLDE | 200k iter | 🔴 **NaN 발산 3회**(iter 86,572 · 87,370 · 87,842 — `loss_ce`·`loss_contrastive` NaN, `loss_mask_*`·`loss_dice_*` 0, `loss_condition`·`loss_depth` 정상, 형태 동일; 80k 재개 후 472 iter 뒤 재발). **80k ckpt(`model_0079999.pth`) 평가 val 66.5427 ≥ 공개 66.51 → 재현 성립**(test 미측정) | 재개·설정 조정 = 생각정리 세션 판단 대기 | 공개값 재현 → depth-guided fusion 한계 분석 |
