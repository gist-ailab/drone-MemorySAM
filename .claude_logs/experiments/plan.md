...
| **P50-EXT** | P50 사전학습 확장(코퍼스/에폭 스케일업) — 제안서 "통과 시 확장" 조항 | 대규모(수일) | ⏸ 등록 + 슬롯 후보 = hpca100 0,1(현재 타유저 점유, 해방 감시) — user 판단(디스커션 B항) 대기. 확장 시 **편광/NIR proxy 추가 생성**으로 3벤치 공통 init화가 설계 포인트 | H22 이득(+0.74)이 스케일에 비례하는지 |
