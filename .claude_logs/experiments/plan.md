...
| ⏸ **DGFusion Swin-T 재학습**(공식 설정 bs8·LR1e-4·200k) | jarvis — **GPU5·7 상실(09-09)** | DELIVER CLDE | 200k iter | 🔴 **NaN 발산 4회**(iter 86,572 · 87,370 · 87,842 · **86,572 재발** — `loss_ce`·`loss_contrastive` NaN, `loss_mask_*`·`loss_dice_*` 0.0, `loss_condition`·`loss_depth` 정상, 네 번 형태 동일). **4차가 1차와 같은 iter**이고 80k ckpt 재개는 데이터 순서를 재현하므로, 원인은 최적화 설정이 아니라 **특정 배치**일 가능성이 높다. 80k ckpt(`model_0079999.pth`) 평가 **val 66.5427 > 공개값 66.51** → 재현 성립. test 1897장 평가 진행 중(09-09 16시) | — | 생각정리 세션 판정: **200k 완주 포기, 80k ckpt를 정본으로 기록**(공식 설정을 바꾸면 재현이 아님). ⚠️ 평가 스플릿 교체는 `DATASETS.TEST`가 아니라 **`DATASETS.TEST_SEMANTIC`**을 덮어써야 한다(DELIVER val 2005장 / test 1897장으로 구별). 복원 킷 `third_party/dgfusion_train_restore/` |
