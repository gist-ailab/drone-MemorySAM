...
| **15** | **no-GT 라우터 실현성 하한** (캐시 전용 합의/다수결 라우터 → 실현가능 회수분) | **없음**(캐시 argmax맵 numpy, 수 분) — confidence 변형만 forward 재실행 | 🟡 **판정에서 파생(2026-08-20)** — 즉시 실행 가능 | GT 없이 입력만으로 오라클 여지를 잡는지 = 학습 게이트 착수의 선행조건. **게이트(사전등록)**: block16에서 **어느 no-GT 변형이든 Δ≥+1.0 → 입력-실현성 확인 → 학습 게이트 착수(신 세대, user GO+GPU)** / 전부 **<+0.5 → 값싼 실현신호 부재, 학습 게이트가 유일한 결정적 검증(user 판단)**. 비대칭: no-GT 실패는 학습 게이트 실패를 증명 못함. 구현=`oracle_spatial_modality.py`에 `--realizable {majority,consensus}` 추가(캐시 전용), labcode(8/22 복귀) 또는 opus 직접. 판정 §4 |
