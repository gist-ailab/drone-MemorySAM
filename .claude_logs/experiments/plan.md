...
| **13** | **Spatial-Modality Oracle 프로브** (픽셀별 모달-부분집합 오라클 선택의 상계 측정, 학습 0) | **아무 빈 GPU 1장 × 수 시간**(순수 추론) | 🟢 **승인(2026-08-18 user)** — SoftMoE-LoRA 재개방 질의(spatial modality routing)의 천장 선측정. 시드런 완주로 jarvis 비면 즉시, 아니면 다른 유휴 1장 | user 지적(H1 반증은 SAM2 계보, spatial×modality 축 DINOv3 미검증) 정당 → CEA(H4)와 동형 오라클로 spatial 축 상계 측정. 대표 ckpt=P46 C3-only 67.79. **게이트(사전 등록)**: Δ=mIoU(oracle)−mIoU(full), **<0.3 → 축 폐쇄(H16 반증, SoftMoE 재구현 금지)** / 0.3~1.0 경계 / **≥1.0 → 라우터 설계 정당화**(별도 제안). 사전확률=Δ작음(drop-modal 한계기여≈0 + per-token 게이트 이미 공간분화). 구현=`tools/oracle_spatial_modality.py`(drop-modality keep-subset 일반화, labcode 위임+검수). 상세 [decisions/2026-08-18-spatial-modality-oracle-probe-proposal.md](../decisions/2026-08-18-spatial-modality-oracle-probe-proposal.md) |
