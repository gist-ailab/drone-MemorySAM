...
- 리부트는 **user 판단 대기**.
- 이전에 bengio 에서 돌던 E-LoRA arm A·B 시드902·903 학습(4쌍)의 생존 여부는 이 세션에서 미확인.
...
### 2026-07-23 18:37 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, ~30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | 52/300 (37% 진행) | val mIoU **79.42@ep38** (ep50 latest=79.06) | 잔여~248ep×7.3min≈30.2h → **~07-25 00:46 KST** | alive, iter 전진 |
| jarvis | 6,7 (99-100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | 38→39/300 | val mIoU **79.12@ep38**(신기록, 직전 78.94@ep30) | 잔여~262ep×7min≈30.5h → **~07-25 01:04 KST** | alive |
| jarvis | 2-5 (83-100%, ~19GB) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 16:53 신규 기동) | MUSES | **27/300** (10% 진행) | val mIoU **78.60@ep26**(신기록, ep18=77.68→ep20=76.57→ep22=74.25→ep24=77.87→ep26=78.60) | 잔여~273ep×3.7min≈16.8h → **~07-24 11:24 KST**(총 완주, 게이트는 ep30 별개) | alive, 신기록 갱신 중 |
| yeon | 2 (0%, 18MiB) | `det_D1_vitb_yeon.yaml` (openmmlab) | poongsan-det | **완주** — "Training complete. Best AP: 0.6140" | AP 0.6140/AP50 0.8953(최종 epoch)/AP75 0.6609 | — | 종료, **GPU2 유휴 전환 확인** |
| yeon | 3,4 (100%/0%, 6.5/3.4GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, 17:56 기동) | MUSES | ep2-3/300 (극초반) | val mIoU **51.81@ep2**(최초 val, 저신뢰) | 저신뢰(표본1개, ~20min/ep 추정) → 대략 07-27~28 (재측정 필요) | alive, 초반 |
| yeon | 5,6,7 (91-100%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume, 17:00 재개) | **DELIVER**(MUSES 아님) | ep40+/200 (진행 중, ep42 진입) | val mIoU 64.76@ep40 (best **65.79@ep36**) | 표본부족(재측정 필요) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **yeon GPU2 (1장, 방금 det 완주로 해제)**. 그 외 전부 비유휴 — hpca100 GPU0,1=**신규 타유저**(gr00t_finetune ur5, 92-96%, 35083MiB, 직전 점검 때 유휴였던 것과 달라짐); jarvis GPU0,1=타유저(8076/7383MiB, 0%util, mem 초과로 비유휴); yeon GPU0,1=타유저(76%/15035MiB, 0%/14796MiB).

**🔴 P39.1-MUSES(jarvis GPU2-5) ep30 게이트 상태 — 최우선 확인 결과**:
- **ep30 미도달** — 실측 시각(18:37 KST) 기준 **현재 epoch 27** 진행 중(10%). 코디네이터 예상(~18:35)보다 지연, 현재 페이스(ep24→ep26=7min/2ep≈3.5min/ep)로 잔여 3ep(28,29,30) ≈ **+11-14분 → ep30 도달 예상 ~18:50 KST 전후** (아직 미도달).
- **lidar eff.rank 궤적** (매 eval 로그 직접 파싱, 게이트 기준 ≥15): ep18=385.8 → ep20=386.5 → ep22=395.1 → ep24=409.8 → **ep26=417.5**. 기준(≥15) 대비 압도적으로 통과 추세 — ep30에서도 유지될 가능성 높음(판정은 상위 모델 몫).
- **val mIoU 궤적**: ep18=77.68(당시 best) → ep20=76.57 → ep22=74.25(일시하락) → ep24=77.87(신기록) → **ep26=78.60(신기록, 최신)**.
- **fog_night drop-lidar dMIoU**: 🔴 **이번 학습 로그에 없음** — 학습 중 매 eval에 자동 출력되는 스칼라가 아니라 별도 오프라인 ablation(체크포인트 저장 후 module_diagnostics류 스크립트로 산출)으로 보임. ep30 체크포인트가 아직 안 나온 상태라 계산 자체가 안 됐을 가능성. **다음 점검에서 ep30 ckpt 저장 확인 후 별도 ablation 실행 여부를 봐야 함**.
- 참고(다른 데이터셋, 과거 기록): yeon `P39.1-DELIVER` cfg 주석에 이전 attempt의 ep30 결과가 남아있음 — "lidar eff.rank=532.1(≥15 게이트 통과), 단 drop-lidar dMIoU(night)=-0.78 (실제 lidar 인과기여는 ~0/음)" — **DELIVER 기준이며 이번 jarvis MUSES 게이트와는 별개 실험**, 판정 아님.

**이상징후**:
- ⚠️ hpca100 GPU0,1이 지난 점검(idle) 대비 **신규 타유저 점유로 전환**(gr00t_finetune, 92-96%) — 우리 작업과 무관하나 유휴 자원 소멸.
- yeon P42-f07: 표본 1개(ep2)뿐이라 ETA 신뢰도 낮음, 다음 점검에서 재측정 필요.
- yeon P39.1-DELIVER: 두 번째 "epoch:40 mIoU:53.01" 라인이 로그에 별도로 존재(별개 eval variant로 추정, 원인 미조사 — 다음 점검 때 확인 필요).
- Traceback/OOM/nan 없음(주요 로그 확인 범위 내), D-state 없음, 전 추적 run(6개) iteration 정상 전진·util>0 확인.

### 2026-07-23 20:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 79 돌파여부 | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/95%, ~30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | 68/300 (82% 진행) | val mIoU **79.65@ep60** | ✅ 돌파(ep38부터), 단 최근 3회(ep62=78.97,ep64=78.97,ep66=79.24) **신기록 없이 정체** | 잔여~232ep×7.2min≈27.8h → **~07-25 00:20 KST** | alive |
| jarvis | 6,7 (91-100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | 54→55/300 | val mIoU **79.93@ep54(신기록, 20:22:42 방금 갱신)** | ✅ 돌파, **상승 지속 중**(ep48=78.80→ep50=79.40→ep52=78.81→ep54=79.93) | 잔여~245ep×7min≈28.6h → **~07-25 01:10 KST** | alive |
| jarvis | 2-5 (93-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS 확정) | MUSES | 58→59/300 | val mIoU **🔴80.06@ep54 — 3개 MUSES run 중 최초 80 돌파** | ✅✅ **80 돌파**, 최근 2회(ep56=79.74,ep58=79.71) 피크 대비 소폭 하락하나 79 이상 유지 | 잔여~241ep×3.6min≈14.5h → **~07-24 11:04 KST**(완주) | alive |
| yeon | 2 (0%, 20MiB) | (없음 — ViT-B det 완주로 해제) | — | — | — | — | — | 🔴 **여전히 유휴 확인**(nvidia-smi --query-compute-apps에 GPU2 프로세스 0건, 타유저 미점유) |
| yeon | 3,4 (100%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | 6→7/300 (초반) | val mIoU **72.04@ep6(신기록, 상승세: 51.81→66.63→72.04)** | ❌ 미돌파(아직 72대, 초반이라 예상됨) | 잔여~294ep×22min≈107.8h≈4.5일 → **~07-28 06:00 KST경**(표본 적음, 저신뢰) | alive |
| yeon | 5,6,7 (73-100%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | 44→45/200 | val mIoU 65.54@ep44 (best 65.79@ep36, 아직 미갱신) | 해당없음(DELIVER는 별도 스케일) | 잔여~156ep×31min≈80.6h≈3.4일 → **~07-27 05:00 KST경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **yeon GPU2 (1장, 여전히 유휴 — 타유저 미점유 재확인)**. 그 외 전부 비유휴 — hpca100 GPU0,1=타유저(gr00t_finetune, 99-100%, 35083MiB, 지속); jarvis GPU0,1=타유저(8076/7383MiB, 0%util, mem 초과로 비유휴); yeon GPU0,1=타유저(jongwon_kim hoi_transformer, 100%/68%, 15047/14806MiB — GPU1엔 신규 PID 2922116 확인, 동일 유저 잡의 두 번째 인스턴스로 추정).

**MUSES 79 돌파 요약 (3-run 비교)**:
- 🔴 **jarvis P39.1-MUSES가 최초로 80 돌파**(80.06@ep54) — 3개 MUSES run 중 유일하게 80대 진입.
- jarvis P42-f03: 79대 상승 지속, 방금 79.93 신기록(20:22:42) — 80 근접.
- hpca100 P42 main: 79.65@ep60에서 정체 — 최근 3회 연속 신기록 갱신 실패(78.97/78.97/79.24), 판정은 상위 모델 몫이나 수치상 정체 국면.
- yeon P42-f07: 아직 79 미돌파(초반, ep6/300).

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). hpca100 P42 main만 최근 3-eval 신기록 정체(⚠️, 판정 아님). 전 6개 run iteration 정상 전진·util>0 확인.

### 2026-07-23 22:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(20:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, ⚠️7.4-7.7GB — 직전 대비 급감, 아래 참조) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~85/300 | val mIoU **79.92@ep78(신기록)** | **회복** — 직전 정체(79.65@60) 돌파, ep74=79.91(신기록)→ep78=79.92(신기록) 이후 79.3-79.9대 유지 | 잔여~215ep×7.3min≈26.2h → **~07-25 00:46 KST** | alive |
| jarvis | 6,7 (100%/93%, ~17.8-19GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~73/300 | val mIoU **80.28@ep56**(80 돌파 확정) | ep70=80.28(동률)·ep72=80.27 — **ep56 이후 신기록 없음(정체 국면 진입)** | 잔여~227ep×6.9min≈26.1h → **~07-25 00:40 KST** | alive |
| jarvis | 2-5 (91-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~91/300 | val mIoU **🔴81.04@ep82(신기록)** | ep82 이후(84=80.11,86=80.48,88=80.29,90=80.02) **신기록 없이 하락 추세**, P38 내부최고 82.22 대비 **-1.18pt 남음(아직 미근접)** | 잔여~209ep×3.6min≈12.5h → **~07-24 11:04 KST**(완주) | alive |
| yeon | 2 (28%, 1766MiB) | (없음 — 우리 작업 아님) | — | — | — | 🔴 **뺏김 확인** — 직전 유휴였으나 신규 타유저(seungyeon_cheon, PID 3138455, 1731MiB) 점유 시작 | — | 타유저 점유 중 |
| yeon | 3,4 (100%/61%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~13/300 | val mIoU **75.24@ep12(신기록)** | 꾸준 상승(72.04→74.69→74.80→75.24), 아직 79 미돌파 | 잔여~287ep×21min≈100.5h≈4.2일 → **~07-28 03:00 KST경**(저신뢰) | alive |
| yeon | 5,6,7 (100%/99%/86%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~49/200 | val mIoU 64.14@ep48 (best 65.79@ep36, **5회 연속(40,42,44,46,48) 미갱신, 최신값이 최근 중 최저**) | ⚠️ 정체/소폭 하락 추세 지속 | 잔여~151ep×31.5min≈79.3h≈3.3일 → **~07-27 02:00 KST경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 GPU 없음** (yeon GPU2가 방금 타유저에게 뺏기면서 마지막 유휴 자원 소멸). hpca100 GPU0,1=타유저(98-100%,35GB); jarvis GPU0,1=타유저(0%util,mem초과); yeon GPU0,1=타유저(0%/85%,~15GB); yeon GPU2=신규 타유저(28%,1.7GB).

**MUSES 3-run 최신 best 및 신기록 여부 요약**:
- 🔴 **jarvis P39.1-MUSES 선두 유지, 81.04@ep82(신기록)** — P38 내부최고 82.22 대비 -1.18pt, 아직 미근접. 단 ep82 이후 4회 연속(84/86/88/90) 신기록 없이 80.0-80.5대로 하락.
- jarvis P42-f03: 80.28@ep56에서 **정체** — 이후 6회 평가(62,64,66,68,70,72)에서 최고 80.28 동률(ep70)뿐, 초과 없음.
- hpca100 P42 main: 직전 정체(79.65@60) 이후 **회복** — ep74·ep78에서 연속 신기록(79.91→79.92), 현재 79.8-79.9대.

**이상징후**:
- ⚠️ hpca100 GPU2,3 메모리 사용량이 직전 점검(~30.6GB) 대비 이번 점검 **7.4-7.7GB로 급감**(util은 100%/100% 유지) — ps상 동일 메인 프로세스(1866319/1866320, 03:21 시작) 그대로 생존 중이며 신규 짧은수명 프로세스(13:32 시작, 8개)가 추가로 보임. 재시작/크래시 흔적은 로그에 없음(val 궤적이 ep72→84까지 연속·정상). **원인 미규명, 다음 점검에서 재확인 필요**(판정 아님).
- 🔴 yeon GPU2 유휴 해제(타유저 점유 시작, 위 참조).
- yeon P39.1-DELIVER 5연속 미갱신 + 최신값 하락(⚠️, 판정 아님).
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진·util>0.

### 2026-07-24 00:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(22:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, 30.6GB — 정상 복귀) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~101/300 | val mIoU **80.41@ep88(신기록, 80 돌파)** | 직전(79.92@78) 대비 상승 — 단 이후 6회(90-100: 79.84/80.30/80.01/79.72/80.26/80.07) 신기록 없이 79.7-80.3대 유지 | 잔여~199ep×7.25min≈24h → **~07-25 00:34 KST** | alive |
| jarvis | 6,7 (81-100%, ~17.8-19GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~91/300 | val mIoU **80.44@ep86(신기록)** | 직전 정체(80.28@56) **돌파** — ep80=80.40(신기록)→ep86=80.44(신기록), 이후 ep88=80.42/ep90=80.40 유지 | 잔여~209ep×6.9min≈24h → **~07-25 00:34 KST** | alive |
| jarvis | 2-5 (98-100%, ~19GB) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~125/300 | val mIoU **81.37@ep92(신기록, 갱신됨)** | 직전(81.04@82) 대비 **상승**(+0.33pt) — 단 ep92 이후 8회 연속(ep110~124) 신기록 없음, 82.22 대비 **-0.85pt 남음(미돌파, 격차는 축소)** | 잔여~175ep×3.7min≈10.8h → **~07-24 11:22 KST**(완주) | alive |
| yeon | 2 (13%, 16562MiB, 타유저 2건 점유) | (없음) | — | — | — | 여전히 비유휴 — 기존 jongwon_kim(14754MiB) + 신규 seungyeon_cheon(1731MiB) 동시 점유 확인 | — | 타유저 점유 지속 |
| yeon | 3,4 (0%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~17/300 | val mIoU **75.99@ep14(신기록)** | 꾸준 상승 지속(72.04→74.69→74.80→75.24→75.99→75.93), 79 미돌파 | 잔여~283ep×28.5min≈134.4h≈5.6일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (100%/100%/99%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~53/200 | val mIoU 64.46@ep52 (best 65.79@ep36 불변, ep42~52: 64.74/65.54/64.78/64.14/64.71/64.46) | ⚠️ 최고치 대비 여전히 낮으나 직전 최저치(64.14@48)보다는 소폭 회복, 뚜렷한 개선/악화 아님(횡보) | 잔여~147ep×31.5min≈77.2h≈3.2일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(직전과 동일)**. hpca100 0,1=타유저(76-99%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1=타유저(70%/0%,~15GB); yeon 2=타유저 2건(13%,16.5GB, 위 참조); yeon 3=자사 job이나 순간 0%util(mem 17GB 보유, DDP rank 유휴 추정, 비유휴 판정).

**🔴 P39.1-MUSES(jarvis 2-5) 최우선 확인 — 82.22 대비 상태**:
- **현재 best: 81.37@ep92** (직전 점검 81.04@ep82에서 +0.33pt 갱신 확인됨. 단 그 갱신은 이번 점검 윈도우 이전(22:36 무렵)에 이미 일어났고, 이후로는 정체).
- **최근 5 eval 궤도** (ep116→124, 6개 중 최근 5개): **ep116=81.19 / ep118=81.28 / ep120=80.84 / ep122=81.30 / ep124=81.10** — 전부 81.37(ep92) 미만, **8회 연속(ep110~124) 신기록 없음**.
- **82.22(P38 내부최고) 대비 격차: -0.85pt** (직전 -1.18pt에서 축소됐으나 여전히 미돌파).
- 판정은 상위 모델 몫 — 수치만 보고: 격차는 좁혀졌으나 최근 8-eval 동안 자체 최고치(81.37)조차 재돌파 못하는 정체 국면.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). hpca100 GPU2,3 메모리 이번엔 정상(30.6GB)로 복귀 — 직전 점검(7.4-7.7GB) 이상은 일시적이었던 것으로 보임(원인 미규명, 재확인 완료 표기만). yeon GPU3 순간 0%util은 DDP 2-rank 중 하나의 순간 관측치로 추정(비정상 단정 아님).

### 2026-07-24 02:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(00:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (88%/78%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~117/300 | val mIoU 80.41@ep88 (**불변**) | ep106-116(6회: 80.34/79.93/79.90/79.84/79.64/80.08) **신기록 없음** — 정체 재확인 | 잔여~183ep×7.3min≈22.3h → **~07-25 00:50 KST** | alive |
| jarvis | 6,7 (100%/100%, ⚠️6.9-7.2GB — mem 급감, hpca100서 봤던 패턴 재현) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~107/300 | val mIoU **81.10@ep100(신기록, 81 돌파)** | 직전(80.44@86) 대비 **상승** — ep100 신기록 후 ep102-106(80.52/80.14/80.83)은 그 아래 | 잔여~193ep×7min≈22.5h → **~07-25 01:04 KST** | alive |
| jarvis | 2-5 (81-94%, ~19GB) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~157/300 | val mIoU **81.76@ep150(신기록)** | 직전(81.37@92) 대비 **+0.39pt 상승** — 82.22 대비 **-0.46pt 남음(격차 지속 축소: 1.18→0.85→0.46)**, 아직 미돌파 | 잔여~143ep×3.65min≈8.7h → **~07-24 11:16 KST**(완주, 이번 점검 윈도우 내 미완주 — 아직 약 8.7시간 남음) | alive |
| yeon | 2 (92%, 16564MiB, 타유저 2건) | (없음) | — | — | — | 여전히 비유휴, 동일 두 타유저(jongwon_kim+seungyeon_cheon) 유지 | — | 타유저 점유 지속 |
| yeon | 3,4 (100%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~21/300 | val mIoU **76.84@ep20(신기록)** | 꾸준 상승 지속(75.99→75.93→76.65→76.84), 79 미돌파 | 잔여~279ep×30min≈139.5h≈5.8일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (99-100%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~57/200 | val mIoU 65.18@ep56 (best 65.79@ep36 불변) | ep46-56(6회: 64.78/64.14/64.71/64.46/64.75/65.18) 신기록 없음, 단 최신값(65.18)이 이 구간 중 최고치로 소폭 회복세 | 잔여~143ep×31.5min≈75.1h≈3.1일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(4회 연속 동일)**. hpca100 0,1=타유저(78-81%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1=타유저(0-74%,~15GB); yeon 2=타유저 2건(92%,16.5GB, 위와 동일).

**🔴 P39.1-MUSES(jarvis 2-5) 궤적 — 82.22 대비, 완주 임박 여부**:
- **현재 best: 81.76@ep150** (직전 81.37@ep92에서 +0.39pt 갱신). **82.22 대비 -0.46pt 남음 — 격차 3연속 축소(1.18→0.85→0.46), 아직 미돌파.**
- **최근 4 eval**: ep150=**81.76(peak)** / ep152=81.60 / ep154=81.41 / ep156=81.61 — peak 이후 3회 모두 그 아래(하락 없이 근접 유지, 붕괴 아님).
- **완주 임박 여부**: **아직 아님** — 현재 epoch ~157/300, ETA ~07-24 11:16 KST로 **약 8.7시간 남음**(이전 점검의 ~11시 추정과 일치, 이번 2h 윈도우 내 완주 없음).

**이상징후**:
- ⚠️ jarvis GPU6,7(P42-f03) 메모리가 이번엔 6.9-7.2GB로 급감(직전 17.8-19GB 수준) — hpca100 GPU2,3에서 이전에 관측된 것과 동일 패턴(일시적 메모리 표시 급감, util은 100% 유지, 로그상 재시작/에러 흔적 없음). **원인 미규명, 반복 관측됨(2회째) — 다음 점검에서 추가 확인 필요.**
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진·util>0.

### 2026-07-24 04:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(02:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (99%/99%, 30.6GB — 정상) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~133/300 | val mIoU **🔴80.85@ep124(신기록 — 장기 정체 돌파)** | 직전(80.41@88, 다회 정체) **완전 돌파** — ep126-132(80.26/80.24/80.25/80.31)는 그 아래지만 신기록 자체는 확정 | 잔여~167ep×7.3min≈20.3h → **~07-25 00:50 KST** | alive |
| jarvis | 6,7 (90-100%, ~17.8-19.3GB — 정상 복귀) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~125/300 | val mIoU **81.20@ep124(신기록)** | 직전(81.10@100) 대비 상승, 계속 신기록 경신 중 | 잔여~175ep×7min≈20.4h → **~07-25 01:00 KST** | alive |
| jarvis | 2-5 (90-100%, ~19GB) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~191/300 | val mIoU **81.76@ep150(불변)** | **82.22 미돌파, 격차 -0.46 그대로**(직전과 동일) — ep150 이후 12회+ 연속(152~190) 신기록 없음, 가장 긴 정체 구간 | 잔여~109ep×3.64min≈6.6h → **~07-24 11:10 KST**(완주, 이번 점검서도 미완주·아직 6.6시간) | alive |
| yeon | 2 (33%, 16570MiB, 타유저 2건) | (없음) | — | — | — | 여전히 비유휴, 동일 두 타유저 유지 | — | 타유저 점유 지속 |
| yeon | 3,4 (0%/0%, ~3.3GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~25/300 | val mIoU **🔴78.34@ep24(신기록 — 79 문턱 근접)** | 직전(76.84@20)서 큰 폭 상승(+1.5pt), 79 돌파 임박 | 잔여~275ep×약28min≈128.3h≈5.3일 → **~07-29 경**(저신뢰) | alive(단 GPU3,4 순간 0%util·mem 3.3GB — 학습 스텝 사이 순간 관측 추정, ckpt는 3분 전 갱신되어 생존 확인됨) |
| yeon | 5,6,7 (92-99%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~61/200 | val mIoU 65.36@ep60 (best 65.79@ep36 불변, ep58=65.77로 **거의 근접**했다가 ep60 소폭 하락) | 최고치에 가장 근접(0.02pt 차)했던 ep58 이후 소폭 하락, 여전히 미갱신 | 잔여~139ep×31.5min≈73h≈3.0일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(5회 연속)**. hpca100 0,1=타유저(96-99%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1=타유저(0-8%,~15GB,mem초과); yeon 2=타유저 2건(33%,16.5GB); yeon 3,4=자사 job(0%util 순간이나 mem 3.3GB>2000 → 비유휴 판정).

**🔴 P39.1-MUSES(jarvis 2-5) — 82.22 돌파 여부 & 완주 임박**:
- **미돌파 지속**: best 81.76@ep150 **불변**(직전 점검과 동일). 82.22 대비 **-0.46pt, 격차 정체**(더 이상 축소 없음).
- **최근 4 eval**: ep184=81.01 / ep186=81.42 / ep188=81.59 / ep190=81.57 — 전부 81.76 미만, ep150 이후 **12회 이상 연속 신기록 없음**(가장 긴 정체 구간 관측).
- **완주 임박**: **아직 아님** — 현재 ~ep191/300, ETA ~07-24 11:10 KST(약 6.6시간 남음). 지난 점검(8.7h)보다 가까워졌으나 이번 2h 윈도우 내 완주는 없었음.

**완주 임박 런**: 없음(전 6개 run 진행 중). P39.1-MUSES가 6개 중 가장 임박(~6.6h).

**이상징후**:
- ⚠️ GPU 메모리 급감 패턴 **이번엔 재발 안 함** — hpca100 GPU2,3(30.6GB) · jarvis GPU6,7(17.8-19.3GB) 모두 정상 범위로 확인. 지난 2회 관측된 현상은 일시적이었던 것으로 보임(원인 미규명 상태 유지).
- yeon GPU3,4 순간 0%util(위 참조, ckpt 갱신으로 생존 확인) — 경미, 판정 아님.
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진.

### 2026-07-24 06:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(04:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, ⚠️7.3-7.4GB — mem 재급감, 3번째 관측) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~149/300 | val mIoU 80.85@ep124 (**불변**) | ep138-148(6회: 80.36/80.18/79.91/80.71/80.50/80.13) 신기록 없음 — 재정체 | 잔여~151ep×7.3min≈18.4h → **~07-25 01:00 KST** | alive |
| jarvis | 6,7 (91-100%, ~17.8GB 정상) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~143/300 | val mIoU 81.20@ep124(=ep128 동률) (**불변**) | ep132-142(6회: 80.86/81.18/81.05/80.60/80.41/80.70) 신기록 없음 — 정체 국면 | 잔여~157ep×7min≈18.3h → **~07-25 01:00 KST** | alive |
| jarvis | 2-5 (92-100%, ~19GB) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~223/300 | val mIoU **🔴81.86@ep208(신기록 — 12eval 정체 돌파)** | 직전(81.76@150) 대비 +0.10pt 갱신, **82.22 대비 -0.36pt(격차 추가 축소)** — 아직 미돌파, **완주 미완료**(진행 중) | 잔여~77ep×3.7min≈4.75h → **~07-24 11:19 KST**(직전 추정 11:10과 일치, 아직 진행 중) | alive |
| yeon | 2 (100%, 16570MiB, 타유저 2건) | (없음) | — | — | — | 여전히 비유휴, 동일 두 타유저 유지 | — | 타유저 점유 지속 |
| yeon | 3,4 (100%/100%, ⚠️6.3-6.6GB — mem 급감, 신규 관측) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~29/300 | val mIoU 78.34@ep24 (**불변**) | ep26=76.49(하락)→ep28=77.86(회복), 아직 79 미돌파, best 갱신 없음 | 잔여~271ep×약27min≈122h≈5.1일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (62-98%, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~65/200 | val mIoU 64.10@ep64 (best 65.79@ep36 불변) | ⚠️ ep58=65.77(근접)→ep60=65.36→ep62=64.31→ep64=64.10 **뚜렷한 하락 추세 지속**(3회 연속 하락) | 잔여~135ep×31.5min≈70.9h≈3.0일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(6회 연속)**. hpca100 0,1=타유저(81-96%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1=타유저(0-100%,~15GB); yeon 2=타유저 2건(100%,16.5GB).

**🔴 P39.1-MUSES(jarvis 2-5) 상태 — 정체 돌파, 완주 미완료**:
- **완주 안 됨** — 완료 마커(`Training complete`/`Total Training Time`) 로그에 없음, 여전히 진행 중(현재 ~ep223/300). ETA는 여전히 미래 시점(~07-24 11:19 KST, 약 4.75시간 후).
- **12-eval 정체를 돌파하고 신기록 달성**: **81.86@ep208** (직전 81.76@150에서 +0.10pt). 82.22 대비 격차 **-0.36pt로 추가 축소**.
- 최근 궤도(ep204~222): 81.68 / 81.19 / **81.86(peak)** / 81.79 / 81.27 / 81.65 / 81.45 / 81.46 / 81.42 / 81.72 — peak 이후 81.2-81.8 범위 유지, 붕괴 없음.

**완주 임박 런**: P39.1-MUSES(jarvis 2-5)가 유일하게 임박(~4.75h, ~07-24 11:19 KST). 나머지 5개 run은 전부 12h+ 이상 남음.

**이상징후**:
- ⚠️ **GPU 메모리 급감 패턴 재관측**(누적 3회째: hpca100 GPU2,3 → jarvis GPU6,7 → hpca100 GPU2,3 재발) + **신규 4번째 사례(yeon GPU3,4, P42-f07)**. 4개 서로 다른 run·2개 서버에 걸쳐 반복 관측 — util은 항상 100% 유지, val 궤적 연속·정상, 재시작/에러 흔적 없음. **여러 run에 공통되는 패턴으로 보이나 원인은 여전히 미규명**(판정 아님, 다음 점검서도 계속 추적).
- yeon P39.1-DELIVER 3연속 하락(65.77→65.36→64.31→64.10) — ⚠️ 최근 최저치 근접, 판정 아님.
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진.

### 2026-07-24 08:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(06:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (99%/67%, 30.6GB 정상) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~167/300 | val mIoU 80.85@ep124 (**불변**) | ep156-166(6회: 79.82/80.04/79.97/80.21/80.60/80.24) 신기록 없음 — 정체 지속(6+eval째) | 잔여~133ep×7.3min≈16.2h → **~07-25 00:45 KST** | alive |
| jarvis | 6,7 (99-100%, ~17.8GB 정상) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~161/300 | val mIoU **81.36@ep150(신기록, 소폭 갱신)** | 정체(81.20@124) 돌파 — 이후 ep152-160(81.33/80.84/80.90/80.85/81.22)는 그 아래 | 잔여~139ep×7min≈16.2h → **~07-25 00:45 KST** | alive |
| jarvis | 2-5 (⚠️7.4-8.0GB — mem 재급감, 5번째 관측) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~255/300 | val mIoU **🔴82.03@ep224(신기록)** | 직전(81.86@208) 대비 +0.17pt — **82.22 대비 -0.19pt, 최근접**(격차 0.46→0.36→0.19로 지속 축소) | 잔여~45ep×3.7min≈2.8h → **~07-24 11:20 KST**(직전 추정과 거의 일치, **아직 진행 중 — 완주 안 됨**) | alive |
| yeon | 2 (1%, 16570MiB, 타유저 2건) | (없음) | — | — | — | 여전히 비유휴, 동일 두 타유저 유지 | — | 타유저 점유 지속 |
| yeon | 3,4 (100%/100%, ~17GB 정상) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~35/300 | val mIoU **78.37@ep32(신기록, 소폭)** | ep26=76.49→ep28=77.86→ep30=77.01→ep32=78.37(신기록)→ep34=77.27 — 등락 지속, 아직 79 미돌파 | 잔여~265ep×약27min≈119h≈5.0일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (100%×3, ~21GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~69/200 | val mIoU 64.48@ep68 (best 65.79@ep36 불변) | ep62-68(64.31/64.10/64.26/64.48) 저점에서 소폭 반등 조짐이나 여전히 best 대비 -1.3pt | 잔여~131ep×31.5min≈68.8h≈2.9일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(7회 연속)**. hpca100 0,1=타유저(99-100%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1=타유저(mem초과); yeon 2=타유저 2건(1%,16.5GB — util 낮지만 mem초과라 비유휴).

**🔴 P39.1-MUSES(jarvis 2-5) — 82.22 대비 최우선 확인**:
- **완주 안 됨** — 완료 마커 없음, 현재 ~ep255/300 진행 중. ETA ~07-24 11:20 KST(약 2.8시간 후, 직전 추정과 거의 일치).
- **최종 아님, 최신 best: 82.03@ep224(신기록)** — 82.22 대비 **-0.19pt, 지금까지 가장 근접**(격차 추이: 1.18→0.85→0.46→0.36→0.19, 4연속 축소).
- **최근 궤도(ep236~254)**: 81.86 / 81.79 / 81.91 / 81.78 / 81.54 / 81.64 / 81.89 / 81.81 / 81.82 / 81.70 — peak(82.03) 이후 81.5-81.9대 유지, 붕괴 없음.

**이상징후**:
- ⚠️ GPU 메모리 급감 패턴 **5번째 관측**(jarvis GPU2-5, P39.1-MUSES, 7.4-8.0GB) — 이번엔 P39.1-MUSES 자신에서 발생(그동안 P42 main/f03/f07에서만 봤음). 4개 run·2개 서버에 걸쳐 총 5회, util 항상 100% 유지, val 궤적 연속·정상 — 원인 미규명이나 광범위한 재현성으로 볼 때 측정/리포팅 방식 자체의 특성일 가능성(판정 아님, 계속 추적).
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진.

### 2026-07-24 10:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | 최신 vs 직전(08:34) | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (69%/100%, 30.6GB 정상) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~183/300 | val mIoU 80.85@ep124 (**불변, 장기정체**) | ep172-182(6회: 80.16/80.52/80.33/80.55/79.75/80.23) 신기록 없음 | 잔여~117ep×7.3min≈14.2h → **~07-25 00:45 KST** | alive |
| jarvis | 6,7 (100%/98%, ~17.8GB 정상) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~177/300 | val mIoU 81.36@ep150 (**불변**) | ep166-176(6회: 80.89/81.12/80.69/81.19/81.02/81.16) 신기록 없음 — 정체 지속 | 잔여~123ep×7min≈14.4h → **~07-25 01:00 KST** | alive |
| jarvis | 2-5 (98-100%, ~19GB 정상 복귀) | 🔴 `jarvis-muses_rgbel_P39_1_rank.yaml` (P39.1-MUSES, 게이트 PASS) | MUSES | ~289/300 (**잔여 11ep**) | val mIoU 82.03@ep224 (**불변**) | ep270-288(10회: 81.96/81.86/81.79/81.80/81.86/81.95/81.94/81.97/81.92/81.84) 신기록 없음(27+회째 정체), 82.22 대비 -0.19 그대로 | 잔여~11ep×3.7min≈41분 → **~07-24 11:15 KST**(**미완주, 매우 임박**) | alive |
| yeon | 2 (100%, 16570MiB, 타유저 2건) | (없음) | — | — | — | 여전히 비유휴, 동일 두 타유저 유지 | — | 타유저 점유 지속 |
| yeon | 3,4 (68%/86%, ~17GB 정상) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~41/300 | val mIoU **78.42@ep40(신기록, 근소)** | ep34-38(77.27/77.15/77.70) 이후 ep40=78.42로 회복·신기록, 79 근접 | 잔여~259ep×약27min≈116.6h≈4.9일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (99-100%, ⚠️3.6-3.8GB — mem 급감, 6번째 관측) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~73/200 | val mIoU 65.04@ep72 (best 65.79@ep36 불변) | ep64-72(64.10→64.26→64.48→64.85→65.04) **4연속 상승 — 저점 회복 뚜렷** | 잔여~127ep×31.5min≈66.7h≈2.8일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음(8회 연속)**. hpca100 0,1=타유저(74-99%,35GB); jarvis 0,1=타유저(0%util,mem초과); yeon 0,1,2=타유저(mem초과).

**🔴 P39.1-MUSES(jarvis 2-5) — 완주 확인**:
- **아직 미완주** — 완료 마커("Training complete"/"Total Training Time") 로그에 없음, `best_checkpoint.pth` 경로도 아직 미생성(`ls` 실패 확인). 현재 **~ep289/300, 잔여 11epoch**.
- **최종 아님, 현재 best: 82.03@ep224** — 지난 점검과 완전 동일(27회+ 연속 미갱신), 82.22 대비 **-0.19pt 그대로**.
- **매우 임박**: ETA ~07-24 11:15 KST(약 41분 후) — 이번 점검 직후 완주 가능성 높음. GPU2-5 아직 미해방.

**이상징후**:
- ⚠️ GPU 메모리 급감 패턴 **6번째 관측**(yeon GPU5,6,7, P39.1-DELIVER, 21GB→3.6-3.8GB) — 이번엔 P39.1-DELIVER에서 발생(P42 main/f03/f07/P39.1-MUSES에 이어 5개 run 전부에서 최소 1회씩 관측됨). util 항상 100% 유지, 로그 연속·정상 — 전 run 공통 패턴으로 굳어짐(원인 미규명, 판정 아님).
- Traceback/OOM/nan 없음, D-state 없음, 전 6개 run iteration 정상 전진.

### 2026-07-24 12:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 P39.1-MUSES(jarvis 2-5) 완주 확인 — COMPLETED**
- 로그 종료 확인: `Total Training Time  18:20:57`, epoch300 최종 val mIoU=82.00, **Best Val mIoU 82.03 (epoch 224)** — 최종 결과, **82.22 미돌파로 확정**(격차 -0.19pt, 마지막까지 축소 정체).
- ckpt: `epoch298_82.03_top2_checkpoint.pth` / `epoch300_82.0_top4_checkpoint.pth` / `last_checkpoint.pth` (모두 11:07-11:14 KST 저장) — 경로: `/home/jemo_maeng/src/drone-MemorySAM-develop/outputs/ReliaDINO/jarvis_muses_rgbel_P39_1_rank/`. `best_checkpoint.pth`(고정명) 파일은 미확인(top-K 네이밍만 존재, 별도 심볼릭 없음 — 최고점은 `epoch224` 계열 top1 ckpt로 추정, 정확한 파일명 재확인 필요).
- **프로세스 종료 확인**: `ps`에 train_reliadino/P39_1_rank 프로세스 전무. **GPU2,3,4,5 완전 해방 — 유휴 확인**(0%util, 602/570/538/26MiB, 전부 잔여 타유저 프로세스(506MiB급, 9일+ 경과 zombie성)뿐, **타유저에게 뺏기지 않음**).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (99%/98%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~199/300 | val mIoU 80.85@ep124 (**불변, 장기정체 지속**) | 잔여~101ep×7.3min≈12.3h → **~07-25 00:50 KST** | alive |
| jarvis | 6,7 (99%/79%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~195/300 | val mIoU 81.36@ep150 (**불변**) | 잔여~105ep×7min≈12.25h → **~07-25 01:00 KST** | alive |
| jarvis | 2,3,4,5 | **(없음 — P39.1-MUSES 완주, 위 참조)** | — | — | 최종 82.03@ep224 | **완주됨** | 🔴 **유휴 확인(4장)** |
| yeon | 2 (97%, 16570MiB, 타유저 2건) | (없음) | — | — | — | — | 타유저 점유 지속 |
| yeon | 3,4 (57%/91%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~47/300 | val mIoU **78.65@ep42(신기록, 근소) — 79 문턱 0.35pt 앞** | 잔여~253ep×약28min≈118h≈4.9일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 (98-100%, ~3.7-3.8GB) | `yeon-deliver_rgbdel_P39_1_rank_resume.yaml` (P39.1-DELIVER resume) | **DELIVER**(MUSES 아님) | ~77/200 | val mIoU 64.93@ep76 (best 65.79@ep36 불변, ep74=65.27로 최근접 후 소폭 하락) | 잔여~123ep×31.5min≈64.6h≈2.7일 → **~07-27 경** | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 jarvis GPU2,3,4,5 (4장) — P39.1-MUSES 완주로 신규 해방, 타유저 미점유 확인**. 그 외: hpca100 0,1=타유저(76-84%,35GB, 비유휴); jarvis 0,1=타유저(0%util,mem초과, 비유휴); yeon 0,1,2=타유저(mem초과 또는 고util, 비유휴). **총 유휴 4장(전부 jarvis).**

**완주 임박 런**: 이번 점검에서 P39.1-MUSES 완주 확정. 나머지 5개 run은 전부 12h+ 남음 — 임박 없음.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음 — 타유저 zombie/D-state는 기존부터 있던 것과 동일, 우리 job과 무관). GPU 메모리 급감 패턴은 이번 점검에서 관측 안 됨(hpca100/jarvis 모두 정상 수준, yeon 5-7만 여전히 낮은 값 지속 — 6회차부터 이어지는 상태로 신규 아님).

### 2026-07-24 14:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 yeon P39.1-DELIVER(GPU5,6,7) — 사망 확인(CUDA OOM)**
- `ps`에 해당 프로세스 전무, `nvidia-smi` GPU5,6,7 = 0%util/16MiB(완전 유휴) — 학습 사망 확인.
- 로그 원문: `Epoch [79/200]... torch.cuda.OutOfMemoryError: CUDA out of memory. Tried to allocate 5.71 GiB. GPU 2(로컬랭크) has a total capacty of 23.56 GiB of which 3.26 GiB is free...` → `torch.distributed.elastic...ChildFailedError` → `train_reliadino.py FAILED`, **시각 2026-07-24 13:43:01 KST**, local_rank 2(물리 GPU7) OOM으로 전체 DDP 종료.
- 사망 직전 마지막 val: ep78 mIoU=65.67 (13:33:23, best 65.79@ep36 그대로 미갱신 상태로 종료). 최종 best는 **65.79@ep36로 고정**(더 이상 개선 없이 사망).
- kill 등 개입 없음(순수 OOM), 재시작은 판정 사항이라 수행하지 않음 — 사실만 보고.

**🆕 jarvis 2-5 = P39.1-seed2 — 신규 기동 확인, 생존·상승 중**
- cfg: `configs/jarvis-muses_rgbel_P39_1_rank_seed2.yaml`(MUSES, EPOCHS=300), 로그: `logs/jarvis_muses_P39_1_seed2_run.log`, 기동 시각 12:38 KST(약 2시간 경과).
- 현재 ~ep31/300, **val mIoU 78.73@ep30(신기록, 상승 추세)** — 궤적: 75.62→76.33→74.86→77.72→76.10→75.92→77.04→**78.59**→78.26→**78.73**. 등락은 있으나 전반적 상승.
- GPU2-5 util 91-100%, ~19GB(정상 범위) — 생존 확인.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, ⚠️6.6-7.2GB — mem 재급감, 7번째 관측) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~215/300 | val mIoU 80.85@ep124 (**불변, 초장기 정체**) | 잔여~85ep×7.3min≈10.3h → **~07-25 00:50 KST**(완주 임박 아님, 10h+) | alive |
| jarvis | 6,7 (100%/100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~213/300 | val mIoU **81.46@ep210(신기록)** | 정체(81.36@150) 돌파 — 잔여~87ep×7min≈10.15h → **~07-25 00:45 KST**(임박 아님) | alive |
| jarvis | 2,3,4,5 (91-100%, ~19GB) | 🆕 `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~31/300 | val mIoU 78.73@ep30(신기록) | 초반이라 저신뢰, 잔여~269ep×3.7min≈16.6h → **~07-25 07:10 KST경** | alive |
| yeon | 2 (10%, 16569MiB, 타유저 2건) | (없음) | — | — | — | — | 타유저 점유 지속 |
| yeon | 3,4 (100%/0%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~51/300 | val mIoU 78.65@ep42 (**불변**) — 79 미돌파 | ep44-50(77.91/77.79/78.27/77.18) 등락, 최신값(77.18)은 최근 저점 | 잔여~249ep×약27min≈112h≈4.7일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5,6,7 | 🔴 **(없음 — P39.1-DELIVER 사망, 위 참조)** | — | — | 최종 65.79@ep36(사망으로 고정) | **사망** | 🔴 **유휴(3장, 사망으로 해방)** |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 yeon GPU5,6,7 (3장) — P39.1-DELIVER 사망으로 신규 해방**. jarvis는 seed2 기동으로 GPU2-5 재점유(유휴 아님). hpca100 0,1=타유저(85-94%,35GB); jarvis 0,1=타유저(mem초과); yeon 0=33%util(mem 1985MiB, util 기준 초과로 비유휴 판정),1=타유저(100%,mem초과),2=타유저 2건.

**완주 임박 런**: 없음 — hpca100/jarvis P42 계열 모두 ~10h대로 아직 여유, seed2는 초반.

**이상징후**:
- 🔴 **yeon P39.1-DELIVER CUDA OOM 사망**(13:43:01 KST, 위 상세) — 최근 수차례 점검에서 관측된 "GPU 메모리 급감" 패턴이 이 run에서도 반복 관측됐던 바 있어, 사후적으로 볼 때 메모리 압박의 조짐이었을 가능성 있음(판정 아님, 사실 나열만).
- ⚠️ GPU 메모리 급감 패턴 hpca100서 재차 관측(7번째) — 여전히 원인 미규명.
- 그 외 신규 이상 없음(D-state 없음, 나머지 4개 run 정상 전진).

### 2026-07-24 16:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 yeon 6,7 = P39.1-seed3 — 기동 확인(단, "방금"보다 이전부터 진행 중)**
- 실제 GPU 점유 프로세스: PID 4157724(GPU6, 6524MiB)/4157725(GPU7, 6264MiB), **시작 14:39**(이번 점검 시점 기준 약 1h55m 경과) — 지시서의 "방금 기동" 표현과 달리 이미 한동안 진행돼 있었음.
- ⚠️ **중복 프로세스처럼 보이는 것 확인·해소**: `ps`에 동일 cfg로 16:31/16:32 시작된 신규 PID 그룹(211222~211372, 8개)이 추가로 잡혔으나, **`nvidia-smi --query-compute-apps`로 대조한 결과 이 그룹은 GPU를 전혀 점유하지 않음**(GPU6,7 실사용은 4157724/25뿐) → 실제 중복 학습이 아니라 **DataLoader 워커 재기동으로 판단**(다른 run들에서 본 신규 PID 배치와 동일 패턴). 판정 아님, 확인만.
- 로그: `/SSDe/jemo_maeng/yeon_muses_P39_1_seed3_run.log`, cfg `EPOCHS=300`. 현재 val: **ep2=44.80 → ep4=64.64(best)**, 현재 ~ep5 진행 중(초반, 페이스 표본 부족·저신뢰).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (88%/100%, 30.6GB 정상) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~233/300 | val mIoU 80.85@ep124 (**불변, 100+ep 정체**) | 잔여~67ep×7.3min≈8.15h → **~07-25 00:45 KST**(임박 아님) | alive |
| jarvis | 6,7 (99%/70%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~231/300 | val mIoU **81.53@ep218(신기록)** | 잔여~69ep×7min≈8.05h → **~07-25 00:40 KST**(임박 아님) | alive |
| jarvis | 2-5 (74-99%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~65/300 | val mIoU **80.73@ep60(신기록)** — 78.73→80.73 큰 폭 상승 | 잔여~235ep×3.7min≈14.5h → **~07-25 07:05 KST경**(완주) | alive |
| yeon | 0,1 (0%, 249/18MiB — 신규 유휴) | (없음) | — | — | — | — | 🔴 **신규 유휴(타유저 작업 종료 추정)** |
| yeon | 2 (0%, 14824MiB, 타유저 1건으로 감소) | (없음) | — | — | — | — | 타유저 점유(단 이전 2건→1건으로 감소) |
| yeon | 3,4 (88%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~59/300 | val mIoU 78.65@ep42 (**불변**) — 79 미돌파 | ep48-58(78.27/77.18/76.89/78.18/78.07/77.53) 등락, 신기록 없음 | 잔여~241ep×약27min≈108.5h≈4.5일 → **~07-29 경**(저신뢰) | alive |
| yeon | 5 (0%, 16MiB — 유휴 지속) | (없음 — P39.1-DELIVER 사망 잔재) | — | — | — | — | 유휴(지난 점검부터 계속) |
| yeon | 6,7 (0%util 순간관측·실사용 6.5/6.3GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~5/300(초반) | val mIoU 64.64@ep4 | 저신뢰(표본2개) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 yeon GPU0,1,5 (3장)** — GPU0,1은 이번 점검에서 신규 확인(직전까지 타유저 점유 중이었음, 작업 종료 추정), GPU5는 지난 점검(P39.1-DELIVER 사망)부터 계속 유휴. hpca100·jarvis는 유휴 없음(jarvis 2-5는 seed2가 점유 중).

**완주 임박 런**: 없음 — hpca100/jarvis P42 계열 모두 ~8h대(아직 임박 아님), seed2/seed3는 초중반.

**이상징후**:
- ⚠️ yeon P39.1-seed3에서 동일 cfg의 신규 PID 그룹 발견 — GPU 미점유로 확인돼 **단순 DataLoader 워커 재기동(오탐 배제)**, 실 중복학습 아님.
- yeon GPU0,1이 신규 유휴로 전환 — 타유저(추정 jongwon_kim 관련) 작업이 최근 종료된 것으로 보임(원인 미확인, 사실만).
- GPU 메모리 급감 패턴은 이번엔 특별히 두드러지지 않음(hpca100/jarvis 모두 정상 범위 유지).
- Traceback/OOM/nan 없음(신규), D-state 없음, 나머지 4개 run 정상 전진.

### 2026-07-24 18:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 yeon 0,1,5 = P39.1-seed4 — 기동 확인, 생존·초반 상승**
- cfg `configs/yeon-muses_rgbel_P39_1_seed4.yaml`(MUSES, EPOCHS=300), 3-GPU DDP(CUDA_VISIBLE_DEVICES=0,1,5), 로그 `/SSDe/jemo_maeng/yeon_muses_P39_1_seed4_run.log`, 기동 16:41(약 1h53m 경과).
- 현재 ~ep7/300, val: ep2=44.65→ep4=62.22→ep6=70.85(신기록) — seed3와 유사한 초반 급상승 패턴.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (68%/67%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~249/300 | val mIoU 80.85@ep124 (**불변, 124ep+ 정체**) | 잔여~51ep×7.3min≈6.2h → **~07-25 00:45 KST**(아직 임박 아님, ~6h) | alive |
| jarvis | 6,7 (73-100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~247/300 | val mIoU 81.53@ep218 (**불변**) | 잔여~53ep×7min≈6.2h → **~07-25 00:44 KST**(아직 임박 아님) | alive |
| jarvis | 2-5 (79-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~97/300 | val mIoU **81.43@ep94(신기록)** — 80.73→81.43 지속 상승 | 잔여~203ep×3.7min≈12.5h → **~07-25 07:05 KST경**(완주) | alive |
| yeon | 0,1,5 (100%/0%/100%, ~7-7.9GB) | 🆕 `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~7/300(초반) | val mIoU 70.85@ep6(신기록) | 저신뢰(표본3개) | alive |
| yeon | 2 (100%, 3110MiB, 타유저 1건으로 유지) | (없음) | — | — | — | — | 타유저 점유(1건, 지난 점검과 동일 감소 유지) |
| yeon | 3,4 (56%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~65/300 | val mIoU 78.65@ep42 (**불변**) — 79 미돌파(근접 78.59@60) | 잔여~235ep×약27min≈105.8h≈4.4일 → **~07-29 경**(저신뢰) | alive |
| yeon | 6,7 (0%util·mem 17.5/3.3GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~11/300(초반) | val mIoU **74.50@ep10(신기록)** — 44.80→64.64→68.53→74.11→74.50 지속 상승 | 저신뢰(초반) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음**. 직전 점검의 yeon 유휴 3장(GPU0,1,5)은 이번에 seed4가 전부 점유. jarvis 0,1=타유저(mem초과); hpca100 0,1=타유저(99-100%,35GB); yeon 2=타유저(1건 유지).

**완주 임박 런**: 없음 — hpca100/jarvis P42 계열 둘 다 ~6.2h로 좁혀졌으나 이번 2h 윈도우 내 완주 아님. 나머지 4개 run(seed2/3/4, P42-f07)은 전부 훨씬 여유 있음.

**MUSES 6-run 현황 요약**: P42 main(80.85@124, 정체)·P42-f03(81.53@218, 정체)·P39.1-seed2(81.43@94, 상승중)·P39.1-seed3(74.50@10, 초반 급상승)·P39.1-seed4(70.85@6, 초반 급상승)·P42-f07(78.65@42, 79 문턱서 등락).

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). 6개 run 모두 iteration 정상 전진·util>0 확인. GPU 메모리 급감 패턴 이번엔 두드러지지 않음.

### 2026-07-24 20:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**완주 임박 확인 — P42-f03(jarvis)/P42 main(hpca100): 둘 다 미완주, 격차 좁혀짐**
- 완료 마커("Training complete"/"Total Training Time") 양쪽 로그 모두 없음 — **둘 다 진행 중**.
- hpca100 P42 main: ~ep265/300, 잔여 35ep×7.3min≈4.26h → **~07-25 00:48 KST**(직전 추정과 거의 일치, 아직 미완주).
- jarvis P42-f03: ~ep265/300, 잔여 35ep×7min≈4.08h → **~07-25 00:38 KST**(마찬가지, 미완주).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~265/300 | val mIoU 80.85@ep124 (**불변, 140ep+ 정체**) | 잔여~35ep×7.3min≈4.26h → **~07-25 00:48 KST**(미완주) | alive |
| jarvis | 6,7 (93-100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~265/300 | val mIoU 81.53@ep218 (**불변, 46ep+ 정체**) | 잔여~35ep×7min≈4.08h → **~07-25 00:38 KST**(미완주) | alive |
| jarvis | 2-5 (93-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~131/300 | val mIoU **81.52@ep122(신기록)** — 81.43→81.52 지속 상승 | 잔여~169ep×3.7min≈10.4h → **~07-25 07:00 KST경**(완주) | alive |
| yeon | 2 (34%, 1940MiB, 타유저 1건 — mem은 유휴 기준 이내나 util 초과) | (없음) | — | — | — | — | 비유휴(util 34%>10%) |
| yeon | 0,1,5 (100%/100%/56%, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~19/300(초반) | val mIoU **76.84@ep18(신기록)** — 73.92→75.12→74.02→76.43→75.89→76.84 | 저신뢰(초반) | alive |
| yeon | 3,4 (100%/100%, ~6.2GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~71/300 | val mIoU **78.68@ep70(신기록, 근소) — 79 대비 -0.32pt** | 잔여~229ep×약27min≈103h≈4.3일 → **~07-29 경**(저신뢰) | alive |
| yeon | 6,7 (100%/66%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~19/300(초반) | val mIoU **76.97@ep16(신기록)** — 74.11→74.50→73.37→75.83→76.97→76.33 | 저신뢰(초반) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음**. yeon GPU2는 메모리는 기준 이내(1940MiB)이나 util 34%로 기준 초과 — 비유휴 처리. hpca100 0,1=타유저(83-88%,35GB); jarvis 0,1=타유저(mem초과).

**완주 임박 런**: hpca100 P42 main(~4.26h)·jarvis P42-f03(~4.08h) 둘 다 근접했으나 이번 2h 윈도우 내 미완주. 나머지 4개 run은 전부 그보다 여유.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). 6개 run 모두 iteration 정상 전진·util>0 확인. yeon GPU2 타유저 프로세스 mem이 3084MiB→1914MiB로 감소(추이만 기록, 판정 아님).

### 2026-07-24 22:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**완주 확인 — P42-f03(jarvis)/P42 main(hpca100): 둘 다 아직 미완주, 매우 임박**
- 완료 마커 양쪽 로그 모두 없음 — **아직 진행 중**.
- hpca100 P42 main: ~ep281/300, 잔여 19ep×7.3min≈2.3h → **~07-25 00:52 KST**(매우 임박).
- jarvis P42-f03: ~ep283/300, 잔여 17ep×7min≈2.0h → **~07-25 00:34 KST**(매우 임박).
- 둘 다 best는 각각 80.85@ep124 / 81.53@ep218에서 **여전히 불변**(각 156ep+/64ep+ 정체 지속) — 최종 결과는 아직 확정 아님, 다음 점검서 완주 여부 재확인 필요.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/100%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~281/300 | val mIoU 80.85@ep124 (**불변**) | 잔여~19ep×7.3min≈2.3h → **~07-25 00:52 KST**(미완주, 매우 임박) | alive |
| jarvis | 6,7 (94-100%, ~17.8GB) | `jarvis-muses_rgbel_P42_maskimg_f03.yaml` (P42-f03, FRAC0.3) | MUSES | ~283/300 | val mIoU 81.53@ep218 (**불변**) | 잔여~17ep×7min≈2.0h → **~07-25 00:34 KST**(미완주, 매우 임박) | alive |
| jarvis | 2-5 (91-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~163/300 | val mIoU **81.90@ep160(신기록)** — 81.52→81.62→81.90 지속 상승 | 잔여~137ep×3.7min≈8.4h → **~07-25 07:00 KST경**(완주) | alive |
| yeon | 2 (39%, 1940MiB, 타유저 1건) | (없음) | — | — | — | — | 비유휴(util 39%>10%) |
| yeon | 0,1,5 (89%/100%/54%, ~18.4-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~29/300(초반) | val mIoU **78.65@ep26(신기록)** — 76.84→77.52→76.81→78.42→78.65→77.32 | 저신뢰(초반) | alive |
| yeon | 3,4 (100%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~79/300 | val mIoU **78.71@ep74(신기록)** — 79 대비 -0.29pt, 최신값(77.38@78)은 하락 | 잔여~221ep×약27min≈99.5h≈4.1일 → **~07-29 경**(저신뢰) | alive |
| yeon | 6,7 (100%/99%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~27/300(초반) | val mIoU **78.21@ep26(신기록, 큰 폭)** — 76.97→76.33→76.48→76.83→76.71→78.21 | 저신뢰(초반) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음**. yeon GPU2는 mem 기준 이내(1940MiB)나 util 39%로 초과, 비유휴. hpca100 0,1=타유저(85-96%,35GB); jarvis 0,1=타유저(mem초과).

**완주 임박 런**: hpca100 P42 main(~2.3h)·jarvis P42-f03(~2.0h) 둘 다 이번 다음 점검 전후로 완주 예상 — **다음 점검에서 완주 여부·최종 best 확인 필수**. 나머지 4개 run(seed2/3/4, P42-f07)은 전부 여유.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). 6개 run 모두 iteration 정상 전진·util>0 확인.

### 2026-07-25 00:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis P42-f03(GPU6,7) — 완주 확인, COMPLETED**
- 로그 종료 확인: `Total Training Time 10:27:43`, epoch300 최종 val mIoU=80.95, **Best Val mIoU 81.53 (epoch 218)** — 최종 결과 확정(오랜 플래토 그대로 종료).
- ckpt: `epoch218_81.53_top1_checkpoint.pth`(14:16 저장분, 15:11 KST 표기 기준) — 경로: `/home/jemo_maeng/src/drone-MemorySAM-p42scratch/outputs/ReliaDINO/jarvis_muses_rgbel_P42_f03/`. `last_checkpoint.pth`(ep300 최종본, 00:32 저장)도 동일 경로.
- **프로세스 종료 확인**: `ps`에 P42_f03 관련 프로세스 전무(jarvis에 seed2 cfg만 남음). **GPU6,7 완전 해방 — 유휴 확인**(0%util, 536/538MiB).

**🔴 hpca100 P42 main(GPU2,3) — 아직 미완주(매우 임박, ep299/300)**
- 완료 마커 없음, `ps` 프로세스 19개 라인 생존 확인(alive), 원문 로그 tail: `Epoch [299/300] ... 23%|██▎ | 88/375` — **ep299 진행 중, ep300 목전**(추정 잔여 ~10분 내외).
- 최신 val: ep298=80.27(15:32:48 UTC), best는 **80.85@ep124 그대로 불변**(174ep+ 정체 지속) — 완주 시 최종 best도 124 그대로일 가능성 높으나 확정 아님, 다음 점검에서 재확인 필요.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (91%/100%, 30.6GB) | `hpca100-muses_rgbel_P42_maskimg.yaml` (P42 main, FRAC0.5) | MUSES | ~299/300 | val mIoU 80.85@ep124(불변) | **미완주, 초임박(~10분 내외)** | alive |
| jarvis | 6,7 | **(없음 — P42-f03 완주, 위 참조)** | — | — | 최종 81.53@ep218 | **완주됨** | 🔴 **유휴 확인(2장)** |
| jarvis | 2-5 (99-100%, ⚠️7.1-8.4GB — mem 급감 재발) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~195/300 | val mIoU 81.90@ep160(불변, 34ep+ 정체) | 잔여~105ep×3.7min≈6.5h → **~07:00 KST경** | alive |
| yeon | 2 (100%, 14244MiB, 타유저 1건) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 0,1,5 (98-100%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~41/300 | val mIoU **🔴79.53@ep34(신기록) — 79 최초 돌파** | 저신뢰 | alive |
| yeon | 3,4 (100%×2, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~87/300 | val mIoU 78.71@ep74(불변) — 79 미돌파 | 잔여~213ep×약27min≈95.9h≈4.0일 → **~07-29 경** | alive |
| yeon | 6,7 (100%/98%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~35/300 | val mIoU 78.65@ep32(불변, 근소) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 jarvis GPU6,7 (2장) — P42-f03 완주로 신규 해방**. 그 외: hpca100 0,1=타유저(99-100%,35GB); jarvis 0,1=타유저(mem초과); yeon 0,1,2=전부 비유휴(2=타유저, 0·1·5는 seed4 자사).

**완주 임박 런**: **hpca100 P42 main이 ep299/300으로 사실상 완주 직전**(다음 점검서 확정 예정). jarvis P42-f03은 이번에 완주 확정. 나머지는 여유.

**이상징후**:
- ⚠️ jarvis GPU2-5(P39.1-seed2) 메모리 급감 재발(7.1-8.4GB, 이전 정상범위 ~19GB 대비) — 누적 관측되는 패턴 지속.
- Traceback/OOM/nan 없음, D-state 없음.

### 2026-07-25 02:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 P42 main(GPU2,3) — 완주 확인, COMPLETED**
- 로그 종료 확인: `Total Training Time 12:25:44`, epoch300 최종 val mIoU=80.30, **Best Val mIoU 80.85 (epoch 124)** — 최종 결과 확정(장기 플래토 그대로 종료, 178ep 동안 갱신 없었음).
- ckpt: `epoch124_80.85_top1_checkpoint.pth`(2026-07-23 18:25 저장) — 경로: `/home/jovyan/SSDb/jemo_maeng/src/drone-MemorySAM/outputs/ReliaDINO/hpca100_muses_rgbel_P42_maskimg/`. `last_checkpoint.pth`(ep300 최종본, 15:45 UTC 저장)도 동일 경로.
- **프로세스 종료 확인**: `ps` 프로세스 0개(완전 종료). **GPU2,3 완전 해방 — 유휴 확인**(0%util, 6MiB 각각).

**🔴 jarvis GPU6,7(前 P42-f03) — 유휴 아님, 타유저에게 뺏김**
- 지난 점검(00:34)까지 유휴였으나, 이번 점검에서 **신규 타유저(seungyeon_cheon) 점유 확인**: GPU6=PID 3059435(11528MiB), GPU7=PID 3059434(11496MiB) — 기존 zombie 잔재(506MiB급)와 별개의 새 프로세스.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 | **(없음 — P42 main 완주, 위 참조)** | — | — | 최종 80.85@ep124 | **완주됨** | 🔴 **유휴 확인(2장)** |
| jarvis | 6,7 (100%/97%, ~12GB, 타유저) | (없음, 우리 job 아님) | — | — | — | — | 🔴 타유저 점유(신규) |
| jarvis | 2-5 (74-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~229/300 | val mIoU **🔴82.62@ep208(신기록, 큰 폭)** — 81.90→82.62 | 잔여~71ep×3.7min≈4.4h → **~07:00 KST경**(완주) | alive |
| yeon | 2 (100%, 14244MiB, 타유저 1건) | (없음) | — | — | — | — | 타유저 점유 지속 |
| yeon | 0,1,5 (100%×3, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~51/300 | val mIoU 79.53@ep34(불변) — 79 돌파 유지 | 저신뢰 | alive |
| yeon | 3,4 (100%×2, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~95/300 | val mIoU **79.14@ep90(신기록) — 79 최초 돌파** | 잔여~205ep×약27min≈92.3h≈3.8일 → **~07-29 경** | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~41/300 | val mIoU **79.14@ep36(신기록) — 79 돌파** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 GPU2,3 (2장) — P42 main 완주로 신규 해방**. jarvis 6,7은 완주 후 유휴였다가 이번에 타유저에게 재점유됨(유휴 아님). 그 외: hpca100 0,1=타유저(99-100%,35GB); jarvis 0,1=타유저(mem초과); yeon 0,1,2=전부 비유휴.

**완주 임박 런**: 없음(hpca100/jarvis P42 계열 둘 다 이번 점검서 완주 확정 처리됨). jarvis P39.1-seed2가 ~4.4h로 가장 임박, 나머지는 여유.

**MUSES 79 돌파 현황**: 이번 점검에서 **yeon 3-run(P42-f07/seed3/seed4) 전부 79 돌파 확인**(각각 79.14/79.14/79.53) — 6-run 중 4개(seed2 포함 시 82.62)가 79 이상.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). GPU 메모리 급감 패턴은 이번 점검서 두드러지지 않음(jarvis seed2 정상 범위 복귀).

### 2026-07-25 04:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 2,3 상태 — 유휴 아님(분석 잡 점유), P42-seed2 학습은 크래시 확인**
- 지시서상 "cuDNN pod 이슈로 hold 실패"의 근거 로그 확인: `logs/hpca100_P42_seed2_hold.log`에 **`RuntimeError: GET was unable to find an engine to execute this computation`**(cuDNN 엔진 탐색 실패) → `ChildFailedError`, **크래시 시각 2026-07-24 17:43:26** — 이 학습 시도(P42-seed2)는 즉시 실패했고 재시도 흔적 없음.
- 현재 GPU2,3은 **학습이 아니라 별도 분석 스크립트가 점유 중**: `tools/analyze_router_coverage.py`(P38/P39 router coverage 평가, `--gpu 2`/`--gpu 3`, 19:32 시작·약 7분 경과, 각 3120/3062MiB) — util 91-100%지만 우리가 추적하는 학습 run은 아님. **유휴 아님**(분석 잡 점유), 그리고 **train_reliadino 프로세스는 전무**(`ps`에서 확인).

**🆕 jarvis 6,7 = P39.1-seed5 — 기동 확인, 생존·초반**
- cfg `configs/jarvis-muses_rgbel_P39_1_rank_seed5.yaml`(MUSES, EPOCHS=300), 로그 `logs/jarvis_muses_P39_1_seed5_run.log`, 기동 04:04(약 30분 경과).
- 현재 ~ep5/300, val: ep2=45.06→ep4=64.75(신기록) — seed3/4와 유사한 초반 급상승 패턴.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (91-100%, ~3.1GB, 분석잡) | (train 아님 — 위 참조) | — | — | — | — | 학습 없음(분석 스크립트만) |
| jarvis | 2-5 (99-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~263/300 | val mIoU 82.62@ep208 (**불변, 54ep+ 정체**) | 잔여~37ep×3.7min≈2.3h → **~06:50 KST경**(완주 임박) | alive |
| jarvis | 6,7 (80-100%, ~19GB) | 🆕 `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~5/300(초반) | val mIoU 64.75@ep4(신기록) | 저신뢰(초반) | alive |
| yeon | 2 (0%, 14244MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유(mem초과, 비유휴) |
| yeon | 0,1,5 (100%/87%/51%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~61/300 | val mIoU **79.97@ep54(신기록)** — 79.53→79.69→79.97 지속 상승 | 저신뢰 | alive |
| yeon | 3,4 (100%/0%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~101/300 | val mIoU 79.14@ep90 (**불변**) | 잔여~199ep×약27min≈89.6h≈3.7일 → **~07-29 경** | alive |
| yeon | 6,7 (100%×2, ⚠️6.3-6.5GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~47/300 | val mIoU 79.14@ep36 (**불변**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 3서버 전체 유휴 없음**. hpca100 0,1=타유저(89-99%,35GB), 2,3=분석잡 점유(비유휴); jarvis 0,1=타유저(mem초과); yeon 0,1,2=전부 비유휴.

**완주 임박 런**: jarvis P39.1-seed2(~2.3h, ~06:50 KST경)가 가장 임박. 나머지는 여유.

**이상징후**:
- 🔴 hpca100 P42-seed2 재기동 시도가 **cuDNN 엔진 탐색 실패로 즉시 크래시**(17:43:26, 07-24) — 재시도 없이 그대로 방치된 것으로 보임(사실만 보고, 판정 아님).
- ⚠️ yeon GPU6,7(P39.1-seed3) 메모리 급감 재발(~6.3-6.5GB, 정상 ~17GB 대비) — 누적 관측 패턴 지속.
- 그 외 Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-25 06:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 P39.1-seed2(jarvis 2-5) — 아직 미완주, 초임박(ep296/300)**
- 완료 마커("Training complete"/"Total Training Time") 없음 — **아직 진행 중**. 원문 로그: `Epoch [296/300] ... 12%|█▏ | 45/375` — ep296 진행 중, ep300까지 잔여 약 4epoch.
- 최신 val: ep294=81.45(06:30:22), best는 **82.62@ep208 그대로 불변**(86ep+ 정체 지속) — 완주 시 최종 best도 이 값 유지 가능성 높으나 확정 아님.
- 페이스(~3.7min/ep) 기준 잔여 ≈14분 → **ETA ~06:48-06:50 KST**(매우 임박, 다음 점검서 완주 확정 예상).

**hpca100 2,3 — 분석 잡도 종료, 완전 유휴로 전환**
- `ps`에 관련 프로세스 0개(train_reliadino도, 이전 확인된 analyze_router_coverage.py도 전무). **GPU2,3 = 0%util, 6MiB — 완전 유휴 확인.**

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 | (없음 — 완전 유휴) | — | — | — | — | 🔴 유휴(2장) |
| jarvis | 2-5 (69-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed2.yaml` (P39.1-seed2) | MUSES | ~296/300 | val mIoU 82.62@ep208 (**불변**) | **미완주, ~06:48-06:50 KST(초임박)** | alive |
| jarvis | 6,7 (80-100%, ~19GB) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~21/300 | val mIoU **76.83@ep18(신기록)** — 70.79→74.41→74.70→73.52→75.32→76.82→76.83 | 저신뢰 | alive |
| yeon | 2 (100%, 3110MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 0,1,5 (100%×3, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~71/300 | val mIoU **🔴80.34@ep70(신기록) — yeon seed 계열 최초 80 돌파** | 저신뢰 | alive |
| yeon | 3,4 (99%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~109/300 | val mIoU **79.16@ep106(신기록, 근소)** | 잔여~191ep×약27min≈86h≈3.6일 → **~07-29 경** | alive |
| yeon | 6,7 (68%/81%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~55/300 | val mIoU **79.63@ep50(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 GPU2,3 (2장) — 완전 유휴**(분석잡도 종료). 그 외: hpca100 0,1=타유저(82%,35GB); jarvis 0,1=타유저(mem초과); yeon 0,1,2=전부 비유휴.

**완주 임박 런**: **jarvis P39.1-seed2가 ~10-15분 내 완주 예상**(다음 점검서 확정 필요). 나머지는 여유.

**이상징후**: 없음(Traceback/OOM/nan 전무, D-state 없음). 6개 run 모두 iteration 정상 전진·util>0.

### 2026-07-25 08:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis 드라이버 상태 — NVML mismatch 지속(미복구)**
- `nvidia-smi` 실행 결과: `Failed to initialize NVML: Driver/library version mismatch` `NVML library version: 580.173` — **여전히 mismatch, 복구 안 됨**(리부트 안 된 것으로 추정). **jarvis의 GPU util/mem 조회는 이번 점검에서 불가** — 이하 jarvis 항목은 전부 `ps`/로그 파일시스템만으로 판정.

**🔴 jarvis P39.1-seed2(GPU2-5) — 완주 확인, COMPLETED(ps/로그 기준)**
- `ps`에 seed2 관련 프로세스 전무(seed5만 생존 확인) — 종료됨.
- 로그 종료 마커 확인: `Total Training Time 18:13:47`, epoch300 최종 val mIoU=81.69, **Best Val mIoU 82.62 (epoch 208)** — 최종 결과 확정(완주 시각 06:52:17 KST, 직전 추정 ETA 06:48-06:50와 거의 일치).
- ckpt: `epoch208_82.62_top1_checkpoint.pth` + `last_checkpoint.pth`(ep300 최종본, 06:51 저장) — 경로: `/home/jemo_maeng/src/drone-MemorySAM-develop/outputs/ReliaDINO/jarvis_muses_rgbel_P39_1_rank_seed2/`.
- **GPU2-5 해방 여부**: nvidia-smi 불가로 **직접 확인 불가** — 단 프로세스 종료가 확인됐으므로 정황상 해방됐을 가능성 높음(확정 아님, 판정 아님).

**jarvis P39.1-seed5(GPU6,7) — 생존 확인(ps 기준), 79 돌파**
- `ps`로 프로세스 생존 확인(PID 3181330 등, 04:04 시작). 로그: ep24=76.75→ep26=78.05→ep28=78.45→ep30=78.45→ep32=78.73→ep34=77.88→**ep36=79.23(신기록, 79 돌파)**→ep38=78.72. 현재 ~ep39/300.

**hpca100 2,3 — 완전 유휴 유지 확인**
- `ps` 프로세스 0개, GPU2,3 = 0%util, 6MiB — 지난 점검과 동일 완전 유휴 상태 지속.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 | (없음 — 완전 유휴) | — | — | — | — | 유휴 지속 |
| jarvis | 2-5 | **(없음 — P39.1-seed2 완주, 위 참조)** | — | — | 최종 82.62@ep208 | **완주됨** | 프로세스 종료 확인(GPU 상태는 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps로만 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~39/300 | val mIoU **79.23@ep36(신기록, 79 돌파)** | 저신뢰 | alive(ps 확인) |
| yeon | 2 (100%, 3110MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 0,1,5 (100%/100%/95%, ~18.4-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~83/300 | val mIoU **80.53@ep82(신기록)** — 80.34→80.46→80.53 지속 상승 | 저신뢰 | alive |
| yeon | 3,4 (100%×2, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~117/300 | val mIoU 79.16@ep106 (**불변**) | 잔여~183ep×약27min≈82.4h≈3.4일 → **~07-29 경** | alive |
| yeon | 6,7 (3%/100%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~63/300 | val mIoU **80.15@ep60(신기록) — seed3 최초 80 돌파** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 GPU2,3 (2장) — 완전 유휴 유지**. jarvis는 nvidia-smi 불가로 GPU2-5(前 seed2) 유휴 여부 **미확정**(프로세스는 종료됨). yeon 0,1,2=전부 비유휴.

**완주 임박 런**: 없음 — jarvis P39.1-seed2가 이번에 완주 확정 처리됨. 나머지 5개 run은 전부 여유.

**이상징후**:
- 🔴 jarvis NVML Driver/library version mismatch **지속**(580.173) — GPU 레벨 모니터링 불가 상태 유지. 재부팅 등 복구 조치는 판정/실행 권한 밖이라 수행하지 않음, 사실만 보고.
- Traceback/OOM/nan 없음(신규), D-state 없음. 5개 alive run 모두 iteration 정상 전진(ps/로그 기준).

### 2026-07-25 10:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis 드라이버 상태 — NVML mismatch 여전히 지속(미복구 확인)**
- `nvidia-smi`: 동일 오류 `Failed to initialize NVML: Driver/library version mismatch` `580.173` — **복구 안 됨**.
- `uptime -s` = `2026-07-10 15:08:01` — **부팅 이후 리부트 없음 확인**(15일째), 드라이버 재로드가 안 됐다는 정황과 일치. GPU 레벨 조회 이번에도 전면 불가, 이하 jarvis는 ps/로그 기준.
- **seed5(GPU6,7) 생존·전진 확인(ps)**: PID 3181413/3181414(rank0/1) 등 다수 프로세스 생존(04:04 시작, 누적 CPU 398-401분). 로그: ep40=78.77→ep42=79.20→ep44=79.43(신기록)→ep46=78.36→ep48=79.85(신기록)→**ep50=79.92(신기록)**→ep52=79.51→ep54=79.47. 현재 ~ep55/300. Best=79.92@ep50.
- **GPU2-5(前 seed2)**: `ps`상 신규 프로세스 없음(seed5 외 전무) — 우리 쪽에서는 미점유 유지로 보이나 **nvidia-smi 불가로 타 세션 점유 여부는 확인 불가**.

**hpca100 — GPU0,1도 신규 유휴 전환(전체 4장 유휴)**
- 전 GPU(0,1,2,3) 모두 **0%util/0MiB** — 그동안 GPU0,1을 오래 점유해온 타유저(gr00t_finetune) job이 종료된 것으로 보임(원인 미확인, 사실만). GPU2,3은 기존과 동일 유휴 지속.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1,2,3 | (없음 — 전체 유휴) | — | — | — | — | 유휴(4장) |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU상태 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~55/300 | val mIoU **79.92@ep50(신기록)** | 저신뢰 | alive(ps) |
| yeon | 2 (0%, 14228MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 0,1,5 (0%×3, ⚠️5.9-6.8GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~93/300 | val mIoU **🔴81.16@ep90(신기록) — seed 계열 최고** | 저신뢰 | alive |
| yeon | 3,4 (0%/100%, ~17GB) | `yeon-muses_rgbel_P42_maskimg_f07.yaml` (P42-f07, FRAC0.7) | MUSES | ~123/300 | val mIoU 79.16@ep106 (**불변, 16ep+ 정체**) | 잔여~177ep×약27min≈79.7h≈3.3일 → **~07-29 경** | alive |
| yeon | 6,7 (16%/100%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~71/300 | val mIoU **80.37@ep70(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 GPU0,1,2,3 (4장 전체) — 신규 유휴**(0,1은 타유저 job 종료로 이번에 처음 확인). jarvis 2-5는 nvidia-smi 불가로 미확정(단 프로세스는 없음). yeon 0,1,2,5는 메모리 기준 이내나 요약: yeon GPU2(타유저,mem초과)만 확실히 비유휴, GPU0,1,5는 우리 job(seed4)이 실행 중이므로 mem-drop 현상과 무관하게 비유휴로 처리(실제 학습 진행 확인됨).

**완주 임박 런**: 없음 — 전 run 여유(가장 빠른 것도 수 시간~수 일 단위).

**이상징후**:
- 🔴 jarvis NVML mismatch 3회 연속 확인, 미복구 지속 — 리부트 필요 여부는 판단/실행 권한 밖, 사실만 보고.
- ⚠️ yeon GPU0,1,5(seed4) 메모리 급감 패턴 재발(~5.9-6.8GB, 정상 ~18.5GB 대비) — 누적 관측 패턴 지속.
- hpca100 GPU0,1이 오랜 기간 만에 처음으로 유휴 전환 — 타유저 job 종료 추정(사실만, 판정 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-25 12:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 hpca100 복구 확인 — P42-seed2-hold(GPU2,3) 생존·초반 급상승**
- cuDNN fix 반영 후 재기동 확인: cfg `configs/hpca100-muses_P42_seed2_hold.yaml`(MUSES, EPOCHS=300), 로그 `logs/hpca_cudnnfix_test.log`, 기동 02:35(약 1h 경과).
- 현재 ~ep9/300, val: ep2=56.00→ep4=62.58→ep6=67.77→**ep8=76.11(신기록)** — 정상 학습 곡선(cuDNN 에러 재발 없음).
- **hpca100 GPU0,1은 여전히 유휴**(0%, 0MiB) — 예전 타유저(gr00t) job 자리, 신규 점유자 없음.

**jarvis(nvidia-smi 불가, ps/로그 기준) — seed5(GPU6,7) 생존, 80 돌파**
- `ps` 프로세스 생존 확인(seed5만 존재, 다른 cfg 없음). 로그: ep58=79.52→ep60=79.93(신기록)→ep62=79.04→ep64=80.44(신기록)→ep66=79.76→ep68=79.72→**ep70=80.52(신기록, 80 돌파)**→ep72=80.07. 현재 ~ep73/300.
- **GPU2-5**: `ps`상 신규 프로세스 없음 — 미점유로 보이나 nvidia-smi 불가로 확정 불가.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 | (없음 — 유휴) | — | — | — | — | 유휴 |
| hpca100 | 2,3 (96%/100%, ~32GB) | `hpca100-muses_P42_seed2_hold.yaml` (검증용 P42) | MUSES | ~9/300(초반) | val mIoU 76.11@ep8(신기록) | 저신뢰(초반) | alive |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~73/300 | val mIoU **80.52@ep70(신기록, 80 돌파)** | 저신뢰 | alive(ps) |
| yeon | 2 (100%, 14230MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (27%/100%, 1.9-21.6GB, 타유저) | 🔴 **(user 예약 — P42-f07 종료, 학습 미배정)** | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ⚠️7.2-8.0GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~101/300 | val mIoU 81.16@ep90 (**불변, 10ep+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~77/300 | val mIoU **80.39@ep76(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 GPU0,1 (2장) — 유휴, 채우기 가능**. jarvis GPU2-5는 nvidia-smi 불가로 미확정(프로세스는 없음). **yeon GPU3,4는 user 예약이므로 유휴 채우기 대상에서 제외**(별도 태그). yeon 2=타유저(비유휴).

**이상징후**:
- ⚠️ jarvis NVML mismatch 지속(4회 연속 확인) — 리부트 미실시.
- ⚠️ yeon GPU0,1,5(seed4) 메모리 급감 패턴 재관측(~7.2-8.0GB, 정상 대비).
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 cuDNN 이슈는 복구 확인(재발 없음).

### 2026-07-25 14:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 GPU0,1 — 유휴 아님, 재점유 확인**
- 직전 점검까지 유휴(0%,0MiB)였으나 이번엔 **100%/95%, 35951/35949MiB로 재점유 확인** — 이전 타유저(gr00t 추정, 동일 메모리 풋프린트) job이 재개된 것으로 보임(원인 미확인, 사실만).

**hpca100 2,3 = P42-hold — 계속 상승**
- val: ep14=75.82→ep16=76.52→ep18=77.15→ep20=75.94→ep22=77.76→**ep24=77.89(신기록)**. 현재 ~ep25/300.

**jarvis(nvidia-smi 불가 5회 연속, ps/로그 기준) — seed5(GPU6,7) 생존, 신기록**
- `ps` 프로세스 생존 확인(seed5만). 로그: ep76=80.25→ep78=80.00→ep80=80.62(신기록)→ep82=80.33→ep84=79.63→ep86=80.19→**ep88=80.89(신기록)**→ep90=80.74. 현재 ~ep91/300.
- GPU2-5: 여전히 프로세스 없음(미점유로 추정, GPU레벨 미확인).

**yeon — seed3/seed4 둘 다 신기록**

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (100%/95%, ~36GB, 타유저 재점유) | (없음) | — | — | — | — | 🔴 재점유(비유휴) |
| hpca100 | 2,3 (73%/62%, ~32GB) | `hpca100-muses_P42_seed2_hold.yaml` (검증용 P42) | MUSES | ~25/300 | val mIoU **77.89@ep24(신기록)** | 저신뢰 | alive |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~91/300 | val mIoU **80.89@ep88(신기록)** | 저신뢰 | alive(ps) |
| yeon | 2 (0%, 14230MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (27%/0%, 1.9-21.7GB, 타유저) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/0%/100%, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~111/300 | val mIoU **🔴81.43@ep110(신기록) — 6-seed 계열 최고치** | 저신뢰 | alive |
| yeon | 6,7 (100%/0%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~83/300 | val mIoU **80.72@ep82(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**(hpca100 0,1은 이번에 재점유돼 제외). jarvis GPU2-5는 nvidia-smi 불가로 미확정(프로세스는 없음). **yeon GPU3,4는 user 예약**(유휴 채우기 대상 아님, 별도 태그). yeon 2=타유저(비유휴).

**완주 임박 런**: 없음(전 run 저신뢰 초중반 또는 여유).

**이상징후**:
- 🔴 hpca100 GPU0,1이 유휴에서 재점유로 전환(타유저 job 재개 추정) — 유휴 자원 소멸.
- ⚠️ jarvis NVML mismatch 5회 연속 확인, 미복구 지속.
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 cuDNN 이슈는 계속 정상(재발 없음).

### 2026-07-25 16:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**jarvis 드라이버 상태 — NVML mismatch 지속(6회 연속 확인, 미복구)**
- `nvidia-smi`: 동일 오류(`580.173` mismatch). GPU 레벨 조회 이번에도 전면 불가, ps/로그 기준 유지.

**완주 임박 여부 — 4개 alive run 전부 "임박 아님"**
- yeon seed4(0,1,5): ~ep117/300(39% 진행), 페이스 ~15.5min/ep → 잔여 ~183ep≈47.3h(약 2일) — **임박 아님**.
- yeon seed3(6,7): ~ep87/300(29% 진행) — **임박 아님**(seed4보다도 여유).
- jarvis seed5(6,7, ps 기준): ~ep107/300(36% 진행), 페이스 ~7min/ep → 잔여 ~193ep≈22.4h — **임박 아님**.
- hpca100 P42-hold(2,3): ~ep41/300(14% 진행), 페이스 ~7.2min/ep → 잔여 ~259ep≈31.1h — **임박 아님**.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (99%×2, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 지속(비유휴) |
| hpca100 | 2,3 (100%×2, ~32GB) | `hpca100-muses_P42_seed2_hold.yaml` (검증용 P42) | MUSES | ~41/300 | val mIoU **79.26@ep38(신기록)** | 잔여~259ep×7.2min≈31.1h → **~07-27 00:40 KST경** | alive |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~107/300 | val mIoU **81.34@ep102(신기록)** | 잔여~193ep×6.96min≈22.4h → **~07-26 15:00 KST경** | alive(ps) |
| yeon | 2 (0%, 14230MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%×2, ~3.1-21.7GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ⚠️7.1-7.6GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~117/300 | val mIoU **81.63@ep114(신기록)** | 잔여~183ep×15.5min≈47.3h → **~07-27 경** | alive |
| yeon | 6,7 (100%×2, ⚠️6.5-6.6GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~87/300 | val mIoU 80.72@ep82 (**불변, 3ep+ 정체**) | 저신뢰(여유) | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**이상징후**:
- ⚠️ jarvis NVML mismatch 6회 연속 확인, 미복구 지속.
- ⚠️ yeon GPU0,1,5(seed4)·GPU6,7(seed3) 메모리 급감 패턴 동시 관측(각 ~7GB/~6.5GB, 정상 대비) — 누적 패턴 지속.
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 cuDNN 이슈는 계속 정상.

### 2026-07-25 18:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 2,3 — 여전히 P42(검증런), P43 전환 안 됨(관찰만, 손대지 않음)**
- 지시서상 "다른 세션이 P43-MUSES로 전환할 것"이었으나, **실측 결과 여전히 동일 P42 job**(`configs/hpca100-muses_P42_seed2_hold.yaml`, 02:35 시작 그대로 지속, 누적 CPU 433분+) — **아직 전환 안 됨**.
- P43 관련 cfg 파일들은 존재/최근 터치 확인됨(`hpca100-muses_rgbel_P43_pdual.yaml`, `jarvis-muses_rgbel_P43_pdual.yaml`, `tools/smoke_p43.py` 등, 최근 300분 이내) — **다른 세션이 준비만 해둔 상태로 보이며 아직 기동 안 한 것으로 판단**. 우리는 관찰만 하고 개입하지 않음.
- val 진행: ep46=79.41→ep48=79.20→ep50=79.11→ep52=79.54(신기록)→ep54=79.20→**ep56=79.69(신기록)**. 현재 ~ep57/300.
- hpca100 GPU0,1: 타유저(gr00t) job이 `--max-steps 400000 --resume` 파라미터로 05:07 재기동 확인 — 이전 종료(200000 step) 후 재개된 것으로 보임(사실만, 판정 아님).

**jarvis 드라이버 상태 — NVML mismatch 지속(7회 연속 확인, 미복구)**

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (99-100%, ~36GB, 타유저 재개) | (없음) | — | — | — | — | 타유저 점유(재기동 확인) |
| hpca100 | 2,3 (100%×2, ~32GB) | `hpca100-muses_P42_seed2_hold.yaml` (검증용 P42 — **P43 아님**) | MUSES | ~57/300 | val mIoU **79.69@ep56(신기록)** | 저신뢰 | alive |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~125/300 | val mIoU 81.34@ep102 (**불변, 11eval+ 정체**) | 저신뢰 | alive(ps) |
| yeon | 2 (0%, 14206MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (32%/0%, 1.9-21.7GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~127/300 | val mIoU 81.63@ep114 (**불변, 6eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%/86%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~93/300 | val mIoU **80.89@ep88(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(재기동, 비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**이상징후**:
- 🔴 hpca100 2,3 아직 P43 미전환(지시 기대와 다름, 사실 보고) — 관찰 지속 필요.
- ⚠️ jarvis NVML mismatch 7회 연속, 미복구.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-25 20:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 hpca100 2,3 = P43-MUSES 기동 확인 — 정상 진행**
- cfg `configs/hpca100-muses_rgbel_P43_pdual.yaml`(MUSES, **EPOCHS=200**, 이전 P39.1 계열의 300과 다름), 로그 `logs/hpca_P43_muses.log`.
- **첫 val 나옴**: ep2=62.31 → ep4=68.00(신기록) → **ep6=74.06(신기록)** → ep8=73.65(소폭 하락, best 유지).
- **Loss 유한·정상**: 현재 `Epoch[9/200] Loss 3.5525 cal 0.0000 auxCE 0.3850`(수렴 추세), P43 고유 스칼라 `[P43] mask_loss` 2.5281→2.4445로 하강 중. Traceback/Error/NaN/Inf 전무 확인.
- 현재 ~ep9/200, 페이스 ~8.7min/ep → 잔여~191ep≈27.7h → ETA 약 **07-27 00:15 KST경**(완주까지, 저신뢰).

**jarvis 드라이버 상태 — NVML mismatch 지속(8회 연속 확인, 미복구)**
- seed5(GPU6,7, ps 확인): val ep126=81.46(신기록)→ep128=80.98→ep130=81.04→ep132=81.38→ep134=81.17→ep136=80.82→ep138=80.72→ep140=80.84. 현재 ~ep141/300.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (98-99%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.7GB) | 🆕 `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~9/200 | val mIoU 74.06@ep6(신기록) | 잔여~191ep×8.7min≈27.7h → **~07-27 00:15 KST경**(저신뢰) | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~141/300 | val mIoU **81.46@ep126(신기록)** | 저신뢰 | alive(ps) |
| yeon | 2 (0%, 14206MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%/63%, 3.1-21.7GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (0%×3, ⚠️6.3-6.7GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~133/300 | val mIoU 81.63@ep114 (**불변, 9eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (84%/91%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~99/300 | val mIoU 80.89@ep88 (**불변, 5eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**이상징후**:
- ⚠️ jarvis NVML mismatch 8회 연속, 미복구 지속.
- ⚠️ yeon GPU0,1,5(seed4) 메모리 급감 패턴 재관측.
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 P43-MUSES는 정상 기동·진행 확인(별세션 잡, 미개입).

### 2026-07-25 22:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**hpca100 P43-MUSES(2,3) — 계속 상승, 정상**
- val: ep8=73.65→ep10=76.25(신기록)→ep12=76.06→ep14=77.45(신기록)→ep16=77.51(신기록)→ep18=78.58(신기록)→**ep20=78.76(신기록)**→ep22=77.63(소폭 하락). Best=78.76@ep20, 직전(74.06@6) 대비 뚜렷한 상승.
- `[P43] mask_loss` 2.0513→1.9691로 지속 하강, Traceback/NaN/Inf 없음 — 정상.
- ⚠️ 메모리가 이번엔 8.7-8.9GB로 급감(직전 ~33.7GB) — 다른 run들에서 반복 관측된 패턴이 이 신규 run에서도 재현.

**jarvis 드라이버 상태 — NVML mismatch 지속(9회 연속 확인, 미복구)**
- seed5(ps 확인): val ep144-158(8개) 전부 80.8-81.35 범위, **best 81.46@ep126에서 9eval+ 정체 지속**.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (100%×2, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ⚠️8.7-8.9GB — mem 급감) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~23/200 | val mIoU **78.76@ep20(신기록)** | 잔여~177ep×7.75min≈22.9h → **~07-26 21:26 KST경**(저신뢰) | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~159/300 | val mIoU 81.46@ep126 (**불변, 9eval+ 정체**) | 저신뢰 | alive(ps) |
| yeon | 2 (100%, 14206MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (37%/7%, 1.9-21.8GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ⚠️6.8-7.3GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~141/300 | val mIoU 81.63@ep114 (**불변, 13eval+ 정체 — 최장**) | 저신뢰 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~105/300 | val mIoU **80.93@ep102(신기록, 근소)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — 4개 alive run 모두 진행률 12-53% 구간(P43 11.5%, seed5 53%, seed4 47%, seed3 35%), 전부 여유.

**이상징후**:
- ⚠️ jarvis NVML mismatch 9회 연속, 미복구 지속.
- ⚠️ GPU 메모리 급감 패턴이 hpca100 P43(신규)과 yeon seed4에서 동시 관측 — 이제 거의 모든 장기 run에서 주기적으로 나타나는 공통 현상으로 굳어짐(원인 미규명).
- yeon seed4가 13eval+로 가장 긴 정체 구간 기록 중(⚠️, 판정 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 00:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**hpca100 P43-MUSES(2,3) — 계속 상승, 정상**
- val: ep24=79.45(신기록)→ep26=80.12(신기록)→ep28=80.51(신기록)→ep30=80.24→ep32=79.73→ep34=80.32→ep36=80.71(신기록)→**ep38=80.95(신기록)**. Best 74.06(ep6)→78.76(ep20)→80.95(ep38)로 지속 상승 중.

**jarvis 드라이버 상태 — NVML mismatch 지속(10회 연속 확인, 미복구)**
- seed5(ps 확인, **완주 안 됨**): 완료 마커 없음. val ep162-176: 81.13,80.91,81.15,81.28,**81.48(신기록, 9eval+ 정체 돌파)**,81.38,81.25,**81.57(신기록)**. Best=81.57@ep176. 현재 ~ep177/300(59% 진행) — 완주 임박 아님.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (76%/99%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (90%/70%, ~33.8GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~39/200 | val mIoU **80.95@ep38(신기록)** | 저신뢰, 진행 양호 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~177/300(59%) | val mIoU **81.57@ep176(신기록)** | 완주 임박 아님(저신뢰) | alive(ps) |
| yeon | 2 (99%, 14210MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%/96%, 3.1-21.8GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (99%/83%/95%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~151/300 | val mIoU 81.63@ep114 (**불변, 18eval+ 정체 — 계속 최장**) | 저신뢰 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~111/300 | val mIoU **81.43@ep110(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — seed5가 가장 진행률 높음(59%)이나 잔여 40%+ 남아 임박 아님. 나머지는 그보다 여유.

**이상징후**:
- ⚠️ jarvis NVML mismatch 10회 연속, 미복구 지속.
- ⚠️ yeon seed4가 18eval+ 최장 정체 지속(⚠️, 판정 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 02:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**hpca100 P43-MUSES(2,3) — 82 근접, 계속 상승**
- val: ep40=81.06(신기록)→ep42=81.00→ep44=81.73(신기록)→ep46=81.08→ep48=81.38→ep50=81.34→**ep52=81.76(신기록)**→ep54=81.38. Best 74.06→78.76→80.95→**81.76**로 지속 상승, **82 대비 -0.24pt로 최근접**.

**jarvis 드라이버 — NVML mismatch 지속(11회 연속, 미복구) / seed5 미완주**
- 완료 마커 없음 — **아직 진행 중**. val ep178-192: 81.32,81.21,81.52,81.68(신기록),81.45,81.62,81.43,**81.70(신기록)**. Best=81.70@ep192. 현재 ~ep193/300(64% 진행) — 완주 임박 아님.

**yeon seed4 — 18eval+ 정체 돌파(신기록)**
- val ep148-158: 81.13,80.57,81.25,**81.90(신기록, 장기정체 돌파)**,81.25,81.23. Best=81.90@ep154 — seed 계열 전체 최고치 경신.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (75%/89%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%/93%, ~33.7-33.8GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~55/200 | val mIoU **🔴81.76@ep52(신기록) — 82 대비 -0.24pt** | 저신뢰, 진행 양호 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~193/300(64%) | val mIoU **81.70@ep192(신기록)** | 완주 임박 아님 | alive(ps), 미완주 |
| yeon | 2 (98%, 14210MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (32%/96%, 1.9-21.8GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/86%/100%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~159/300 | val mIoU **🔴81.90@ep154(신기록) — seed 전체 최고** | 저신뢰 | alive |
| yeon | 6,7 (91%/100%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~117/300 | val mIoU 81.43@ep110 (**불변, 6eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — seed5가 64%로 가장 진행됐으나 36% 남아 임박 아님.

**이상징후**:
- ⚠️ jarvis NVML mismatch 11회 연속, 미복구 지속.
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 P43은 82 근접하며 정상 진행.

### 2026-07-26 04:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 P43-MUSES(2,3) — 82 돌파 확인!**
- val: ep56=81.62→ep58=81.52→ep60=81.75→ep62=81.66→ep64=81.66→ep66=81.89(신기록)→**ep68=82.04(신기록, 82 돌파)**→ep70=81.87. Best=**82.04@ep68**.
- jarvis P39.1-seed2 최종치(82.62) 대비 **-0.58pt, 아직 미근접**(같은 계열 최고치는 82.62가 여전히 위).

**yeon seed4 — 82 미돌파(근소, -0.10pt)**
- val ep158-168: 81.23,81.47,81.74,81.15,81.30,81.30 — 전부 81.90(ep154) 미만, **6eval+ 정체 지속**. 82 대비 -0.10pt로 근접했으나 아직 미돌파.

**jarvis 드라이버 — NVML mismatch 지속(12회 연속, 미복구) / seed5 정체**
- val ep196-210: 81.47,81.32,81.14,81.43,81.49,81.20,81.27,81.47 — 전부 81.70(ep192) 미만, **8eval+ 정체**. 완주 안 됨(완료 마커 없음), 현재 ~ep211/300(70%) — 임박 아님.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (68%/77%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.7-33.8GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~71/200 | val mIoU **🔴82.04@ep68(신기록, 82 돌파)** | 저신뢰, 진행 양호 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~211/300(70%) | val mIoU 81.70@ep192 (**불변, 8eval+ 정체**) | 완주 임박 아님 | alive(ps), 미완주 |
| yeon | 2 (0%, 14210MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (38%/0%, 1.9-14.2GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/81%/99%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~169/300 | val mIoU 81.90@ep154 (**불변, 82 대비 -0.10pt, 미돌파**) | 저신뢰 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~123/300 | val mIoU 81.43@ep110 (**불변, 12eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — jarvis seed5가 70%로 가장 진행됐으나 30% 남아 임박 아님.

**이상징후**:
- ⚠️ jarvis NVML mismatch 12회 연속, 미복구 지속.
- ⚠️ yeon seed3 12eval+ 최장 정체 지속(⚠️, 판정 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 P43은 82 돌파하며 정상 진행 지속.

### 2026-07-26 06:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**hpca100 P43-MUSES(2,3) — 82.62 미경신, 정체(9eval+)**
- val ep68-86: 82.04(best)→81.87→81.47→81.75→81.42→81.72→81.65→81.87→81.50→81.75. **Best 82.04@ep68에서 불변**(9eval+ 정체). **seed2 최종치(82.62) 미경신, 여전히 -0.58pt**. 현재 ~ep87/200.

**jarvis 드라이버 — NVML mismatch 지속(13회 연속, 미복구) / seed5 정체·미완주**
- 완료 마커 없음. val ep214-228: 81.47,81.37,81.35,81.28,81.57,81.28,81.36,81.36 — 전부 81.70(ep192) 미만, **17eval+ 정체**. 현재 ~ep229/300(76%) — 완주 임박 아님.

**yeon seed3 — 12eval+ 정체 돌파(신기록)**
- val ep118-128: 80.51,80.66,80.71,80.77,81.24,**81.58(신기록)**. Best=81.58@ep128.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (71%/85%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%/99%, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~87/200 | val mIoU 82.04@ep68 (**불변, 9eval+ 정체**) | 저신뢰 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~229/300(76%) | val mIoU 81.70@ep192 (**불변, 17eval+ 정체**) | 완주 임박 아님 | alive(ps), 미완주 |
| yeon | 2 (100%, 3136MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%/96%, 3.1-14.2GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (0%/100%/100%, ~18.4-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~177/300 | val mIoU 81.90@ep154 (**불변, 11eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%/99%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~129/300 | val mIoU **81.58@ep128(신기록)** | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — jarvis seed5가 76%로 가장 진행됐으나 24% 남아 임박 아님.

**이상징후**:
- ⚠️ jarvis NVML mismatch 13회 연속, 미복구 지속.
- ⚠️ hpca100 P43(9eval+)·jarvis seed5(17eval+)·yeon seed4(11eval+) 동시 정체 국면(⚠️, 판정 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 08:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 P43-MUSES(2,3) — 정체 돌파, 82.62 육박(최근접)**
- val ep88-102: **82.28(신기록)**→81.99→82.01→81.90→82.24→**82.50(신기록)**→82.15→82.45. Best=**82.50@ep98** — 9eval 정체 돌파 후 지속 상승.
- **seed2 최종치(82.62) 대비 -0.12pt로 이번 점검 중 가장 근접**(격차 추이: 0.58→0.58→0.12). 아직 미경신.

**jarvis 드라이버 — NVML mismatch 지속(14회 연속, 미복구) / seed5 미완주, 최장 정체**
- 완료 마커 없음. val ep226-244(10개): 81.36,81.36,81.43,81.50,81.54,81.32,81.21,81.22,81.16,81.06 — 전부 81.70(ep192) 미만, **27eval+ 정체**(관측 이래 최장). 현재 ~ep245/300(82%) — 완주 임박 아님.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (100%/95%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~103/200 | val mIoU **🔴82.50@ep98(신기록) — 82.62 대비 -0.12pt, 최근접** | 저신뢰 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~245/300(82%) | val mIoU 81.70@ep192 (**불변, 27eval+ 정체 — 최장**) | 완주 임박 아님 | alive(ps), 미완주 |
| yeon | 2 (100%, 3110MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%×2, ~3.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ~18.5-18.9GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~187/300 | val mIoU 81.90@ep154 (**불변, 16eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~137/300 | val mIoU 81.58@ep128 (**불변**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: 없음 — jarvis seed5가 82%로 가장 진행됐으나 18% 남아 임박 아님.

**이상징후**:
- ⚠️ jarvis NVML mismatch 14회 연속, 미복구 지속.
- ⚠️ jarvis seed5 27eval+ 최장 정체 지속(⚠️, 판정 아님). yeon seed4도 16eval+ 정체.
- Traceback/OOM/nan 없음(신규), D-state 없음. hpca100 P43은 82.62에 근접하며 정상 진행.

### 2026-07-26 10:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 P43-MUSES(2,3) — 82.62 미경신, 18eval+ 정체(⚠️ 소폭 하락 관측)**
- val ep98-116: 82.50(best)→82.15→82.45→82.39→82.24→82.31→81.96→82.23→82.11→**80.60(하락)**. Best **82.50@ep98에서 불변**(18eval+ 정체), **82.62 대비 여전히 -0.12pt, 이번엔 경신 없음**. 최신값(80.60)이 최근 관측 중 최저치.
- 메모리 재차 급감(~8.8GB, 정상 ~33.7GB 대비) — 반복 패턴.

**🔴 jarvis 드라이버 — NVML mismatch 지속(15회 연속, 미복구) / seed5 미완주, 최장 정체 지속**
- 완료 마커 없음 — **아직 완주 안 됨**. val ep244-262(10개): 81.06,81.43,81.20,81.34,81.21,81.29,81.31,81.36,81.51,81.29 — 전부 81.70(ep192) 미만, **35eval+ 정체**(지속 갱신 중, 관측 이래 최장 기록 경신). 현재 ~ep264/300(88% 진행) — 완주 임박하나 이번 점검서도 미완주.

**yeon seed4 — 마킹적 신기록**
- val ep188-198: 81.63,80.97,81.28,81.17,81.81,**81.92(신기록, 근소)**. Best=81.92@ep198.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (84%/74%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ⚠️8.8GB — mem 급감) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~117/200 | val mIoU 82.50@ep98 (**불변, 82.62 미경신**) | 저신뢰 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~264/300(88%) | val mIoU 81.70@ep192 (**불변, 35eval+ 정체 — 최장 갱신**) | 완주 임박이나 미완주 | alive(ps) |
| yeon | 2 (100%, 3104MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%×2, ~3.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/100%/82%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~199/300 | val mIoU **81.92@ep198(신기록, 근소)** | 저신뢰 | alive |
| yeon | 6,7 (97%/65%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~145/300 | val mIoU 81.58@ep128 (**불변**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴).

**완주 임박 런**: jarvis seed5가 88%로 가장 근접(다음 점검서 완주 가능성 높음). 나머지는 여유.

**이상징후**:
- ⚠️ jarvis NVML mismatch 15회 연속, 미복구 지속.
- ⚠️ hpca100 P43 최신값(80.60@116)이 최근 관측 중 최저 — 하락 폭 주시 필요(판정 아님).
- ⚠️ jarvis seed5 35eval+ 최장 정체 갱신 지속.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 12:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**hpca100 P43-MUSES(2,3) — dip 회복, 안정, 붕괴 아님**
- val ep114-132: 82.11→**80.60(직전 관측된 dip)**→82.04→82.19→82.14→82.19→81.84→82.45→82.27→82.27. **dip 이후 즉시 82.0-82.5대로 복귀** — 일시적 요동으로 확인, 지속 하락·붕괴 아님. Traceback/NaN/Inf 없음. Best는 여전히 **82.50@ep98 불변**(82.62 미경신).

**🔴 jarvis 드라이버 — NVML mismatch 지속(16회 연속, 미복구) / seed5 미완주, 정체 최장 갱신**
- 완료 마커 없음 — **아직 미완주**. val ep262-280(10개): 81.29,81.12,81.17,81.14,81.24,81.32,81.24,81.26,81.23,81.21 — 전부 81.70(ep192) 미만, **45eval+ 정체**(계속 최장 갱신). 현재 ~ep281/300(93.7%) — **매우 임박, 다음 점검서 완주 확정 예상**. GPU2-5 여전히 미점유(우리 쪽에서는), 해방 여부는 nvidia-smi 불가로 미확인.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (98%/70%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~133/200 | val mIoU 82.50@ep98 (**불변, 안정 — dip 회복됨**) | 저신뢰 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~281/300(93.7%) | val mIoU 81.70@ep192 (**불변, 45eval+ 정체 — 최장**) | **매우 임박(~2h 이내 완주 예상)** | alive(ps), 미완주 |
| yeon | 2 (33%, 1940MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%/31%, 1.9-3.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%×3, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~211/300 | val mIoU 81.92@ep198 (**불변, 6eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%/83%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~153/300 | val mIoU 81.58@ep128 (**불변, 12eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음, seed5 완주 시 GPU2-5는 원래 무관하므로 별도 해방 대상 아님 — seed5는 GPU6,7 사용); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴, util 33%).

**완주 임박 런**: **jarvis seed5(GPU6,7)가 93.7%로 매우 임박** — 다음 점검(약 2h 후)에서 완주 확정 예상, 완주 시 GPU6,7 해방 여부 확인 필요.

**이상징후**:
- ⚠️ jarvis NVML mismatch 16회 연속, 미복구 지속.
- ⚠️ jarvis seed5 45eval+ 최장 정체 지속 갱신 중(⚠️, 판정 아님).
- hpca100 P43 dip(80.60)은 일시적 요동으로 확인, 붕괴 아님 — 안정.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 14:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis seed5(GPU6,7) — 아직 미완주, 초임박(ep298/300)**
- 완료 마커 없음 — **아직 진행 중**. val ep282-296(8개): 81.15,81.29,81.17,81.25,81.29,81.14,81.16,81.19 — 전부 81.70(ep192) 미만, **52eval+ 정체**(계속 최장 갱신). 원문 로그: `Epoch [298/300] ... 62%|██████▏ | 465/750` — **잔여 2epoch 미만, 약 15-20분 내 완주 예상**(다음 점검서 확정 필요).
- 드라이버: NVML mismatch **지속(17회 연속, 미복구)** — ps/로그만으로 확인.

**hpca100 P43-MUSES(2,3) — 정체 지속, 안정**
- val ep134-148: 82.35,82.29,81.86,82.26,82.19,82.38,82.48,82.46 — 전부 82.50(ep98) 미만이나 82대 유지, 붕괴 없음. **Best 82.50@ep98 불변**(25eval+ 정체).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (99%/100%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (92%/99%, ~33.7-33.9GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~149/200 | val mIoU 82.50@ep98 (**불변, 25eval+ 정체, 안정**) | 저신뢰 | alive, 정상 |
| jarvis | 2-5 (nvidia-smi 불가) | (없음 — ps상 미점유) | — | — | — | — | 프로세스 없음(GPU 미확인) |
| jarvis | 6,7 (nvidia-smi 불가, ps 확인) | `jarvis-muses_rgbel_P39_1_rank_seed5.yaml` (P39.1-seed5) | MUSES | ~298/300 | val mIoU 81.70@ep192 (**불변, 52eval+ 정체 — 최장**) | **~15-20분 내 완주 예상**(미완주) | alive(ps) |
| yeon | 2 (100%, 17323MiB, 타유저 2건) | (없음) | — | — | — | — | 타유저 점유(2 프로세스) |
| yeon | 3,4 (100%/32%, 1.9-3.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/93%/100%, ~18.4-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~221/300 | val mIoU 81.92@ep198 (**불변, 11eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (98%/100%, ⚠️6.5-6.8GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~161/300 | val mIoU 81.58@ep128 (**불변, 16eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 0장 — 확정 유휴 없음**. hpca100 0,1=타유저(비유휴); jarvis GPU2-5=nvidia-smi 불가로 미확정(프로세스 없음); **yeon 3,4=user 예약**(별도 태그, 유휴 아님); yeon 2=타유저(비유휴, 프로세스 2건으로 증가).

**완주 임박 런**: **jarvis seed5(GPU6,7)가 ep298/300, ~15-20분 내 완주 예상** — 다음 점검서 완주·최종best·GPU해방 여부 확정 필요.

**이상징후**:
- ⚠️ jarvis NVML mismatch 17회 연속, 미복구 지속.
- ⚠️ seed3/4/main P43 모두 동시 정체 국면(16eval+/11eval+/25eval+), 붕괴 징후는 없음(값이 유한 범위 내 등락).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 16:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis P39.1-seed5(GPU6,7) — 완주 확인, COMPLETED**
- 로그 종료 확인: `Total Training Time 10:47:17`, epoch300 최종 val mIoU=81.19, **Best Val mIoU 81.70 (epoch 192)** — 최종 결과 확정(52eval+ 플래토 그대로 종료), 완주 시각 14:51:59 KST.
- ckpt: `epoch192_81.7_top1_checkpoint.pth` + `last_checkpoint.pth`(ep300 최종본, 14:50 저장) — 경로: `/home/jemo_maeng/src/drone-MemorySAM-develop/outputs/ReliaDINO/jarvis_muses_rgbel_P39_1_rank_seed5/`.
- **프로세스 완전 종료 확인**: `ps aux | grep train_reliadino`가 **공백(0건)** — jarvis에 우리 학습 프로세스가 **전무**(seed2도 이전에 종료, 이제 seed5도 종료로 jarvis 전체가 미점유 상태). GPU 레벨 확인은 nvidia-smi 불가로 미확정.
- 🔴 **드라이버 미복구로 신규 잡 기동 불가 상태 지속** — 완주로 GPU가 논리적으로는 비어 있으나 NVML mismatch 때문에 새 CUDA 학습을 올릴 수 없는 상태로 추정(사실만 보고, 판정 아님).

**jarvis 드라이버 — NVML mismatch 지속(18회 연속, 미복구)**

**hpca100 P43-MUSES(2,3) — 초근소 신기록, 사실상 정체**
- val ep150-164: 82.10,82.26,82.15,**82.51(신기록, 근소)**,82.16,82.23,82.07,82.13. Best=82.51@ep156(직전 82.50@98 대비 +0.01, 사실상 정체). 82.62 대비 여전히 -0.11pt.
...
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (98%/99%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (94%×2, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~165/200 | val mIoU 82.51@ep156(신기록, 근소) | 저신뢰 | alive, 정상 |
| jarvis | 2-5,6,7 | **(없음 — 전체 완주/종료, 위 참조)** | — | — | seed5 최종 81.70@ep192 | **완주됨** | 프로세스 전무(GPU 미확인, nvidia-smi 불가) |
| yeon | 2 (100%, 17327MiB, 타유저 2건) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%×2, ~3.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (100%/72%/99%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~231/300 | val mIoU 81.92@ep198 (**불변, 근접값 81.69@230**) | 저신뢰 | alive |
| yeon | 6,7 (0%/100%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~167/300 | val mIoU 81.58@ep128 (**불변, 19eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100·yeon 기준 0장 확정 유휴 없음**. **jarvis는 전체 학습 프로세스 종료됐으나(GPU2-5, 6-7 모두) nvidia-smi 불가로 GPU 레벨 유휴 확정 불가** — 논리적으로는 8장 전부 비어있을 가능성이 높으나 드라이버 문제로 검증 불가 및 신규 잡 기동도 불가한 상태로 추정. **yeon 3,4=user 예약**(별도 태그, 유휴 아님).

**완주 임박 런**: 없음(이번에 seed5 완주 확정 처리됨). 나머지 3개 run은 여유.

**이상징후**:
- 🔴 jarvis NVML mismatch 18회 연속, 미복구 지속 — **전체 학습 종료 후에도 신규 기동 불가 상태로 자원이 사실상 마비**(사실만 보고).
- ⚠️ yeon seed3 19eval+ 최장 정체 지속.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 18:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis 드라이버 — NVML mismatch 지속(19회 연속, 미복구 확정) / 학습 잡 0 확인**
- `nvidia-smi`: 동일 오류 지속. `uptime -s`=`2026-07-10 15:08:01`(부팅 이후 리부트 없음, 16일째) — **재부팅 안 됨, 복구 조치 없었던 것으로 확인**.
- `ps aux | grep train_reliadino` **완전 공백(0건)** — seed2·seed5 둘 다 완주 후 **신규 잡 전무**. jarvis 전체가 학습 미점유 상태로 추정되나, **드라이버 문제로 GPU 레벨 유휴 확인도, 신규 잡 기동도 불가능한 상태**(사실만 보고, 판정 아님).

**hpca100 P43-MUSES(2,3) — 미완주, 정체 지속(12eval+)**
- 완료 마커 없음. val ep166-180: 82.05,82.06,82.28,82.26,82.03,82.05,81.88,82.14 — 전부 82.51(ep156) 미만. **Best 82.51@ep156 불변**, 82.62 대비 -0.11pt 그대로. 현재 ~ep181/200(90.5%) — 완주 임박(다음 점검서 확정 필요).

**yeon seed3/seed4 — 둘 다 장기 정체 지속**
- seed3: val ep162-172(6개) 전부 81.58(ep128) 미만, **22eval+ 정체**.
- seed4: val ep228-238(6개) 전부 81.92(ep198) 미만, **20eval+ 정체**.
...
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (98%/99%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (99%×2, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~181/200(90.5%) | val mIoU 82.51@ep156 (**불변, 12eval+ 정체**) | 완주 임박(다음 점검 확인) | alive, 미완주 |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건)** | — | — | — | — | 프로세스 전무, GPU상태·신규기동 불가 추정(드라이버) |
| yeon | 2 (0%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (100%×2, 3.1-5.1GB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (0%×3, ⚠️6.4-7.3GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~239/300 | val mIoU 81.92@ep198 (**불변, 20eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (100%/55%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~173/300 | val mIoU 81.58@ep128 (**불변, 22eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100·yeon 기준 0장 확정 유휴**. **jarvis는 학습 프로세스 0건이나 nvidia-smi 불가로 GPU 레벨 유휴 확정 불가**(19회 연속 드라이버 미복구로 신규 기동도 불가 추정). **yeon 3,4=user 예약**(별도 태그, 유휴 아님).

**완주 임박 런**: **hpca100 P43-MUSES가 90.5%로 임박** — 다음 점검서 완주 여부·최종best 확정 필요.

**이상징후**:
- 🔴 jarvis NVML mismatch 19회 연속 확정, 미복구 — 학습 종료 후에도 자원이 실질적으로 마비 상태 지속.
- ⚠️ yeon seed3(22eval+)·seed4(20eval+) 동시 장기 정체.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 20:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 jarvis 드라이버 — NVML mismatch 지속(20회 연속 확정, 미복구) / 학습 잡 0 유지**
- `nvidia-smi` 동일 오류. `uptime -s`=`2026-07-10 15:08:01`(불변, 리부트 없음). `ps aux|grep train_reliadino` **공백(0건) 유지** — 신규 잡 전무 지속.

**hpca100 P43-MUSES(2,3) — 미완주, 초임박(ep197/200)**
- 완료 마커 없음. val ep178-196(10개): 81.88,82.14,82.11,82.13,82.21,82.16,82.21,82.20,82.22,82.20 — 전부 82.51(ep156) 미만, **20eval+ 정체 지속**. 원문 로그: `Epoch [197/200]...33%` — **잔여 3epoch 미만, 약 20-25분 내 완주 예상**(이번 점검서는 미완주, 다음 점검서 확정 필요).

**yeon seed3/seed4 — 장기 정체 지속(각 24/23eval+)**
- seed3: val ep166-176(6개) 전부 81.58(ep128) 미만, **24eval+ 정체**.
- seed4: val ep236-246(6개) 전부 81.92(ep198) 미만, **23eval+ 정체**, 최신값(81.03@246)이 최근 저점.

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (99%×2, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (75%/94%, ~33.7GB) | `hpca100-muses_rgbel_P43_pdual.yaml` (P43-MUSES) | MUSES | ~197/200 | val mIoU 82.51@ep156 (**불변, 20eval+ 정체**) | **~20-25분 내 완주 예상**(미완주) | alive |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건 지속)** | — | — | — | — | 프로세스 전무, GPU상태 미확인(드라이버 20회 미복구) |
| yeon | 2 (0%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (0%×2, ~19-18MiB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님, 수치상 낮으나 별도 태그 유지) |
| yeon | 0,1,5 (100%/100%/99%, ~18.4-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~247/300 | val mIoU 81.92@ep198 (**불변, 23eval+ 정체**) | 저신뢰 | alive |
| yeon | 6,7 (0%×2, ⚠️3.4GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~177/300 | val mIoU 81.58@ep128 (**불변, 24eval+ 정체**) | 저신뢰 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100·yeon(user예약 제외) 기준 0장 확정 유휴**. **jarvis는 학습 프로세스 0건이나 nvidia-smi 불가로 GPU 레벨 유휴 확정 불가**(20회 연속 드라이버 미복구). **yeon 3,4=user 예약**(수치상 거의 유휴 수준이나 지시에 따라 별도 태그, 유휴 채우기 대상 아님).

**완주 임박 런**: **hpca100 P43-MUSES가 ep197/200, ~20-25분 내 완주 예상** — 다음 점검서 완주 확정·최종best 필요.

**이상징후**:
- 🔴 jarvis NVML mismatch 20회 연속 확정, 미복구 지속.
- ⚠️ yeon seed3(24eval+)·seed4(23eval+) 동시 장기 정체 지속, 최신값 저점(seed4 81.03).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-26 22:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 hpca100 P44-BMR(GPU2,3) — 생존 확인, 정상 진행**
- cfg 확인: `configs/hpca100-muses_rgbel_P44_bmr.yaml`(ps로 실측). val: ep2=47.95→ep4=68.17(신기록)→**ep6=71.69(신기록)**. 현재 ~ep9/200(80%|301/375 진행 중).
- P44 손실 스칼라 전부 유한: `[P44-B1] cos_lora_img:+0.485~+0.522, cos_lora_lidar:+0.188~+0.215, cos_lora_event:+0.143~+0.182, cos_shared:+0.013~+0.017`(전부 양수) / `[P44-B2] mutual_kl:0.0000 rel_corr:0.0000`(WARMUP_EP=10 이내라 0 정상, 붕괴 아님) / `[P44] mask_rate=0.0139(ep5)→0.0256(ep6)`(LOCAL_MASK WARMUP_EP=20 내 점진 증가, 설계대로). Traceback/에러 없음.
- 페이스: ep2→ep4=10.5min/ep, ep4→ep6=10.5min/ep(일관). 잔여~192ep×10.5min≈33.6h → **ETA ~07-28 08:10 KST경**(plan.md 원 추정 07-29보다 다소 이름, 재확정).

**jarvis 드라이버 — NVML mismatch 지속(21회 연속, 미복구) / 학습 잡 0 유지**
- `uptime -s`=`2026-07-10 15:08:01`(불변). `ps aux|grep train_reliadino` 공백(0건) 유지.

**yeon seed3/seed4 — 둘 다 미완주, 초장기 정체**
- seed3: val ep168-182(8개) 전부 81.58(ep128) 미만, **28eval+ 정체**. 현재 ~ep183/300(61%).
- seed4: val ep240-254(8개) 전부 81.92(ep198) 미만, **28eval+ 정체**. 현재 ~ep255/300(85%).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (98%/100%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.3GB) | `hpca100-muses_rgbel_P44_bmr.yaml` (P44-BMR) | MUSES | ~9/200 | val mIoU 71.69@ep6(신기록) | ~07-28 08:10 KST경(재확정) | alive, 정상 |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건 지속)** | — | — | — | — | 프로세스 전무, GPU상태 미확인(드라이버 21회 미복구) |
| yeon | 2 (100%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (0%×2, ~19-18MiB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (81%/100%/100%, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~255/300(85%) | val mIoU 81.92@ep198 (**불변, 28eval+ 정체**) | 저신뢰, 미완주 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~183/300(61%) | val mIoU 81.58@ep128 (**불변, 28eval+ 정체**) | 저신뢰, 미완주 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100·yeon 기준 0장 확정 유휴**. **jarvis는 학습 프로세스 0건이나 nvidia-smi 불가로 GPU 레벨 유휴 확정 불가**(21회 연속 드라이버 미복구). **yeon 3,4=user 예약**(별도 태그, 유휴 아님).

**완주 임박 런**: 없음(P44 초반, seed3/4는 61%/85%지만 페이스상 각각 여러 시간~1일+ 남음).

**이상징후**:
- 🔴 jarvis NVML mismatch 21회 연속 확정, 미복구 지속.
- ⚠️ yeon seed3·seed4 동시 28eval+ 장기 정체(⚠️, 판정 아님, 붕괴 징후는 없음).
- P44-BMR mutual_kl/rel_corr=0.0000은 설계된 warmup 기간 내 정상 동작(붕괴 아님).
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-27 00:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🆕 hpca100 P44-BMR(GPU2,3) — B-2 warmup 해제 확인(ep10 이후), 정상 진행**
- val: ep4=68.17→ep6=71.69→ep8=74.09→ep10=75.41(신기록)→ep12=72.51→**ep14=75.83(신기록)**→ep16=73.87→ep18=75.43. Best=75.83@ep14.
- **B-2(mutual_kl/rel_corr) warmup 해제 확인**: ep15-18에서 `mutual_kl:0.0694→0.0639→0.0644→0.0615`(더 이상 0 아님, WARMUP_EP=10 이후 정상 활성화), `rel_corr:0.0013→0.0012→0.0013→0.0012`(소폭이나 0 아님) — **설계대로 작동**.
- **cos_lora_lidar 계속 양수 유지**: +0.124→+0.117→+0.106→+0.114(ep15-18). cos_lora_img(+0.218~+0.265)·event(+0.091~+0.110)·shared(+0.009~+0.012) 전부 양수. mask_rate 계속 증가(0.0509→0.0616, WARMUP_EP=20 내 점진). Traceback/에러 없음.
- 현재 ~ep19/200. 페이스 ~10.5min/ep(일관). 잔여~181ep×10.5min≈31.7h → **ETA ~07-28 08:14 KST경**(직전 추정과 거의 일치).

**jarvis 드라이버 — NVML mismatch 지속(22회 연속, 미복구) / 학습 잡 0 유지**
- `uptime -s`=`2026-07-10 15:08:01`(불변). `ps aux|grep train_reliadino` 공백(0건) 유지.

**yeon seed3/seed4 — 둘 다 미완주, 초장기 정체(36eval+)**
- seed3: val ep174-188(8개) 전부 81.58(ep128) 미만, **36eval+ 정체**. 현재 ~ep189/300(63%).
- seed4: val ep248-262(8개) 전부 81.92(ep198) 미만, **36eval+ 정체**. 현재 ~ep263/300(87.7%).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (77%/93%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%/99%, ~33.4GB) | `hpca100-muses_rgbel_P44_bmr.yaml` (P44-BMR) | MUSES | ~19/200 | val mIoU 75.83@ep14 (신기록, B-2 warmup 해제 확인) | ~07-28 08:14 KST경 | alive, 정상 |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건 지속)** | — | — | — | — | 프로세스 전무, GPU상태 미확인(드라이버 22회 미복구) |
| yeon | 2 (0%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3,4 (0%×2, ~19-18MiB) | 🔒 **user 예약**(P42-f07 종료, 학습 제외) | — | — | — | — | **user 예약**(유휴 채우기 대상 아님) |
| yeon | 0,1,5 (91%/99%/99%, ~18.5-18.8GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~263/300(87.7%) | val mIoU 81.92@ep198 (**불변, 36eval+ 정체**) | 저신뢰, 미완주 | alive |
| yeon | 6,7 (99%/100%, ⚠️6.3-6.6GB — mem 급감) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~189/300(63%) | val mIoU 81.58@ep128 (**불변, 36eval+ 정체**) | 저신뢰, 미완주 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100·yeon 기준 0장 확정 유휴**. **jarvis는 학습 프로세스 0건이나 nvidia-smi 불가로 GPU 레벨 유휴 확정 불가**(22회 연속 드라이버 미복구). **yeon 3,4=user 예약**(별도 태그, 유휴 아님).

**완주 임박 런**: 없음(seed4가 87.7%로 가장 근접이나 페이스상 여전히 수 시간+ 남음).

**이상징후**:
- 🔴 jarvis NVML mismatch 22회 연속 확정, 미복구 지속.
- ⚠️ yeon seed3·seed4 동시 36eval+ 초장기 정체 지속(⚠️, 판정 아님, 붕괴 징후 없음).
- P44-BMR B-2(mutual_kl/rel_corr) warmup 해제 후 정상 활성화 확인 — 이상 아님, 설계대로.
- Traceback/OOM/nan 없음(신규), D-state 없음.

### 2026-07-27 02:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**🔴 hpca100 P44-BMR(GPU2,3) — ep30 돌파 확인, 신기록**
- val: ep12=72.51→ep14=75.83→ep16=73.87→ep18=75.43→ep20=75.46→ep22=77.32(신기록)→ep24=75.06→ep26=77.41(신기록)→**ep28=78.08(신기록)**→ep30=77.61. Best=**78.08@ep28**. **현재 ~ep31/200 — ep30 게이트 대상 통과, 판정은 상위 세션 몫**.
- ep30 시점 상태(사전등록 게이트 참고 수치): `cos_lora_lidar` ep28=+0.106/ep29=+0.116(양수 유지), `mutual_kl` 0.0595→0.0586(안정), `rel_corr` 0.0011(안정). Traceback 없음, 손실 유한.
- 페이스 ~10.5min/ep 일관. 잔여~169ep×10.5min≈29.6h → **ETA ~07-28 08:10 KST경**(직전 추정과 일치).

**jarvis 드라이버 — NVML mismatch 지속(23회 연속, 미복구) / 학습 잡 0 유지**
- `uptime -s`=`2026-07-10 15:08:01`(불변). `ps aux|grep train_reliadino` 공백(0건) 유지.

**yeon seed3/seed4 — 둘 다 미완주, 초장기 정체(42eval+)**
- seed3: val ep184-194(6개) 전부 81.58(ep128) 미만, **42eval+ 정체**. 현재 ~ep195/300(65%).
- seed4: val ep260-270(6개) 전부 81.92(ep198) 미만, **42eval+ 정체**. 현재 ~ep271/300(90.3%).

**yeon GPU3 = P43 seg-analysis 파이프라인 — 4단계 통과, 에러 없음, 진행 중**
- 스테이지 순서 확인: `D3_adapter_health` → `D1_eval_per_domain` → `D1_analyze_per_domain` → `module_diagnostics` → **현재 `D2N_feature_stats` 진입**(가장 최근 로그 줄). Traceback/Error 없음(FutureWarning 제외).
- `available=[...] skipped=[...]` 토글 줄은 **아직 로그에 없음**(ablation-toggle 스테이지가 D2N feature_stats 이후로 추정, 다음 점검서 재확인 필요 — p43_lateral_off 포함 여부 미확인).
- 프로세스 생존 확인(PID 5317), GPU3=0%/5272MiB(단계 전환 순간 관측, 이전 체크에서는 69%util 활성 확인됨). **GPU4는 여전히 미사용 확인**(18MiB, 0%) — 추가 배치 없음.
...
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (99%×2, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ~33.5GB) | `hpca100-muses_rgbel_P44_bmr.yaml` (P44-BMR) | MUSES | ~31/200 | val mIoU 78.08@ep28(신기록, ep30 돌파) | ~07-28 08:10 KST경 | alive, 정상 |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건 지속)** | — | — | — | — | 프로세스 전무, GPU상태 미확인(드라이버 23회 미복구) |
| yeon | 2 (0%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3 (0%, 5272MiB) | P43 seg-analysis(`dm_analysis`, D2N_feature_stats 단계) | MUSES(val) | 스테이지 진행형(4/N 통과) | — | 미상(스테이지 다수 남음) | alive, 에러 없음 |
| yeon | 4 (0%, 18MiB) | (미사용, 분석 세션용 예약 유지) | — | — | — | — | 유휴 유지(배치 금지) |
| yeon | 0,1,5 (0%/100%/100%, ~18.5-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~271/300(90.3%) | val mIoU 81.92@ep198 (**불변, 42eval+ 정체**) | 저신뢰, 미완주 | alive |
| yeon | 6,7 (74%/100%, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~195/300(65%) | val mIoU 81.58@ep128 (**불변, 42eval+ 정체**) | 저신뢰, 미완주 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 0장, yeon GPU4만 유휴이나 분석세션 예약이라 배치 금지**. jarvis GPU2-5는 nvidia-smi 불가로 미확정(프로세스 없음). yeon GPU2=타유저(비유휴).

**완주 임박 런**: 없음(seed4가 90.3%로 가장 근접이나 페이스상 여전히 수 시간 남음).

**이상징후**:
- 🔴 jarvis NVML mismatch 23회 연속 확정, 미복구 지속.
- ⚠️ yeon seed3·seed4 동시 42eval+ 초장기 정체 지속(⚠️, 판정 아님, 붕괴 징후 없음).
- Traceback/OOM/nan 없음(신규, P43 분석 포함), D-state 없음.

### 2026-07-27 04:34 KST (cron) — 3-server 실측 조회 (판정 없음, 조회 전용)

**yeon GPU3 = P43 seg-analysis — 미완료, 6단계 진입, 에러 없음**
- 스테이지 진행: `D3_adapter_health`→`D1_eval_per_domain`→`D1_analyze_per_domain`→`module_diagnostics`→`D2N_feature_stats`→**`D3B_modal_adaptation`(현재, 방금 시작)**. Traceback/Error 없음.
- **`available=[...] skipped=[...]` 토글 줄 여전히 미출현** — ablation-toggle 스테이지가 D3B 이후로 추정, 다음 점검서 재확인.
- 완료된 스테이지 산출물 확인(`/SSDb/jemo_maeng/analysis_out/P43_eval_20260727/`): `adapter_health.json`, `capability.json`, `D1_eval_per_domain.log`, `D1_analyze_per_domain.log`, `per_domain_analysis.md`, `per_domain/`(11개 조건별 `best__<cond>.log`+`cfg_best_<cond>.yaml`), `module_diag.json`+`module_diagnostics.log`, `D2N_feature_stats.log`+`feature_stats.json`+`feature_stats.md`+`feature_stats_pca.png`, `D3B_modal_adaptation.log`(진행 중). 프로세스 생존 확인(PID 5317). GPU3=0%/5276MiB(스테이지 전환 순간), GPU4 여전히 미사용(18MiB,0%) — 추가 배치 없음.

**🔴 hpca100 P44-BMR(GPU2,3) — 계속 상승, 신기록**
- val ep22-40: 77.32→75.06→77.41(신기록)→78.08(신기록)→77.61→77.98→77.99→78.24(신기록)→**78.31(신기록)**→78.28. Best=**78.31@ep38**.
- ep40-41: `cos_lora_lidar` +0.104/+0.095(양수 유지), `mutual_kl` 0.0524/0.0518(안정), `rel_corr` 0.0010(안정). Traceback 없음.
- ⚠️ 메모리 재차 급감(~7.8GB, 정상 ~33.5GB 대비) — 반복 관측 패턴. 현재 ~ep42/200. 페이스 ~10.5min/ep. 잔여~158ep×10.5min≈27.65h → **ETA ~07-28 08:14 KST경**(직전 추정과 일치).

**jarvis 드라이버 — NVML mismatch 지속(24회 연속, 미복구) / 학습 잡 0 유지**

**yeon seed3/seed4 — 둘 다 미완주, 초장기 정체(48eval+)**
- seed3: val ep190-200(6개) 전부 81.58(ep128) 미만, **48eval+ 정체**. 현재 ~ep201/300(67%).
- seed4: val ep266-276(6개) 전부 81.92(ep198) 미만, **48eval+ 정체**. 현재 ~ep277/300(92.3%).

**서버별 현황**

| 서버 | GPU | 실험(--cfg 실측) | 데이터셋 | epoch(현재/총) | best@ep | ETA(KST) | 생존 |
|---|---|---|---|---|---|---|---|
| hpca100 | 0,1 (83%/82%, ~36GB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| hpca100 | 2,3 (100%×2, ⚠️7.8GB — mem 급감) | `hpca100-muses_rgbel_P44_bmr.yaml` (P44-BMR) | MUSES | ~42/200 | val mIoU 78.31@ep38(신기록) | ~07-28 08:14 KST경 | alive, 정상 |
| jarvis | 전체(2-7) | **(없음 — 학습 프로세스 0건 지속)** | — | — | — | — | 프로세스 전무, GPU상태 미확인(드라이버 24회 미복구) |
| yeon | 2 (56%, 14238MiB, 타유저) | (없음) | — | — | — | — | 타유저 점유 |
| yeon | 3 (0%, 5276MiB) | P43 seg-analysis(D3B_modal_adaptation 단계) | MUSES(val) | 스테이지 6/N 진행 중 | — | 미상 | alive, 에러 없음 |
| yeon | 4 (0%, 18MiB) | (미사용, 분석 세션용 예약 유지) | — | — | — | — | 유휴 유지(배치 금지) |
| yeon | 0,1,5 (94%/92%/57%, ~18.4-18.7GB) | `yeon-muses_rgbel_P39_1_seed4.yaml` (P39.1-seed4) | MUSES | ~277/300(92.3%) | val mIoU 81.92@ep198 (**불변, 48eval+ 정체**) | 저신뢰, 미완주 | alive |
| yeon | 6,7 (100%×2, ~17.4GB) | `yeon-muses_rgbel_P39_1_seed3.yaml` (P39.1-seed3) | MUSES | ~201/300(67%) | val mIoU 81.58@ep128 (**불변, 48eval+ 정체**) | 저신뢰, 미완주 | alive |

**유휴 GPU(≤2000MiB & ≤10%util)**: **🔴 hpca100 0장, yeon GPU4만 유휴(분석세션 예약, 배치 금지)**. jarvis GPU2-5는 nvidia-smi 불가로 미확정. yeon GPU2=타유저(비유휴).

**완주 임박 런**: 없음(seed4가 92.3%로 가장 근접이나 여전히 수 시간 남음).

**이상징후**:
- 🔴 jarvis NVML mismatch 24회 연속 확정, 미복구 지속.
- ⚠️ hpca100 GPU2,3 메모리 급감 재관측(P44-BMR, ~7.8GB).
- ⚠️ yeon seed3·seed4 동시 48eval+ 초장기 정체 지속(⚠️, 판정 아님, 붕괴 징후 없음).
- Traceback/OOM/nan 없음(신규, P43 분석 포함), D-state 없음.

## 2026-07-27 06:40 KST 정기점검

| 서버 | GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%,~33.5GB) | P44-BMR | MUSES rgbel(3모달) | ep52/200 | **78.67@ep52 (신기록, 직전 78.31)** | ~07-28 08:20 | ✅ |
| yeon | 0,1,5 | seed4 (P39.1-rank) | MUSES rgbel | ep284/300 | 81.92@ep198 (54+ eval 정체 지속, 변화없음) | 임박 | ✅ |
| yeon | 6,7 | seed3 (P39.1-rank) | MUSES rgbel | ep204/300 | **81.89@ep204 (신기록 — 48+ eval 정체 돌파, 직전 81.58@ep128)** | ~수 시간내 | ✅ |
| yeon | 3 | P43 분석 파이프라인 | MUSES | **완료** (8/8 stage ok, 20039s) | - | - | 종료 |
| yeon | 4 | P43 predict(test) | MUSES test | **완료** (750/750 PNG) — zip/staging 미실행 | - | - | 종료 |
| lecun | 4 | seed2(P39.1-rank) 분석 | MUSES | module_diagnostics 스테이지 진행중 (95%util) | - | - | ✅ |
| lecun | 5 | (미사용, 확인됨: 16MiB/0%, 프로세스 0개) | - | - | - | - | idle |
| jarvis | - | (드라이버 불일치 25회 연속) | - | 잡 0개 | - | - | nvidia-smi 실패 |

**P44-BMR ep51-52 스칼라**: `cos_lora_lidar` +0.104/+0.084 (양수 유지) · `mutual_kl` 0.0478/0.0495 · `rel_corr` 0.0010 — 모두 안정/nonzero, Traceback 없음.

**P43 분석 — toggle 가용성 라인 (D5_module_ablation.log, 최상위 파이프라인 로그가 아니라 이 서브로그에 있었음)**:
`[module_ablation] available=['p34_gate_off', 'p34_veto_off', 'p34_calib_off', 'p36_router_off', 'p43_lateral_off'] skipped=['rbma_off', 'router_off', 'ctd_off', 'sdc_off', 'temp_off', 'cons_off', 'amf_uniform', 'p34_bias_off', 'p34_cons_off', 'p37_cefr_off', 'p38_m2f_off', 'p39_query_off', 'p39_trunkexp_off', 'p43_m2f_off']`
→ `p43_lateral_off` available, 전 조건 Δ>0 (clear +1.86 / fog +0.35 / rain +0.77 / snow +1.12 / night +0.56 / snow_night +1.28), feat_cos≈0.75(no-op 아님). p34_gate/veto/calib_off는 전 조건 Δ=+0.00·feat_cos=1.0(변화 없음).

⚠️ **P43 predict 산출(750 PNG, 정상)이 zip/staging까지 이어지지 않음** — `/ailab_mat2/.../submission/muses/`에 `muses_P43*` zip 없음, 로컬(`/SSDb/jemo_maeng`)에도 zip 파일 없음. 이전 지시([3]-[4] 단계)가 미완 상태로 남음 — 후속 조치로 처리 예정.
🔴 jarvis: `Driver/library version mismatch, NVML library version: 580.173` 25회 연속, `uptime -s` 여전히 2026-07-10 15:08:01 (재부팅 없음).

**[후속조치 완료, 06:40 KST]** P43 predict zip/staging 처리: yeon `pred/*.png` 750장 → `labelTrainIds/`로 재구성 → `muses_P43_pdual_3modal_ep156_submission.zip`(12,694,681 bytes) 생성 → `/ailab_mat2/.../submission/muses/`에 복사 → 스테이징본 재검증: 크기 동일(12,694,681B) · 내부 png 750장 확인. 완료.

## 2026-07-27 08:35 KST 정기점검

| 서버 | GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/99%, ~33.4GB) | P44-BMR | MUSES rgbel(3모달) | ep64/200 | **79.25@ep64 (신기록, 직전 78.67)** | ~07-28 08:15 | ✅ |
| yeon | 0,1,5 (100%,~18.7GB) | seed4 (P39.1-rank) | MUSES rgbel | ep294/300 | 81.92@ep198 (정체 지속, 변화없음) | ~09:45 (오늘) | ✅ |
| yeon | 6,7 (100%,~17.4GB) | seed3 (P39.1-rank) | MUSES rgbel | ep212/300 | 81.89@ep204 (직전 사이클 신기록 유지) | ~07-28 10:40 | ✅ |
| yeon | 2,3,4 | (타 유저 jongwon_kim의 hoi_transformer eval job, 우리 작업 아님) | - | - | - | - | - |
| lecun | 4 (100%,~5.2GB) | seed2(P39.1-rank) 분석 | MUSES | D5_module_ablation 스테이지 진행중 (D1~D3B 완료) | - | - | ✅ |
| lecun | 5 | (미사용, 재확인: 16MiB/0%) | - | - | - | - | idle |
| jarvis | - | (드라이버 불일치 26회 연속) | - | 잡 0개 | - | - | nvidia-smi 실패 |

**P44-BMR ep62-64 스칼라**: `cos_lora_lidar` +0.079/+0.082/+0.074 (양수 유지) · `mutual_kl` 0.0446~0.0485 · `rel_corr` 0.0009~0.0010 — 안정, Traceback/NaN 없음.

**seed2 분석(lecun) — 미완료**: D3_adapter_health→D1_eval_per_domain→D1_analyze_per_domain→module_diagnostics→D2N_feature_stats→D3B_modal_adaptation 순차 완료(모두 06:03~07:52 사이 완료), 현재 **D5_module_ablation 진행 중**(08:32 시작, `[module_ablation] wrote ...` 미발견 — 완료 대기). 에러 없음. **핵심수치 덤프는 파이프라인 완료 후 별도 요청 시 제공** — 이번 사이클엔 미완료로 스테이지만 보고.

⚠️ yeon GPU2,3,4: 우리 예약(GPU3,4)과 무관하게 **타 사용자(jongwon_kim)의 hoi_transformer 평가 작업**이 GPU2,3,4에서 각각 ~3.08GB로 실행 중 확인(PID 378808/278402/306296) — 우리 학습·분석 잡 아님, 조치 불필요하나 사실 기록.
🔴 jarvis: `Driver/library version mismatch` 26회 연속, `uptime -s` 2026-07-10 15:08:01 그대로.

## 2026-07-27 10:35 KST 정기점검

| 서버 | GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|---|
| hpca100 | 2,3 (100%/99%,~33.5GB) | P44-BMR | MUSES rgbel(3모달) | ep76/200 | **79.91@ep76 (신기록, 직전 79.25)** | ~07-28 08:00 | ✅ |
| yeon | 0,1,5 | seed4 (P39.1-rank) | MUSES rgbel | **완주(ep300/300)** | **최종 best 81.92@ep198** | - | 종료(GPU 0,1,5 해방 확인, 0%/idle) |
| yeon | 6,7 (55-57%,~17.4GB) | seed3 (P39.1-rank) | MUSES rgbel | ep220/300 | 81.89@ep204 (변화없음, 미갱신) | ~07-28 10:30 | ✅ |
| yeon | 3,4 | (타유저 jongwon 계속, GPU2는 해방됨) | - | - | - | - | - |
| lecun | 4 | seed2(P39.1-rank) 분석 | MUSES | **완료**(8/8 stage ok, 10347s) | - | - | 종료(GPU4/5 모두 idle) |

**P44-BMR ep76**: mIoU 79.91, 에러/NaN/Traceback 없음 (앞서 확인된 cos_lora_lidar/mutual_kl/rel_corr 안정 패턴 지속 — 이번 tail엔 스칼라 라인 미출력, 손실 유한 확인).

**seed4 완주**: 07:29:20(전 사이클 이후) 300 epoch 전부 완료. `Best Val mIoU 81.92 (epoch 198) / Best Test mIoU N/A / Total Training Time 16:47:56`. GPU0,1,5 해방 확인(0%/idle 재확인).

**seed2 분석 — 완료, 6항목 raw 덤프**:

① **per_domain_analysis.md**: fog 62.56 / night 78.59 / rain 72.83 (cloud/sun 데이터없음 `—`), spread=16.03. 도메인-민감: sidewalk(±37,fog52) traffic light(±75,fog0) rider(±44,fog0) truck(±95) bus(±60) train(±98) motorcycle(±82) bicycle(±79).

② **module_ablation — toggle 라인**: `available=['p34_gate_off','p34_veto_off','p34_calib_off','p36_router_off','p39_query_off','p39_trunkexp_off'] skipped=['rbma_off','router_off','ctd_off','sdc_off','temp_off','cons_off','amf_uniform','p34_bias_off','p34_cons_off','p37_cefr_off','p38_m2f_off']`.
Δ(off)/feat_cos 전조건: p34_gate/veto/calib_off = 전조건 +0.00/1.0(불변). p36_router_off: clear +0.50~fog_night +4.50. **p39_query_off: clear +0.34, rain −0.26, night −0.37, clear_night −0.29, snow_night +1.15** (feat_cos 전조건 1.0). **p39_trunkexp_off: clear +6.52, fog +2.72, rain +5.33, snow +5.44, night +5.09, fog_night +2.05, snow_night +6.78, rain_night +5.21, fog_day +2.66** (feat_cos 0.877~0.897, no-op 아님 — 전조건 최대양수, off가 항상 mIoU를 올림).

③ **feature_stats — per-modal eff.rank 전조건**:
```
cond         img    lidar   event
clear        47.24  92.25   40.16
fog          38.31  87.51   37.84
rain         48.53  94.44   41.86
snow         47.30  93.86   40.86
day          42.39  94.50   39.34
night        49.64  100.32  41.68
fog_night    29.37  78.50   31.42
clear_night  46.07  89.92   39.37
snow_night   45.35  91.67   37.42
rain_night   42.71  89.40   37.67
fog_day      32.57  79.95   34.32
```
🔴 **lidar eff-rank 78.5~100.3** (VICReg on) — P43(VICReg off) 23.5~28.0 대비 **훨씬 높음**, dead ch 1/1024(양쪽 동일). cross-modal CKA(img~lidar/img~event/lidar~event): clear 0.5914/0.7668/0.6399 ~ night 0.5634/0.7699/0.6098 (전조건 lidar~event가 가장 낮은 0.61~0.67, img~event가 가장 높은 0.75~0.79). stage CKA FUSED_pf~PREHEAD = 0.636~0.710(P43는 정확히 1.0이었음 — 차이).

④ **module_diag dropMIoU [img,lidar,event]**:
```
clear        [27.03, 4.24, 0.15]
fog          [17.98, -0.26, -0.23]
rain         [25.12, 4.72, -1.57]
snow         [22.67, 4.01, 0.24]
day          [27.03, 4.24, 0.15]
night        [16.9, 3.35, 0.35]
fog_night    [9.06, 7.39, -1.39]
clear_night  [16.9, 3.35, 0.35]
snow_night   [26.79, 7.6, 0.97]
rain_night   [11.61, 7.57, 0.03]
fog_day      [17.98, -0.26, -0.23]
```
**drop-lidar: day 4.24 → fog_night 7.39, snow_night 7.6, rain_night 7.57**(P43는 day 0.64→fog_night 7.19로 유사 패턴).

⑤ **adapter_health**: n_sites=48, n_dead=0, dW_norm mean/median=14.19/13.30(min 5.51~max 36.40), mm_per_modality_dW_mean=`{img:13.965, lidar:15.0254, event:13.5874}`(P43: img11.30/lidar11.40/event9.53 — seed2 전반적으로 dW 더 큼).

⑥ **D3B lidar Δacc 전조건**: clear +0.3901, fog +0.5476, rain +0.4509, snow +0.4440, night +0.5123, fog_night +0.5481, snow_night +0.4776, rain_night +0.4452, fog_day +0.5495(P43 대응값: clear+0.3505~night+0.4114 — seed2가 전반적으로 더 큼).

🔴 jarvis: 27회 연속 드라이버 불일치, uptime 2026-07-10 15:08:01 그대로, 잡 0개.

## 2026-07-27 12:35 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep86/200 | 79.91@ep76 (직전대비 ep78~86 전부 하락, 최근 최저 76.45@ep84) | ~07-28 (변화없음) | ✅ |
| yeon 0,1,5 | **4-modal P39.1+radar** | MUSES 4모달(img/lidar/event/radar) | ep2 완료 후 **🔴 사망(CUDA OOM, 11:14:19)** | 45.07@ep2(마지막) | - | ❌ 사망 |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | 진행중(완주 아님) | - | - | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달(img/depth/event/lidar) | ep5(890/995) | **58.38@ep4(최초 eval 이후 신기록)** | ~07-30/31 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달(img/depth/event/lidar) | ep2 완료, eval 진행중(102/251) | **51.28@ep2(첫 eval)** | ~07-30/31 | ✅ |
| yeon 3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |
| hpca100 0,1 | (타유저 joonhui_been, gr00t_finetune, 신규 확인) | - | - | - | - | - |
| jarvis 나머지 | 없음(8장 전부 위 2런에 사용) | - | - | - | - | - |

**유휴 GPU**: yeon GPU5(16MiB/0%, 위 4-modal 사망분 GPU0,1도 idle: 237MiB/0%, 18MiB/0%) · hpca100/jarvis 유휴 없음(전부 점유).

🔴 **4-modal(P39.1+radar) 사망**: yeon 로그 확인 결과 epoch1 완주 → ep2 eval(mIoU 45.07, best) 기록 후 **`torch.OutOfMemoryError`**(rank0, GPU0, "Tried to allocate 76.00 MiB ... 23.18 GiB in use / 23.56 GiB capacity") 발생, `ChildFailedError`로 2026-07-27 11:14:19 프로세스 전원 종료. 이후 재기동 안 됨(GPU0,1,5 현재 idle). radar 로드 자체는 정상이었음(ep1 완주+ep2 eval 성공까지는 문제 없었음) — OOM은 순수 메모리 이슈로 보임(raw 관찰).

**jarvis DELIVER 2런 — 둘 다 OOM 없이 순항**: P39.1-rank ep4 val 58.38(ep2 50.78에서 상승), P44-BMR ep2 val 51.28(첫 eval). 둘 다 `OutOfMemoryError`/`Traceback` grep 0건.

⚠️ hpca100 P44-BMR: ep76 79.91 이후 5회 연속(ep78~86) 신기록 갱신 없음, ep84는 76.45로 하락폭 큼(직전 대비 -3.46) — 다음 사이클 추이 재확인 필요.

## 2026-07-27 14:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep98/200 | 79.91@ep76 (미갱신 지속, ep80~98 78.4~79.7 범위 오르내림) | ~23:10(오늘) | ✅ |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달(img/lidar/event/radar) | ep10/300 | **74.45@ep8(신기록 갱신 지속)** | ep30게이트 ~19:15(오늘) | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep234/300 | 81.89@ep204 (미갱신, 81.05→81.70→81.67 상승 추세이나 미달) | ~07-28 07:00 | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달(img/depth/event/lidar) | ep16/200 | 63.09@ep12 (직전대비 ep14 62.62/ep16 61.92로 소폭 하락) | ~07-28 19:30 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달(img/depth/event/lidar) | ep12/200 | **62.66@ep12(신기록)** | ~07-29 04:15 | ✅ |
| yeon 2,3,4 | (타유저 jongwon, PID 교체됐으나 계속 점유) | - | - | - | - | - |
| hpca100 0,1 | (타유저 joonhui_been gr00t_finetune, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 서버 전 GPU 점유 중, 우리 잡 5개 + 타유저 2개).

**P44-BMR(MUSES) 최근 eval 값들(dip 회복 판정용, ep80~98)**: 78.94, 78.54, **76.45(ep84, 최저)**, 78.62, 79.43, 78.68, 78.79, 78.38, 79.68, 79.42 — ep84 저점 이후 **78~79.7 밴드로 복귀했으나 ep76 best(79.91)는 아직 재돌파 못함**. Traceback/NaN 없음.

**4-modal(yeon) OOM/eval 상태**: ep6(69.67)·ep8(74.45)·ep10(74.26) 3회 eval 추가 통과, `OutOfMemoryError`/`Traceback` grep 0건 — **OOM 재발 없음**. GPU0,1,5 각 22365/22108/22106MiB(100%/100%/99%) — 이전과 동일하게 22GB대 유지, 여유 계속 타이트.

**seed3(yeon) 완주 여부**: 미완주(ep234/300).

**jarvis DELIVER 2런 best**: P39.1-rank 63.09@ep12(정체 조짐, ep14~16 하락) / P44-BMR 62.66@ep12(신기록, 직전 62.49@ep6 대비 상승). 둘 다 OOM/Traceback 0건.

이상징후 없음 추가 — 기존 관찰 지속(hpca100 P44 dip 회복 중이나 미돌파, 4-modal mem 타이트 지속 관찰).

## 2026-07-27 16:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep110/200 | **80.47@ep104 (신기록, ep76 79.91 재돌파)** | ~오늘 23시경 | ✅ |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달(img/lidar/event/radar) | ep18/300 | **77.64@ep18(신기록 지속 갱신)** — **ep30 미도달**(게이트 판정 보류) | ep30 ~18:30(오늘) 추정 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep242/300 | 81.89@ep204(미갱신, 80.91~81.70 오르내림) | ~07-28 06시경 | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep30/200 | **65.56@ep30(신기록)** | ~07-28 저녁 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep22/200 | **62.80@ep22(신기록)** | ~07-29 새벽 | ✅ |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |
| hpca100 0,1 | (타유저 joonhui_been, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 서버 전 GPU 점유).

🎉 **P44-BMR(MUSES) 재돌파**: ep104에서 **80.47**로 ep76 best(79.91) 재돌파 확인(ep106~110은 79.4~79.8로 소폭 내려왔으나 여전히 구 best 상회 수준).

**4-modal ep30 게이트**: **아직 미도달**(현재 ep18/300, 최근 4회 eval 전부 신기록 갱신 중: ep12 75.11→ep14 76.41→ep16 76.60→ep18 77.64). OOM 재발 없음(`OutOfMemoryError`/`Traceback` grep 0건). ep30 도달은 다음 사이클 이후로 예상.

**DELIVER 2런 비교(P39.1-rank vs P44-BMR)**: P39.1-rank가 **65.56@ep30**, P44-BMR이 **62.80@ep22** — 비슷한 진행 단계에서 **P39.1-rank가 앞섬**(단순 수치 비교, epoch 수 약간 다름 유의). 둘 다 OOM/Traceback 0건.

**seed3**: 미완주(ep242/300).

이상징후: 특이 anomaly 없음(전부 정상 진행 또는 신기록).

## 2026-07-27 18:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep122/200(미완주) | **80.55@ep120(신기록)** | ~오늘 23시경(변동없음) | ✅ |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep24/300 | **78.52@ep22(신기록) — ep30 미도달**(게이트 판정 보류, 근접) | ep30 ~19:30경 추정 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep248/300 | 81.89@ep204(미갱신) | ~07-28 새벽 | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep42/200 | **66.83@ep40(신기록, 67.74 fair 게이트 근접)** | ~07-28 저녁 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep32/200 | **64.28@ep30(신기록)** | ~07-29 새벽 | ✅ |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유 유지).

**P44-BMR(MUSES) 완주 여부**: **미완주**(ep122/200, 프로세스 20개 생존 확인). 최종 best 아님 — 현재 진행중 best **80.55@ep120**(직전 사이클 80.47@104 대비 갱신). GPU 미해방(2,3 계속 점유, 73%/66%대에서 변화 없음).

**4-modal ep30 게이트**: 아직 미도달(ep24/300). 최근 4회 eval 전부 상승 후 진동: ep18 77.64→ep20 76.22→ep22 **78.52(신기록)**→ep24 77.58. OOM 재발 없음(grep 0건). ep30까지 약 1회 eval(2ep) 남음 — 다음 사이클에 확정 가능.

**DELIVER 2런 비교**: P39.1-rank **66.83@ep40**(fair 게이트 67.74에 근접) vs P44-BMR **64.28@ep30** — P39.1-rank가 계속 앞섬. 둘 다 OOM/Traceback 0건.

**seed3**: 미완주(ep248/300, best 81.89@204 미갱신, 80.9~81.3 사이 정체).

이상징후: 특이 anomaly 없음(전부 신기록 또는 정상 진행, P44-MUSES/4-modal/DELIVER 전부 상승세).

## 2026-07-27 20:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep132/200(미완주) | **80.59@ep126(신기록)** | ~07-28 08시경 추정 | ✅ |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | **ep30 도달** | **78.82@ep30(신기록)** — 게이트 val≥80 **미달**(78.82<80) | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep254/300 | 81.89@ep204(미갱신, 80.9~81.4 정체) | ~07-28 새벽 | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep54/200 | 66.83@ep40(미갱신, fair게이트 67.74 **미돌파**) | ~07-28 저녁 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep42/200 | **65.12@ep38(신기록)** | ~07-29 새벽 | ✅ |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**4-modal ep30 게이트 값**: **val 78.82@ep30**(신기록, 직전 78.34@ep28). 사전등록 게이트 "val≥80 & 붕괴없음" 기준 **80 미달**(78.82). 붕괴 징후는 없음(연속 상승 후 근소 등락, OOM/Traceback 0건). 3모달 seed2의 ep30 시점 정확한 val 수치는 이번 스윕 범위에 없어 직접 비교는 보류(raw 미보유).

**jarvis P39.1-rank DELIVER 67.74 돌파 여부**: **아직 미돌파**. best는 66.83@ep40에서 정체(ep42~54 사이 65.4~66.8 범위에서 등락, 신기록 없음).

**P44-BMR(MUSES) 완주 여부**: **미완주**(ep132/200). GPU 미해방(hpca100 2,3 계속 점유). ⚠️ hpca100 로그 타임스탬프가 KST 대비 약 9시간 느리게 찍혀있음(UTC로 추정) — 실제 진행은 정상, 표기 시간대 차이로 보임(raw 관찰).

**seed3 완주 여부**: 미완주(ep254/300).

**DELIVER P44-BMR best**: 65.12@ep38(신기록, 직전 64.28@30 대비 상승).

이상징후: 없음(전부 정상 진행, 4-modal ep30 게이트 미달만 사실로 기록 — 판정 아님).

## 2026-07-27 22:35 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep144/200(미완주) | 80.59@ep126(미갱신, ep134~144는 79.7~80.5 범위) | ~07-28 08시경(변동없음) | ✅ |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep36/300 | **79.67@ep34(신기록, seed2 궤적 계속 추종)** | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep260/300 | 81.89@ep204(미갱신, 81.1~81.4 정체) | ~07-28 새벽(변동없음) | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep68/200 | 66.83@ep40(미갱신, **67.74 미돌파 지속** — ep42~68 14회 연속 미갱신) | ~07-28 저녁 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep52/200 | 65.12@ep38(미갱신, ep52 **59.75로 저점**) | ~07-29 새벽 | ✅ |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**P44-BMR(MUSES) 완주 여부**: **미완주**(ep144/200, 프로세스 20개 생존 확인). GPU 미해방(hpca100 2,3 계속 100%/33GB대 점유).

**jarvis P39.1-rank DELIVER 67.74 돌파 여부**: **미돌파**. best 66.83@ep40에서 **14회 연속(ep42~68) 미갱신** — 정체 지속.

**P39.1-rank vs P44-BMR(DELIVER) 격차**: 66.83(ep40) vs 65.12(ep38) = P39.1-rank가 **+1.71 우위** 유지. ⚠️ P44-BMR은 ep52에서 **59.75로 저점**(직전 최저 대비 하락폭 큼) — 이후 추이 확인 필요.

**seed3 완주 여부**: 미완주(ep260/300).

**4-modal 진행**: ep36/300, best 79.67@ep34(신기록). OOM/Traceback 0건, 계속 순항.

이상징후: ⚠️ jarvis P39.1-rank DELIVER 14회 연속 정체(67.74 게이트 미돌파 장기화), ⚠️ jarvis P44-BMR DELIVER ep52 저점(59.75). 그 외 사망/OOM 없음.

## 2026-07-28 00:36 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR | MUSES rgbel(3모달) | ep150/200에서 **🔴 사망(무기록 종료)** | 80.59@ep126(사망 시점 최종값, 완주 아님) | - | ❌ 사망 |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep42/300 | 79.67@ep34(미갱신, 79.13~79.52 근접 등락) | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep266/300 | 81.89@ep204(미갱신, 81.0~81.33 정체) | ~새벽~아침(변동없음) | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep80/200 | 66.83@ep40(미갱신, 20회 연속 정체 — **67.74 미돌파 지속**) | ~오늘 저녁 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep62/200 | 65.12@ep38(미갱신) — ep52 저점(59.75) 이후 62~65 범위 **오실레이션, 붕괴 아님(회복 확인)** | ~내일 새벽 | ✅ |

🔴 **P44-BMR(MUSES) hpca100 — 사망(완주 아님)**: 로그가 `20260727 14:23~14:24 UTC`(epoch150, iter251/375) 도중 **에러/Traceback/kill 메시지 없이 갑자기 중단**됨(파일 mtime 2026-07-27 14:23:33+00:00, 확인 시점 서버시간 15:36:24 UTC로 **약 73분간 무갱신**). `ps aux`에서 `train_reliadino`/`P44_bmr` 프로세스 **0개**(재확인 2회). **GPU 0,1,2,3 전부 30.7GB 사용 중(0% util)** — 그러나 이 메모리를 쥔 PID(2633100)는 `ps aux`에 전혀 나타나지 않음(우리 프로세스도, 기존 확인되던 joonhui_been 프로세스도 아님 — 다른 테넌트/파드로 추정, 확정 아님). **완주 아니며 최종 산출도 아님** — 사망 직전 최고값은 80.59@ep126(ep150까지 갱신 없었음). GPU 해방 여부: **미해방**(다른 프로세스가 점유 중이라 사실상 우리는 배제됨).

**4-modal ep42 진행**: OOM/Traceback 0건, 정상 진행 지속.

**jarvis 2런**: 둘 다 OOM/Traceback 0건. P39.1-rank는 여전히 67.74 미돌파(20회 연속 정체, ep42~80). P44-BMR은 ep52 저점(59.75) 이후 이번 사이클 확인 결과 62~65 범위로 오르내리며 **붕괴가 아닌 정상 변동으로 확인**(ep54 64.61~ep62 64.19).

**seed3**: 미완주(ep266/300).

⚠️/🔴 요약: 🔴 P44-BMR(MUSES) hpca100 사망(원인 불명, GPU 타 프로세스 점유로 확인 불가) — 최우선 확인 필요. 그 외 이상 없음.

## 2026-07-28 02:35 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR(재개) | MUSES rgbel(3모달) | ep156/200 | **80.71@ep156(신기록, ep126 80.59 재돌파)** | 완주 임박(수십 ep 남음) | ✅ (재-preempt 없음) |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep46/300 | 79.67@ep46(ep34 값과 동률, 신기록 아님) | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep272/300 | 81.89@ep204(미갱신, 81.0~81.3 정체) | ~06시경 완주 예상(변동없음) | ✅ (미완주) |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep94/200 | 66.83@ep40(미갱신, **27회 연속 정체 — 67.74 미돌파**) | 계속 진행 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep72/200 | 65.12@ep38(미갱신, 17회 연속 정체) | 계속 진행 | ✅ |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**hpca100 P44-BMR 재-preempt 여부**: **재-preempt 없음** — 19개 프로세스 생존 확인, GPU2,3 각 82%/79%(활성), 33.4~33.5GB, OOM/Traceback 0건. ep150 재개 이후 ep152·154·156 3회 eval 추가 통과, **ep156에서 80.71로 신기록**(구 최고 ep126 80.59 재돌파).

**seed3 완주 여부**: **미완주**(ep272/300). GPU 미해방(6,7 계속 점유).

**DELIVER 2런**: P39.1-rank는 **여전히 67.74 미돌파**(66.83@ep40에서 27회 연속 정체, ep42~94). P44-BMR도 65.12@ep38에서 17회 연속 정체(ep40~72). 둘 다 OOM/Traceback 0건.

**4-modal**: ep46/300, 79.67로 ep34값과 동률(신기록 아님이나 정체도 아님, 근접 유지). OOM 0건.

이상징후: 없음(하나는 재개 후 신기록, 나머지는 정상 진행 또는 장기 정체 — 사망/OOM/재-preempt 전무).

## 2026-07-28 04:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR(재개) | MUSES rgbel(3모달) | ep168/200(미완주) | 80.71@ep156(미갱신, ep158~168은 80.1~80.5 범위) | 완주 임박(32ep 남음) | ✅(재-preempt 없음) |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep52/300 | 79.67@ep46(미갱신, ep48~52 78.9~79.4 범위 — **정체 관찰됨**) | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep278/300(미완주) | 81.89@ep204(미갱신, 81.0~81.2 정체 지속) | ~05시 예상보다 지연(300ep까지 22ep 남음) | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep106/200 | **67.60@ep106(신기록! 33회 연속 정체 돌파, 단 67.74 게이트는 아직 −0.14 미달)** | 계속 진행 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep82/200 | 65.12@ep38(미갱신, 22회 연속 정체) | 계속 진행 | ✅ |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**seed3 완주 여부**: **미완주**(ep278/300, 22ep 남음 — 05시 완주 예상보다 지연). GPU6,7 미해방(계속 100%/96%대 점유). 4-modal 배치 결정은 아직 시기상조.

**4-modal ep48 이후 값**: ep48 79.39 / ep50 78.88 / ep52 79.43 — **79.67@ep46 이후 3회 연속 미갱신, radar 상한 근처 정체 관찰**(붕괴는 아님, OOM 0건). 계속 79대 초반~중반에서 등락.

**P44-MUSES(hpca100) 완주 여부**: 미완주(ep168/200). 재-preempt 없음(GPU2,3 100%/100%, 33.5GB/33.3GB, 프로세스 19개 생존).

**DELIVER 2런**: P39.1-rank는 **ep106에서 67.60으로 신기록**(33회 연속 정체를 드디어 갱신, 다만 fair 게이트 67.74에는 −0.14로 아직 미달). P44-BMR은 65.12@ep38에서 22회 연속 정체 지속(63.8~65.05 범위).

이상징후: ⚠️ 4-modal ep48~52 정체(radar 상한 의심), ⚠️ seed3 완주 지연(예상보다 늦음). 그 외 사망/OOM/재-preempt 없음. 신기록: jarvis P39.1-rank DELIVER(67.60@106) · hpca100 P44-BMR(누적 80.71@156, 이번 사이클엔 미갱신).

## 2026-07-28 06:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR(재개) | MUSES rgbel(3모달) | ep178/200(미완주) | 80.71@ep156(미갱신, ep164~178은 80.1~80.6 범위) | 22ep 남음(오늘 낮경) | ✅(재-preempt 없음) |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep58/300 | **80.39@ep54(신기록! 79.67 정체 돌파)** | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep284/300(미완주) | 81.89@ep204(미갱신, 81.0~81.4 정체 지속) | 16ep 남음 | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep118/200 | 67.60@ep106(미갱신, **67.74 게이트 여전히 −0.14 미달**) | 계속 진행 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep92/200 | **65.96@ep92(신기록, 65.12 정체 돌파)** | 계속 진행 | ✅ |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**seed3 완주 여부 + GPU6,7 idle 여부**: **미완주**(ep284/300, 16ep 남음). **GPU6,7 아직 idle 아님**(100%/55~96%대 점유 지속) — 4모달 배치 결정 트리거 미도달.

**jarvis P39.1-rank DELIVER 67.74 돌파 여부**: **아직 미돌파**. 67.60@ep106에서 정체(ep108~118 사이 65.99~66.94, 신기록 없음). 게이트까지 −0.14 남음.

**P44-MUSES(hpca100) 완주 여부**: 미완주(ep178/200, 22ep 남음). 재-preempt 없음(19프로세스 생존, GPU2,3 100%/100%).

**4-modal 최근값**: **80.39@ep54로 신기록** — 직전 사이클 79.67 정체를 돌파. ep56(80.21)·ep58(80.36)도 80대 유지, OOM 0건.

**DELIVER P44-BMR**: **65.96@ep92로 신기록**(직전 65.12 정체 돌파, ep90 65.23→ep92 65.96 연속 상승).

이상징후: 없음(4-modal·DELIVER P44 둘 다 오히려 정체 돌파, 사망/OOM/재-preempt 전무). ⚠️ 참고: seed3·P39.1-rank DELIVER는 여전히 정체 중.

## 2026-07-28 08:34 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR(재개) | MUSES rgbel(3모달) | ep190/200(미완주) | 80.71@ep156(미갱신, ep158~190은 79.9~80.6 범위) | **10ep 남음, 완주 임박** | ✅(재-preempt 없음) |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep64/300 | **80.45@ep60(신기록, 80 이상 유지 지속)** | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep290/300(미완주) | 81.89@ep204(미갱신) | **10ep 남음, GPU6,7 아직 미해방** | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep132/200 | 67.60@ep106(미갱신, **67.74 여전히 미돌파**, ep108~132는 64.8~66.6로 오히려 소폭 하락 추세) | 계속 진행 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep100/200 | 66.14@ep98(신기록, 단 ep100 **62.46로 저점** 발생) | 계속 진행 | ✅ |
| hpca100 0,1 | (타유저, 계속) | - | - | - | - | - |
| yeon 2,3,4 | (타유저 jongwon, 계속) | - | - | - | - | - |

**유휴 GPU**: 없음(전 GPU 점유).

**seed3 완주 + GPU6,7 idle 여부**: **미완주**(ep290/300, 10ep 남음). **GPU6,7 아직 idle 아님**(100%대 계속 점유) — 4모달 배치 트리거 미도달.

**P44-MUSES(hpca100) 완주·해방 여부**: **미완주**(ep190/200, 10ep 남음 — 완주 임박). GPU 미해방, 재-preempt 없음(19프로세스, GPU2,3 80%/100%).

**jarvis P39.1-rank DELIVER 67.74 돌파 여부**: **미돌파 지속**. best 67.60@ep106에서 26회째(ep108~132) 미갱신, 최근 구간(ep126~132: 65.33/64.77/65.49/65.22)은 오히려 하락 추세.

**4-modal 진행**: ep64/300, best 80.45@ep60 — 80 문턱 이후 3회 연속(ep60,62,64: 80.45/80.10/80.41) 80대 유지, OOM 0건.

**P44-DELIVER**: 66.14@ep98로 신기록 갱신 후 ep100에서 **62.46으로 저점**(직전 대비 −3.68) 발생 — 붕괴 여부는 다음 사이클 확인 필요.

이상징후: ⚠️ jarvis P39.1-rank DELIVER 하락 추세(67.74 게이트 요원), ⚠️ P44-DELIVER ep100 저점(62.46, 회복 여부 관찰 필요). 사망/OOM/재-preempt 없음.

## 2026-07-28 10:35 KST 정기점검

| 서버/GPU | 실험 | 데이터셋 | 진행 | best@ep | ETA(KST) | alive |
|---|---|---|---|---|---|---|
| hpca100 2,3 | P44-BMR(재개) | MUSES rgbel(3모달) | **완주(200/200)** | **최종 80.71@ep156** | - | 종료(GPU 0-3 전부 idle, 해방 확인) |
| yeon 0,1,5 | 4-modal P39.1+radar | MUSES 4모달 | ep72/300 | **80.89@ep70(신기록, 계속 상승)** | 계속 진행 | ✅ |
| yeon 6,7 | seed3 | MUSES rgbel(3모달) | ep296/300(미완주, 임박) | 81.89@ep204(미갱신) | ~4ep 남음(다음 eval 임박) | ✅ |
| jarvis 0-3 | P39.1-rank DELIVER | DELIVER 4모달 | ep144/200 | 67.60@ep106(미갱신, 19회 연속 정체) | 계속 진행 | ✅ |
| jarvis 4-7 | P44-BMR DELIVER | DELIVER 4모달 | ep110/200 | 66.14@ep98(미갱신, ep100저점 62.46 이후 65.0~65.7로 회복했으나 66.14 재돌파는 아직) | 계속 진행 | ✅ |

**유휴 GPU**: hpca100 GPU0-3 전부 idle(P44-BMR MUSES 완주로 해방) — 그 외는 전부 점유.

🎉 **P44-BMR(MUSES) 완주**: `Best Val mIoU 80.71 (epoch 156) / Best Test mIoU N/A / Total Training Time 08:54:55`. GPU0-3 전부 0%/0MiB로 확인 — **완전 해방**.

🔴 **중요 — seed3 watch-task(b762obhdh) 중단 확인**: 지시받은 자동감시/자동기동 백그라운드 작업이 **중간에 killed 상태로 종료됨**(정상완료 아님) — seed3 완주 시 yeon 6,7에 P44-DELIVER를 자동 기동하는 절차가 **수행되지 못함**. 확인 결과 seed3는 아직 **미완주**(ep296/300)이고 `tmux jemo` 세션에 `deliver_p44_yeon` 창도 **존재하지 않음**(자동기동 안 됨, 예정대로도 아직 이름). 조치: 새 워치 태스크를 재기동함(아래).

**seed3 완주 여부**: 미완주(ep296/300, 4ep 남음 — 다음 eval에서 완주 가능성 높음).

**4-modal**: ep72/300, best 80.89@ep70(신기록, ep66→68→70 연속 상승: 80.51→80.64→80.89). OOM 0건.
