...
| **P34-ReliaDINO (카드 A)** | ✅ **구현 완료 (feasibility probe 확정 시 학습 착수)** — 2026-07-12, branch `worktree-p33-impl` | DINOv3-RBMA 스켈레톤: `semseg/models/reliadino/`(FrozenViTEncoder=timm DINOv3 ViT-L/16 frozen+per-modality LoRA(qkv Q/V, r8)+SimpleFPN / ReliabilityGatedFusion=cross-modal memory-style attn+RBMA-v2 bias(λ₁B_cal+λ₂B_cons)+competence gate(rel_cal softmax τ0.25)+corr_veto training-free floor / conv seg head, **SAM2 import 0**) + `train_reliadino.py`(OHEM+DDP+2ep eval+top-5 ckpt 동일 포맷) + configs `{b200,hinton}-deliver_rgbdel_P34_reliadino.yaml`. CPU smoke PASS(양 ViT 계열+patch14 폴백; LoRA grad 4모달 확인, frozen 무grad). **HF timm dinov3 가중치 un-gated 확인**(폴백 dinov2 불필요할 전망). 설계 = `research_vault/architecture/P34_ReliaDINO_design_20260712.md`. **B200 launch-ready (07-12 pre-staging)**: ① 코드 3종 B200 배포+md5 검증 ② env timm 0.4.12 문제 우회 = 격리 라이브러리 `~jemo_maeng/pylibs_p34`(timm 1.0.24+hf_hub 1.4.1, PYTHONPATH 주입 — 공용 env 무변경) + DINOv3 ViT-L 실가중치 1.2GB hinton HF캐시→B200 `~jemo_maeng/hf_cache` 이송 ③ **B200 real-weights CPU smoke PASS**(dinov3 로드 확인·폴백 경고 0, logits (1,25,512,512), seg/cal/auxCE loss 유한, trainable 46.8M/349.9M=13.4%) ④ 원커맨드 런처 `~jemo_maeng/launch_p34.sh`(P33.2 tmux C-c 정지→GPU2-5 검증→jemo:p34 torchrun, `--dry-run` 검증 완료) ⑤ 폴백(M0-a) 준비 = `tools/eval_public_cmnext.py`(plain CMNeXt per-condition test eval, py_compile+strict-load 검증) + 공식 CMNeXt-B2 rgbdel ckpt(mIoU 66.30, md5 e3ba84b5) 로컬+B200 `~jemo_maeng/ckpts/` 확보 |

**⚡ 2026-07-16 추가 — P37a (CEFR-Head + CA2-NoText anchor)**

| 트랙 | 상태 | 내용 |
|------|------|------|
| **P37a (CEFR)** | ✅ **구현 완료 (학습 대기)** — branch `worktree-p33-impl`, 미커밋 | P36 위에 config-gated 2-pass CEFR: pass-1(P36 경로, no_grad) logits → q=softmax detach(s16 pool) → **POST-attention fused_tokens** 위 per-modality zero-init routing head + CA2 anchor `A_{i,k}=λ1·rel_cal_i+λ2(t)·log p̂_i(k)`(p̂=calibrated aux posterior detach, λ2 linear warmup 0→0.5 @10ep via `_current_epoch`) → class-expected collapse `w̄_i=Σ_k q_k·w_{i,k}` → `fused'=Σ w̄_i·fused_i` → feature blend `(1−σ(a))·gate_fused+σ(a)·fused'`(a init −4, σ≈0.018) → 공유 FPN+head 2회째 pass(+P36 router residual 유지). Anti-collapse: zero-init·detached q·decisive reg·**AECF hinge floor(batch-marginal entropy)**. `MODEL.CEFR.ENABLE=false` → P36과 byte-identical(smoke 검증: max\|Δ\|=0). 변경: `semseg/models/reliadino/{fusion,model}.py`(CEFRHead, `_decode` 2-pass), `train_reliadino.py`(cefr_reg loss 항+`p37/cefr_w_*`·`p37/cefr_sigma_a` 로깅). configs: `bengio-deliver_rgbdel_P37a_cefr.yaml`(768² BS1 grad-ckpt, 공정레시피+DGFUSION_AUG+LORA_NORM_CAP 20.0, EPOCHS 200), `yeon-smoke_P37a_cefr.yaml`(EPOCHS 2, ROOT placeholder). CPU smoke 5종 전부 PASS(@init max\|Δ\|=0.00139, grads/warmup/hinge/step-변화 확인) |
