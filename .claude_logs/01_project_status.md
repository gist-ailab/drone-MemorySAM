...
| **P30-Det 객체검출 (yeon)** | 🟡 **구현 완료, yeon 학습 준비**(2026-06-30) | P29-Det(P28+mean+FCOS) → **P30 백본(RBMA+SDC) 기반 확장**: ① reliability-anchored router 모달 융합(mean 대체) ② Object-Query decoder가 융합 memory feature에 cross-attend(primary, Hungarian set loss) + FCOS aux. `LoRA_Sam_P30_Det`/`MemorySAMDetectorP30`/`query_decoder.py`. **CPU smoke PASS**(loss·grad·eval·state_dict). 브랜치 `worktree-p30-det`. lecun 만석 → **yeon push/pull 후 학습**. config `det/det_P30_indoor`. 상세 [02_model_arch.md](02_model_arch.md) P30-Det |
