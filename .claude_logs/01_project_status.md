...
| **P30-Det 객체검출 (yeon)** | 🟢 **학습 중**(2026-06-30 15:23 시작, yeon GPU 0,5 nproc=2) | P29-Det(P28+mean+FCOS) → **P30 백본(RBMA+SDC) 기반 확장**: ① reliability-anchored router 모달 융합(mean 대체) ② Object-Query decoder가 융합 memory feature에 cross-attend(primary, Hungarian set loss) + FCOS aux. `LoRA_Sam_P30_Det`/`MemorySAMDetectorP30`/`query_decoder.py`. CPU smoke PASS. **ep0 진행 정상**(loss~9, n_pos>0, ~1.35s/it→~72min/ep, ~19GB/GPU). Total 118.2M/Trainable 49.1M. 데이터셋 /drone_nas 미마운트 → 로컬→yeon `/SSDb/jemo_maeng/dset/poongsan` rsync(7.6G/43484png, train 6408장). lecun 만석으로 yeon 사용(가용 0,5 2장). NCCL 2h timeout 픽스 포함. config `det/det_P30_indoor_yeon`, wandb `p30-det`(offline), tmux `jemo:p30det`, log `logs/det_P30_indoor_yeon/run_20260630_152342.log`. 브랜치 `worktree-p30-det`. 상세 [02_model_arch.md](02_model_arch.md) P30-Det |
