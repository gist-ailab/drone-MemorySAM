...
| 트랙 | 상태 | 최신 수치 / 다음 액션 |
|------|------|----------------------|
| **SAM2 RBMA seg (P30, B200 DELIVER)** | 🟢 **학습 중**(신규, ~11:20 시작, nproc=4) | ep4(2026-06-30 12:48) · Day-Val 13.76 / Test 13.77, fresh ramp-up 정상. config `b200-deliver_rgbdel_P30_physaug`(P29 대비 diff 미확인). EPOCHS=200, ~18.5min/ep→~60h. 모니터 RUN-4 |
...
| **P29-Det 객체검출 (Jarvis)** | 🔴 **사망**(2026-06-30 03:09:30, NCCL collective timeout→전 rank SIGABRT) | 01:50 시작 후 ~1h19m 만에 사망, **체크포인트 0개=진척 전부 소실**. 1순위 의심=`find_unused_parameters` DDP rank desync. config `det/det_P29_indoor_jarvis`, wandb `p29-det`. **재시작/수정 사용자 지시 대기.** 상세: [15_training_monitor_log.md](15_training_monitor_log.md) RUN-3 |
| **P30-Det 객체검출** | ⏸ **구현 완료, 학습 보류(TODO)**(2026-06-30 yeon GPU 양보로 ep0 중 중단) | P29-Det(P28+mean+FCOS) → **P30 백본(RBMA+SDC) 기반 확장**: ① reliability-anchored router 모달 융합(mean 대체) ② Object-Query decoder가 융합 memory feature에 cross-attend(primary, Hungarian set loss) + FCOS aux. `LoRA_Sam_P30_Det`/`MemorySAMDetectorP30`/`query_decoder.py`. **CPU smoke PASS** + **yeon e2e ep0 진행 검증됨**(loss~9, n_pos>0, ~71min/ep→**총 ~60h(2.5일)**, ~19GB/GPU, Total 118.2M/Trainable 49.1M, crash 0). 데이터셋 /drone_nas 미마운트 → **yeon `/SSDb/jemo_maeng/dset/poongsan` 에 rsync 완료**(7.6G/43484png, train 6408장 유지) — 재실행 시 재전송 불필요. 브랜치 `worktree-p30-det`(origin push), config `det/det_P30_indoor_yeon`. **재개 방법**: 빈 GPU 확인 후 `CUDA_VISIBLE_DEVICES=<free> PROTOCOL_BUFFERS_PYTHON_IMPLEMENTATION=python torchrun --nproc_per_node=N train_det.py --cfg configs/det/det_P30_indoor_yeon.yaml`. **로깅 주의**: yeon에 wandb 설치됨(0.28.0)이나 미로그인 → 온라인 대시보드 보려면 `wandb login` 필요(아니면 tensorboard `outputs/det/det_P30_indoor_yeon/tb_logs` 사용). 상세 [02_model_arch.md](02_model_arch.md) P30-Det |
