...
**한 일**: 재추론 없이 두 모델의 제출 마스크(`eval_macvi`, 345장)와 서버 CSV(`frames_*.csv`)만으로 렌더링하는 `tools/render_multiaqua_compare_video.py`를 추가했다(labcode 작성 → 미리보기·코드 검토 → 2차 수정). Thermal은 모델 입력용 `thermal_processed`가 유효 영역에서 8단계 정도의 값밖에 없어서, 원본 uint16 `thermal_camera`를 퍼센타일 스트레칭한 뒤 CLAHE와 INFERNO 컬러맵을 적용하고 센서 시야로 잘라 확대했다. test는 GT가 로컬에 없으므로 GT 칸을 "서버 비공개"로 표시했다. 같은 날 user 요청으로 화면을 다듬었다: 부제를 "MaCVi · MULTIAQUA Challenge — 1st Prize"로 바꾸고, Baseline 라벨에서 "P8 (no night-aug)"를 뺐으며, 인트로에 저자 줄(Jemo Maeng, Sangmin Park, Seongju Lee, Kyoobin Lee†)을 넣고 아웃트로를 없앴다. 또한 val GT에서 라벨이 1~4가 아닌 영역(촬영 보트 등, 챌린지 IoU 계산 제외)을 세 칸 모두 검게 칠해(`--mask_ignore`), 채점되지 않는 곳의 예측이 오답처럼 보이지 않게 했다. test는 GT가 없고 같은 시퀀스(lj4)의 학습 프레임도 없어 이 처리를 적용하지 못한다. 최종본은 1920×1080, 30fps, 3분 23초(한 장당 1.6초)이며, 발행 위치는 `/ailab_mat2/personal/jemo_maeng/src/Project/Drone/drone-memorysam/videos/inference_multiaqua-challenge_gatedmemorysam-vs-p8noaug/`(mp4·사이드카 json·선별 프레임 csv)이다.
...
**확인한 것**: 실행 중인 학습 프로세스의 열린 파일을 직접 조회해 **SSDb의 `outputs/`를 쓰고 있는 학습이 없음**을 확인했다(저장 경로는 전부 `/tmp/jemo_scratch/outputs/`). 따라서 SSDb 쪽 산출물은 학습에 영향을 주지 않고 옮길 수 있다.

**한 일**:
- run 디렉터리 15건(실험 10건 + 스테이징·import 폴더)을 `/drone_nas/.../ckpts/hpca100_archive_20260909/`로 run 단위 tar 총 **158G**를 이관했다. 서버 디스크를 쓰지 않도록 스트리밍 tar(`ssh … "tar cf -" > NAS/<이름>.tar`)를 썼고, 학습을 방해하지 않도록 서버 쪽 우선순위를 낮췄다. **결과: SSDb 여유 121G → 287G(사용률 95% → 87%)**, 레포 디렉터리 123G → 880M.
- 원본 삭제 전 3단계 검증을 모두 통과시켰다 — ① 크기 대조 ② `tar tf`로 끝까지 읽어 구조 확인 + 항목 수를 서버 파일 개수와 대조(14건 일치) ③ 각 tar에서 체크포인트를 실제로 꺼내 md5 대조(14건 일치).
- 스테이징·import 폴더에 섞여 있던 P38 ep156·P39 ep146·4모달 brokenradar ep122와 그 사본 5개가 NAS 정본과 **md5까지 동일**함을 확인했다(`6ba4873c…`·`8d382554…`·`d2a4c493…`).
- 산출물 위치 지도 [infra/artifact-locations.md](../infra/artifact-locations.md)를 신설했다 — 실험별 현재 위치, 서버에 남긴 것과 그 이유(활성 학습 경로·데이터셋·HF 백본 캐시·venv), 복원 명령, 기존 아카이브 색인. 이관 대상 9개 실험 행에는 `experiments/registry.md`에도 표기를 붙였고, NAS 아카이브 폴더에 같은 내용의 README를 두었다.

**부수 확인**: 정리 과정에서 **일일 카드 E1M(카드 E1의 중간층 4탭 읽기를 MUSES로 이식)이 2026-09-08 22:44에 40 epoch을 완주**한 것을 확인했다 — 트레이너 val-best **80.64@ep35**(최종 ep40 80.49), 총 학습 6시간 15분. 산출물이 휘발성 `/tmp`에 있어 아카이브로 회수했다(`…_E1M_COMPLETED_tmp.tar`). 공식 native 채점과 판정 기록은 아직 하지 않았다. 이 완주로 hpca100 GPU 0·3이 비었다.

**사고 기록**: 전송을 `nohup`으로 분리해 띄웠더니 세션이 그 작업을 완료로 처리한 뒤에도 프로세스가 살아 있었는데 `ps`에는 잡히지 않았다. 죽은 줄 알고 통합 스크립트를 새로 띄운 탓에 **여러 프로세스가 같은 출력 파일에 겹쳐 쓰는 상황**이 벌어졌고, 로그에 같은 run의 완료 기록이 두 번씩 찍히고 중간에 잘린 파일(15.3G)이 나타났다. 크기만 대조했다면 정상으로 보였을 것이다. 잔존 프로세스를 로컬·원격 양쪽에서 정리한 뒤 내용 기반 검증(위 ②③)을 추가해 통과시켰다. **장시간 전송은 `nohup`으로 분리하지 말고 세션이 추적하는 방식으로 띄울 것. `ps`에 안 보인다고 죽은 것으로 단정하지 말 것.**
