...
**한 일**:
- run 디렉터리 14건(실험 9건 + 스테이징·import 폴더)을 `/drone_nas/.../ckpts/hpca100_archive_20260909/`로 run 단위 tar 총 **147G**를 이관했다. 서버 디스크를 쓰지 않도록 스트리밍 tar(`ssh … "tar cf -" > NAS/<이름>.tar`)를 썼고, 학습을 방해하지 않도록 서버 쪽 우선순위를 낮췄다. **결과: SSDb 여유 121G → 287G(사용률 95% → 87%)**, 레포 디렉터리 123G → 880M.
