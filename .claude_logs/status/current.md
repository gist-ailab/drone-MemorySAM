...
- **DELIVER 3시점([analysis/2026-07-20-p39-deliver-3ckpt-compare.md](../experiments/analysis/2026-07-20-p39-deliver-3ckpt-compare.md))**: val **65.68@ep64로 P38 피크(65.19) 첫 돌파**했으나 **test 5-cond 평균은 최저 50.98**(ep60 51.96 > ep38 51.65 > ep64) — **val↔test 순위 역전**. 손실의 대부분이 **RailTrack 단독 −20.4**(cloud 59.2→6.4, 전 조건 진행형). 원인 = query·router가 동일 클래스를 상충 점유(ep38 night: query_off RailTrack −25.4, router_off −24.4) + gate/calib이 thin-class에 **유해**(ep60 night off 시 +35.9/+26.0 — 3세대 no-op에서 유해로 판정 변경). thin-class 게이트 **세 시점 모두 0/3 미달**.
- **MUSES 공식 test**: P39-DPC ep146 = **78.881**(P38-m2f 79.025 −0.144, P34-3modal 78.979 −0.098) → 미돌파. 단 **주야 격차 5.14→3.73(−1.41)**로 개선, clear_night +3.69인데 **fog_night 62.68로 −12.05 붕괴**(전 제출 최저)가 전체를 상쇄. fog_night 정밀 대조분석(P39 ep146 vs P38 ep156, 조합 CASE 지원 커밋 dee524f) 진행 중.
