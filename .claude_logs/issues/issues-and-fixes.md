...
| ID | 상태 | 한 줄 |
|----|------|-------|
| **ISSUE-038** | ✅ **CLOSED (2026-09-23)** — 감시 세션 대조: 수정 3파일은 develop 과 같거나 develop 이 상위 집합, 미추적 46개 중 develop 에 없던 6개(평가 config 3·스크립트 3)를 develop 에 커밋(스크립트 2개는 scripts/hpca100/ 로 이동). hpca100 체크아웃은 develop 으로 갱신해도 코드 손실 없음(단 실행 중 학습이 있으면 pull 금지 규칙은 유지) · 원문: 🔴 OPEN (2026-09-22 등록) | **hpca100 repo의 R1/R2·QAF 구현이 미커밋 로컬 수정으로만 존재 — pull 하면 날아간다.** hpca100 `~/SSDb/jemo_maeng/src/drone-MemorySAM` 의 HEAD 는 오래된 cddc319(09-08 경)인데 working tree 에 fusion.py·model.py·train_reliadino.py **+566줄 미커밋 수정**(BOUNDARY_REFINE·COMPONENT 손실·QAF 품질 헤드/두 패스)이 있고, boundary_refine.py·component_loss.py·quality_head.py 가 추적 안 되는 파일로 추가돼 있다. 이 상태가 "user가 죽인 R1/R2 세션을 그대로 재기동"한 실체(09-22 17:40 재개, 정상 가동 중, tmux `r1_s821`/`r2_s821`). **처방**: develop 의 해당 커밋(7714f81~0bceb2c P54-QAF 계열)과 내용 대조 후 hpca100 로컬 수정을 develop 에 반영·커밋하거나, develop 최신으로 갈아탄 뒤 재검증. 그 전까지 **hpca100 repo 에서 pull/checkout 금지**. 경과 = [status/history-2026H2.md](../status/history-2026H2.md) 2026-09-22 엔트리 |
| **ISSUE-036** | 🟡 **OPEN → 조치 중 (2026-09-18 12:10 판정: legal v2(nearest-exact) 채택. P46 본 런 test v1 55.18→v2 56.39(+1.21), E1 확정 시드1 54.63→55.97(+1.34), 하락 클래스 0, 얇은 4클래스 +2.7~2.9. 잔여: 확정 런 5건·val 재채점, v2 래퍼 가드 등재, 헤드라인 표 v2 전환)** | **legal 하네스의 native 복원 업샘플이 floor 정렬이라 우리 수치만 약 −1.3 낮게 잰다.** `val.py:_unpad_resize_to_orig`가 1024 예측을 native 1042로 올릴 때 `F.interpolate(mode="nearest")`(floor 정렬)를 써 반 픽셀 계통 편차가 생긴다. 실측(DGFusion 80k test RLE 317장, 우리 GT 덤프 대조): nearest 52.34 / nearest-exact 53.68 / PIL NEAREST 53.68, 얇은 객체 4클래스 50.25 vs 53.38(Pole 51.3 vs 57.1). 기준선 변환(`tools/baseline_failure/common.py:resize_nearest` = PIL 중심 정렬)은 편차가 없어 **기준선 대비 비교가 우리에게 불리하게 기울어 있었다.** 이것이 같은 ckpt(P46 C3-only 본 런 ep70)가 학습기 복제 1024 격자 경로 56.99 / legal 55.18로 갈린 원인이다(격자 되돌림 재채점 55.18 불변 = 격자 아닌 정렬 문제). **영향**: legal native 수치 전부(확정 3쌍·5시드 평균·N6·실패 분석 D2/D3의 우리 모델 값)가 약 −1.3(얇은 클래스 −3~−6) 편향, 카드 간 상대 판정은 무영향. **처방**: `tools/legal_rescore_v2.py`(val.py 무수정 몽키패치, nearest-exact)로 P46 본 런·E1 확정 시드1 재채점 → 하네스 v2 채택·전수 재채점 판정(카드 §5-34 (2-정정)). 그 전까지 헤드라인 수치 변경 금지. 이미지별 분석 정본 격자는 1024로 통일. |
| **ISSUE-037** | 🔴 **OPEN (2026-09-20 발견)** | **config 의 모듈 토글이 코드에 없으면 조용히 무시된다.** E17 확정 200ep 1차 기동(jarvis, 체크아웃 b99ce18 = 09-09)에서 `MODEL.DETAIL_BRANCH.ENABLE: true` 가 있어도 `semseg/models/reliadino/detail_branch.py` 부재로 세부 가지 없이(total_trainable 58,790,687 = E1 base) 8분 학습 후 폐기. 기동 검증 기준에 **파라미터 수 + 모듈 로그 줄** 추가. 코드 조치(대기): `train_reliadino.py`/`model.py` 에서 소비되지 않은 config 키·ENABLE 토글에 대해 assert. 서버 체크아웃은 develop 최신 필수(코드 단일출처 규칙 1.7) | 감시 세션 2026-09-20 |
| **ISSUE-035** | 🟡 **OPEN (2026-09-17 발견 · 가중치는 생존)** | **DELIVER 헤드라인 56.99 런의 체크포인트 경로가 어디에도 기록되지 않았고, 원본 디렉터리는 정리돼 임시 사본만 남아 있었다.** 런 = `jarvis_deliver_rgbdel_P46_ctr_c3only`(C3 λ0.1) val-best ep70. jarvis `outputs/` 는 정리돼 이 런이 없고, 보관 루트(NAS ckpts)·가중치 보관소(/ailab_mat2)·hpca100·bengio 에도 없어 한때 소실로 판단했다. **실제로는 yeon 에 같은 파일 두 벌이 있다** — `/SSDe/jemo_maeng/temp_ckpts/epoch70_67.79_top1_checkpoint.pth` 와 `drone-MemorySAM-p49/_eval_ckpts/p46_c3only_lam01_ep70/` (1,865,010,172 바이트, 2026-07-30, md5 d340e3fe9824bd922fc7fe6eff7a8b26). 근거: 옆 `eval_logs/job1_p46c3only_eval1024.log` 원문 "P46 C3-only 본run epoch70(val 67.79) — CMNeXt-protocol 1024-resized-GT TEST mIoU = 56.99". **위험**: 두 사본 모두 임시 성격의 경로라 정리 과정에서 지워질 수 있다. **처방**: ① 보관 루트(NAS ckpts)에 md5 대조로 정본 이관 ② registry 행에 체크포인트 경로 기록 ③ 헤드라인급 런은 산출물 위치 문서에 경로를 남기는 것을 규칙화. 교훈: "소실"로 단정하기 전에 평가용 사본(`temp_ckpts`·`_eval_ckpts`)과 파일명 패턴(학습 해상도의 트레이너 val 이 이름에 들어감 — 1024 재평가 값과 다름)까지 검색할 것 | **재발 2건째(2026-09-18 19:40)**: E1·E13 확정 게이트의 시드821 분모(C3-only 200ep, epoch90_67.3_top1)도 카드 §0-1 기록 경로에 NAS 보존본이 없었다 — bengio·yeon 스테이징 사본(md5 854a2c9d…)에서 회수해 `ckpts/p46_c3only_seed20260821_200ep_20260918/` 로 정본화. 규칙: 게이트 분모 ckpt 는 판정 전에 NAS md5 확인 필수.
