...
| # | 태스크 | 우선순위 | 상태 |
|---|--------|---------|------|
| R1 | **`.wandb_key` 시크릿 rotate + git history purge** (origin에 이미 커밋됨 — 보안) | 🔴 | ☐ |
| R2 | doc 19가 경고한 stale 경로 정리: repo 루트 `objdet/`·`configs/det/det_P9_base.yaml`(구버전 stride) — worktree(p30-det/p29-det) 최신본과의 관계 명시 or 아카이브 | 🟠 | ☐ |
| R3 | 리팩토링 후속: 각 서버(B200/jarvis)에서 develop pull 시 학습 스크립트 무결성 확인 (진행 중 학습은 pull 금지) | 🟠 | ☐ |
