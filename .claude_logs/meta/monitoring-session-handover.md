...
- 🔴 **감시 생존 확인을 `TaskList` 로 하지 마라.** 감시는 `Monitor` 로 걸리고 `TaskList` 는 별개의 작업
  ...
  **판별법**: `ps -eo pid,args --no-headers | wc -l` 이 수십 개에 그치면 격리된 것이다(정상 워크스테이션은
  ...
  ✅ **대체 확인법 — Monitor 출력 파일의 갱신 시각으로 센다.** 감시는 `Monitor` 로 걸릴 때마다
  `/tmp/claude-1000/<프로젝트슬러그>/<세션UUID>/tasks/<task-id>.output` 에 출력을 append 하므로,
  그 파일의 mtime 이 곧 감시의 마지막 활동 시각이다. 프로세스를 못 봐도 파일은 보인다.

```bash
D=/tmp/claude-1000/<프로젝트슬러그>/<세션UUID>/tasks
date '+현재 %H:%M:%S'
ls -lt --time-style=+%m-%d_%H:%M "$D"/*.output | head -20   # 심링크(.output -> subagents/*.jsonl)는 서브에이전트 전사본이니 제외
for f in "$D"/*.output; do [ -L "$f" ] && continue
  echo "=== $(basename $f .output) ($(stat -c %y $f | cut -c12-16)) ==="; head -1 "$f" | cut -c1-90; done
```

  판정: 각 감시의 첫 줄에 `[<대상이름>] WATCH_START` 가 찍히므로 **어떤 대상이 감시되고 있는지 첫 줄로**
  알 수 있고, mtime 이 그 감시의 폴링 주기(최대 30분) 안이면 살아 있는 것이다. 주기를 크게 넘겼는데
  `[exited with code 0]` 같은 종료 표시도 없으면 죽은 것이다. 🔴 방금 실행한 `Bash` 호출도 같은 폴더에
  `.output` 을 남겼다가 지우므로, **막 생겼다 사라지는 파일은 감시가 아니다**(2026-09-11 에 이 둘을
  감시로 착각해 `cat` 했다가 `No such file` 을 받았다).

  🔴 **`persistent: true` 없이 건 감시는 한꺼번에 만료된다(2026-09-11 실사고, 일곱 건 동시 사망).**
  `timeout_ms` 로만 건 감시들이 21:26~21:48 사이에 줄줄이 끊겼다 — E14·E1확정·arm A·CAFuser·
  E13확정·E13확정s2·E13s3. 같은 시각에 걸린 것들이 같은 시각에 만료되므로 **한 건이 죽었으면 나머지도
  의심하라.** 살아남은 것은 `persistent: true` 로 건 넷(arm B·arm C·P52 두 시드·E13M)뿐이었다.
  **완주까지 몇 시간 이상 걸리는 런의 감시는 예외 없이 `persistent: true` 로 걸어라.**
