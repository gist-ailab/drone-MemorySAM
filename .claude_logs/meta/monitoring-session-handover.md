...
### 1-1. 감시 대상과 임계 (2026-09-10 04:00 KST 실측 — 열둘)

> 🔴 **09-09 22:00 판을 대체한다.** 밤사이 바뀐 것: E13 완주(→ legal 재채점 + **E1 확정 런 200ep** 기동),
> **E3s2 가 CUDA OOM 으로 죽고 hpca100 GPU0 을 타 사용자에게 뺏김**.

`CYCLES` 는 **그 런의 실측 `[Val]` 갱신 간격 × 2.5 ÷ 폴링**으로 정한다. 🔴 **간격은 학습 속도가 아니라
`[Val]` 줄 두 개의 타임스탬프 차이로 재라** — 평가 시간을 빼먹으면 틀린다(E3s2 에서 두 번 틀렸다).

| 항목 | 서버 | tmux 세션 | 실측 ep 시간 | `[Val]` 간격 | 폴링 | `CYCLES`(현재 설정) |
|---|---|---|---|---|---|---|
| 1-a | `yeon` | `p52_deliver_s1`, `p52_deliver_s2` | 39.5 / 39.0분 | 2ep ≈ 78분 | 1800초 | 7 |
| 1-b | `yeon` | `elora_a_r16` (arm A) | 40.0분 | 2ep ≈ 80분 | 1500초 | 8 |
| 1-c | `yeon` | `elora_c_shres` (arm C) | 41.0분 | 2ep ≈ 82분 | 1500초 | 8 |
| 1-d | `jarvis` | `elora_b_shared` (arm B) | 19.0분 | 2ep ≈ 38분 | 1500초 | 4 |
| 1-e | `jarvis` | **`e1_confirm200`** (E1 확정 런 200ep) | 19.0분 | 2ep ≈ 38분 | 1500초 | 6 |
| 1-f | `jarvis` | `e4b_resume` | 30.0분 | 5ep ≈ 150분 | 1500초 | 20 |
| 1-g | `jarvis` | `e1s2_resume` | 29.6분 | 5ep ≈ 148분 | 1500초 | 20 |
| 1-h | `jarvis` | **`e13_legal`** (재채점, 완료 시 종료) | — | — | 600초 | — |
| 1-i | `hpca100` | `hpca100_E12` | 58.4분 | 5ep ≈ 292분 | 1500초 | 31 |
| 1-j | `hpca100` | `hpca100_E2s2` | 56.6분 | 5ep ≈ 283분 | 1500초 | 30 |
...
로그 경로: yeon `<yeon-p38>/logs/<세션>_launch.log` · jarvis `/SSDb/jemo_maeng/src/drone-MemorySAM/logs/<세션>_launch.log`
...
현재 설정된 `CYCLES` 는 모두 실측 간격의 3배 이상이라 오탐 여지가 없다(계산상 필요한 최소치보다 여유 있게 잡혀 있다).

⚠️ **hpca100 은 공유 GPU 다.** E3s2 가 평가 구간에서 38.2GB(A100 40GB 의 93%)를 쓰다가, 타 사용자가
34GB 를 잡자 20MiB 도 할당 못 해 OOM 으로 죽었다. 여유를 1~2GB 만 남기고 돌리면 남이 들어오는 순간
잃는다. hpca100 에 새로 올릴 때는 평가 구간 peak 를 낮춰라(학습 중 test 평가 끄기 등).

**완주 임박 순서(09-10)**: e13_legal 04:45 → E4b 11:15 → E2s2 13:35 → E12 13:40 → E1s2 13:40 → B0s2 15:05.
그 뒤 09-12 에 arm B 03시 · P52 s2 18시 · E1 확정 런 18시 · P52 s1 20시 · arm A 22시가 몰린다.
