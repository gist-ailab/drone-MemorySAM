...
🔴 **2026-09-10 02:2x — E3s2(1-j) 가 CUDA OOM 으로 죽고 hpca100 GPU0 을 잃었다.** 원인은 타 사용자가 그
GPU 에 34,104MiB 를 잡은 것이고 우리 설정 문제가 아니다. 감시는 즉시 `SESSION_GONE` 을 냈으나 막을 수
있는 종류가 아니었다. **다만 E3s2 는 평가 구간에서 38.2GB(A100 40GB 의 93%)를 쓰고 있어 여유가 1.8GB
뿐이었다** — 공유 GPU 에서 이 정도로 꽉 채우면 남이 조금만 들어와도 죽는다. 재개할 때는 학습 중 test
평가를 끄든지 해서 평가 구간 peak 를 낮춰라. ckpt 는 `epoch10_63.33_top1` 까지 보존됐고 NAS md5 일치를
확인했다.
