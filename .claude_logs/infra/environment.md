---
legacy_id: 14
legacy_file: 14_environment_and_infra.md
moved: 2026-07-08
---
...
🔴 **`sam2` 모듈을 찾지 못하는 함정 (서버 목록 갱신 2026-09-08)**

`sam2` 가 editable 로 설치되지 않은 서버에서 SAM2 계열 코드(`val.py` 포함)를 실행하면
`ModuleNotFoundError: No module named 'sam2'` 로 즉시 죽는다. 실행 전에 다음을 지정한다.

```bash
export PYTHONPATH=<repo>/semseg/models/sam2:$PYTHONPATH
```

해당 서버: **`lecun` · `yeon` · `hpca100`**. hpca100 은 2026-09-08 E2 legal 재채점을 기동하다
이 오류로 실패해 확인됐다(그 전까지 문서에는 lecun·yeon 만 적혀 있었다). 학습(`train_reliadino.py`)은
이 변수 없이도 뜨지만 **평가(`val.py`)에서 걸리므로**, 재채점을 거는 자리에서 특히 주의한다.
