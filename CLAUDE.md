...
├── ckpts/          ← 웨이트/체크포인트 (.pth). <run>_<YYYYMMDD>/
├── analysis_logs/  ← eval·분석·시각화 산출물. <model>_eval_<YYYYMMDD>/ = report/ + viz/ + perdomain/
└── train_logs/     ← 학습 런 로그 (원격에서 rsync 회수)
```
**모든 세션(P38-PQ 등 신규 포함)은 새 학습/평가/분석 산출을 여기에 저장**하고, 새 작업 전략 세우기 전 이 루트를 먼저 확인한다. 원격(hpca100/jarvis/yeon)에서 돌린 결과는 휘발/분산되므로 `rsync`로 여기에 회수해 **누적**한다.
> 경로 변천: `/mnt/HDD2/src/logs/`(ISSUE-023 쓰기불가) → `/drone_nas/drone/analysis_logs/`(flat) → **위 nested**(07-17). `analysis_logs/`는 이전 완료라 지금 바로 신 경로에 쓰면 되고, 과거 **웨이트(`ckpts/`) 백필 rsync는 진행 중** — 완료+검증 전까지 과거 ckpt 유일본은 `/nas_jm/drone_ckpts`(삭제 금지). 단일 출처 = 메모리 `eval-logs-stats-location`.
