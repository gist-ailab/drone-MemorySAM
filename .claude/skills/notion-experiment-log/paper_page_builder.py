...
def sec_problem():
    return [
        ...
        table([
            ["트랙", "목표", "현재(합법)", "판정"],
            ["Seg — DELIVER (4모달 CLDE)", "논문 publish: SOTA 상회 (MM SAM-adapter val 69.60 / test 57.35)", "1차 규칙(학습기 val-best top1): test 최고 56.99 · 5시드 평균 53.83 · legal val 최고 67.89(E1 확정 시드3, 단일 시드·확정 전). 보조(정본 하네스 val 재선택 규칙): 평균 54.39 · 최고 55.29", "1차(같은 4모달 DGFusion 56.71): 단일 런 최고 +0.28 상회 · 5시드 평균 53.83 은 미달. 2차(2모달 MM SAM-adapter 57.35): −0.36. val 은 1차 −0.90 · 2차 −1.71"],
...
def sec_cards():
    return [
        ...
        P(B("Δ 는 모두 공식 재채점(평가 하네스 1024·native-GT 배치 1) test 기준이고, 괄호 안은 시드별 값이다. "),
...
def sec_plan():
    return [
        ...
        P(T("세 최고치는 구조(ReliaDINO P39.1 트렁크)가 같지만 손실 설정이 다르다 — DELIVER 는 C3 를 켜고 MUSES·MCubeS 는 끈다. 그래서 "),
          ...
          T(". 5시드 평균 53.83 은 DGFusion 에 미달하므로 단일 런 최고만 인용하지 않는다.")),
        h2("6.1 진행 중 (서버별)"),
        P(B("다섯 서버 실측(2026-09-17 04:00 KST). "),
          T("DELIVER 확정 3쌍의 같은 시드 기준선 두 개가 09-17 오후에 끝나고, MUSES 200 epoch 풀 런 네 종이 09-17~19 에 차례로 끝난다. MCubeS 이식은 09-16 밤 3페어로 종결됐다. "),
          B("아래 진행 수치는 전부 트레이너가 학습 중 찍는 val 이며 공식 재채점 값이 아니다"),
          T(" — 채택·폐기 판정은 평가 하네스(1024 해상도·native-GT 배치 1) 또는 MUSES 공식 채점기 재채점으로만 한다.")),
        table([
            ["서버 / GPU", "실험 — 무엇을 바꿨나", "데이터셋", "진행(트레이너 val)", "ETA · 비고"],
            ["jarvis 1,7 / 5,6", "C3-only 시드 902 / 903 — 확정 게이트의 같은 시드 기준선(확정 레시피에서 중간층 4탭만 뺀 구성). 902 는 타 사용자 점유로 죽어 ep54 에서 이어 받음", "DELIVER 4모달", "ep148 / ep164 — 최고 67.20 / 66.84", "09-17 20:55 / 15:45"],
            ["jarvis 0,3", "E-LoRA arm C 시드 903 — 공유 r8 + 센서별 잔차 r8", "DELIVER 4모달", "ep130 — 최고 66.96", "09-18 04:40"],
            ["jarvis 4", "E7 = MUSES 기준선 200 epoch 풀 런(4탭 없음) — 풀 런 세 종의 기준선 쪽", "MUSES 3모달", "ep55 — 최고 79.98", "09-18 11:35"],
            ["bengio 0~7", "E-LoRA A(센서별 r16) · B(완전공유 r16) × 시드 902·903", "DELIVER 4모달", "ep78~80 — 최고 66.80~68.57", "09-20 09:10~14:05"],
            ["yeon 0,1 / 2,3", "E1 확정 시드4 / C3-only 시드4 — 탭 코드가 든 새 체크아웃", "DELIVER 4모달", "ep56 / ep58 — 최고 67.05 / 66.74", "09-21 05:15 / 00:45"],
            ["yeon 4,5", "E13 확정 시드4 — E1 과 검정력을 맞추는 넷째 페어", "DELIVER 4모달", "ep60 — 최고 67.28", "09-20 05:10"],
            ["yeon 6,7", "E-LoRA arm C 시드 902 (옛 체크아웃, 코드 버전 각주)", "DELIVER 4모달", "ep68 — 최고 68.69 (현재 DELIVER 트레이너 val 최고)", "09-20 22:35"],
            ["hpca100 0", "E1M = 4탭 읽기의 MUSES 200 epoch 풀 런(시드 3407) — 디스크 사고 뒤 ep38 에서 이어 받음", "MUSES 3모달", "ep180 — 최고 81.94", "09-17 08:20"],
            ["hpca100 3", "E13M = 4탭 + 센서별 prototype 의 MUSES 풀 런(시드 3407)", "MUSES 3모달", "ep40 — 최고 80.86 (같은 ep40 에서 기준선 E7 79.31 대비 +1.55)", "09-18 23:38"],
            ["hpca100 1 / 2", "E13M / E1M 풀 런 둘째 시드(20260902) — 제출 헤드라인이 단일 시드에 걸리지 않게 하는 페어", "MUSES 3모달", "ep25 / ep20 — 78.74 / 78.68", "09-19 03:48 / 03:34"],
            ["hpca100 0 (대기)", "E17 = 4탭 읽기 + 고해상도 세부 가지 40 epoch 스크린 — 앞 런 완주 직후 자동 기동 예약", "DELIVER 4모달", "미기동", "09-17 08:20 기동 예정"],
            ["lecun", "우리 학습 없음 — 입출력 병목으로 학습 자리에서 제외, 짧은 추론·평가만", "—", "유휴", "타 사용자 두 명이 점유"],
        ]),
        P(B("이번 갱신에서 표에서 내린 런: "),
          T("MCubeS 셋째 페어 두 건(200 epoch 완주 — 이로써 MCubeS 이식 종결, 6.2 참조).")),
        ...
        table([
            ["#", "실험", "왜", "자원", "게이트"],
            ["0", "🟢 E17 = 중간층 4탭 읽기 레시피 + 고해상도 세부 가지(입력 이미지에서 stride-4/8 세부를 뽑아 FPN 에 게이트 잔차로 더함) 40 epoch 스크린 — hpca100 A100 한 장에 대기 기동 예약 완료, 앞 런 완주 직후 자동 기동", "얇은 객체(Pole·TrafficLight·Pedestrian·Static) 병목 직격. 지금 헤드라인의 약점이 DELIVER 시드 평균이라 여기를 겨냥한다", "hpca100 1장 (예약됨)", "24클래스 Δ vs 4탭 ≥ +0.5 · 얇은 객체 4클래스 Δ ≥ +2.0 · 악조건 손실 없음 · SOTA 거리 병기"],
            ["1", "✅ 분모 = 같은 시드 C3-only 200ep(생각정리 결정) 기동 완료 → 09-17 완주 후 E13·E1·E-LoRA 3페어 게이트 판정", "사전 등록 게이트의 페어는 같은 시드끼리", "jarvis 4장(진행 중)", "3페어 mean Δtest ≥ +1.0 그리고 24클래스 mean Δ ≥ +0.5"],
