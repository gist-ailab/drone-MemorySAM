...
def sec_plan():
    return [
        ...
        P(T("세 최고치는 구조(ReliaDINO P39.1 트렁크)가 같지만 손실 설정이 다르다 — DELIVER 는 C3 를 켜고 MUSES·MCubeS 는 끈다. 그래서 "),
          ...
          T(". 5시드 평균 53.83 은 DGFusion 에 미달하므로 단일 런 최고만 인용하지 않는다.")),
        h2("6.1 진행 중 (서버별)"),
        P(B("네 서버 실측(2026-09-16 21:30 KST). "),
          T("E1 확정 3시드가 끝났고, 남은 것은 같은 코드 기준선 두 개(09-17)와 넷째 페어들이다. MCubeS 이식은 09-16 밤에 3페어로 종결됐고, 비워진 hpca100 두 장에는 MUSES 풀 런의 둘째 시드 페어를 올렸다. "),
          B("아래 진행 수치는 전부 트레이너가 학습 중 찍는 val 이며 공식 재채점 값이 아니다"),
          T(" — 채택·폐기 판정은 평가 하네스(1024 해상도·native-GT 배치 1) 재채점으로만 한다.")),
        table([
            ["서버 / GPU", "실험 — 무엇을 바꿨나", "데이터셋", "진행(트레이너 val)", "ETA · 게이트"],
            ["jarvis 1,7 / 5,6", "C3-only 시드 902 / 903 — 확정 게이트의 같은 시드 기준선(E1 확정에서 중간층 4탭만 뺀 구성). 902 는 타 사용자 점유로 죽어 ep54 에서 이어 받음", "DELIVER 4모달", "ep109 / ep127 — 최고 66.99 / 66.26", "09-17 14~20시"],
            ["jarvis 0,3", "E-LoRA arm C 시드 903 — 공유 r8 + 센서별 잔차 r8", "DELIVER 4모달", "ep95 — 최고 66.60", "09-18 02시경"],
            ["bengio 0~7", "E-LoRA A(센서별 r16)·B(완전공유 r16) × 시드 902·903", "DELIVER 4모달", "ep57~63 — 최고 66.80~67.50", "09-20 09~14시"],
            ["yeon 0,1 / 2,3", "E1 확정 시드4 / C3-only 시드4 — 탭 코드가 든 새 체크아웃", "DELIVER 4모달", "ep39 / ep41 — 최고 67.05 / 66.17", "09-21"],
            ["yeon 4,5", "E13 확정 시드4 — E1 과 검정력을 맞추는 넷째 페어", "DELIVER 4모달", "ep36 — 최고 67.28", "09-20 오전"],
            ["yeon 6,7", "E-LoRA arm C 시드 902 (옛 체크아웃, 코드 버전 각주)", "DELIVER 4모달", "ep51 — 최고 67.23", "09-20 22시경"],
            ["hpca100 0", "E1M 200ep 풀 런(MUSES test 제출 후보) — 디스크 사고 뒤 ep38 에서 이어 받음", "MUSES 3모달", "ep136 — 최고 81.94", "09-17 09시경"],
            ["hpca100 1 / 2", "E13M / E1M 200ep 풀 런 둘째 시드(20260902) — 제출 헤드라인이 단일 시드에 걸리지 않게 하는 페어. 앞 런 완주 직후 대기 기동으로 올라감", "MUSES 3모달", "ep2 / ep1 — 방금 기동", "09-18~19"],
            ["hpca100 3", "E13M 200ep 풀 런(MUSES test 제출 후보) — 시드 3407", "MUSES 3모달", "ep10 — 최고 75.58", "09-18~19"],
                    ]),
        P(B("이번 갱신에서 표에서 내린 런: "),
          T("MCubeS 셋째 페어 B0Mc·E1Mc 시드 0827(200ep 완주 — 이로써 MCubeS 이식 종결, 6.2 참조).")),
