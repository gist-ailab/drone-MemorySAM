...
def sec_cards():
    return [
        ...
        h2("4.2 카드 표 (2026-09-16 04:00 KST 기준)"),
        ...
        table([
            ...
            ["E13M MUSES 이식", "E13 이 두 벤치에서 공통으로 오르는가", "두 시드 완결", "—", "✅ 공식 val 80.80 / 81.13 (평균 80.97) — E7 대비 +0.89 · E1M 대비 +0.55 로 게이트 ①② 통과, 두 벤치 공통 상승 첫 카드. 단 rain/night −1.79 는 두 시드에서 재현된 실제 손실"],
            ...
            ["E13Mc / E1Mc MCubeS 이식", "E13·E1 이 세 번째 벤치(편광·NIR 4센서)에서도 오르는가 · 레시피 그대로, 시드 3개씩", "완결 — 단 E1Mc 두 시드는 무효", "—", "🔴 판별 불가(09-15 철회) — yeon 체크아웃에 TAPS 코드가 없어 E1Mc 두 시드가 TAPS 없이 돌았다. 이 같은 시드 재실행이 기준선보다 +1.1~1.35 높게 나와 MCubeS 는 개입 없이도 1점 넘게 흔들린다. 유효 E1Mc 1시드 +1.71, E13Mc 3시드 +0.40 모두 구별되지 않아 hpca100 같은 코드 기준선 B0Mc 3시드로 재판정"],
...
def sec_plan():
    return [
        ...
        h2("6.1 진행 중 (서버별)"),
        P(B("네 서버 실측(2026-09-15 12:30 KST). "),
          T("hpca100 은 공유 볼륨이 가득 차 09:24~09:40 에 여섯 런이 멈췄다가, 판정 끝난 체크포인트를 정리해 12:24 에 네 장 모두 다시 돌기 시작했다. "
            "yeon 4·5 는 비어 있던 사이 다른 사용자가 가져가 MCubeS 0827 쌍을 lecun 으로 옮긴다. "),
        ...
        table([
            ...
            ["jarvis 2,4", "비어 있음 — E1 확정 시드3 완주(02:43)·legal 재채점(03:44) 끝, 다음 카드 결정 대기", "—", "—", "—"],
            ["jarvis 1,7 / 5,6", "C3-only 200ep 시드 902 / 903 — 확정 게이트의 같은 시드 기준선(E1 확정에서 TAPS 만 뺀 구성)", "DELIVER 4모달", "ep42 / ep40 — 최고 66.67 / 65.55", "09-17 13~15시"],
            ["jarvis 0,3", "E-LoRA arm C 시드 903 — 공유 r8 + 센서별 잔차 r8", "DELIVER 4모달", "ep12 — 63.87", "09-18 01시경"],
            ["bengio 0~7", "E-LoRA A(센서별 r16)·B(완전공유 r16) × 시드 902·903 — 공유가 센서별을 앞선 1페어 결과의 3시드 확인", "DELIVER 4모달", "ep16~17 — 64.1~65.3", "09-20 09~15시"],
            ["yeon 0,1 / 2,3", "E1 확정 시드4 / C3-only 시드4 — TAPS 코드가 든 새 체크아웃으로 재기동(옛 체크아웃 런은 TAPS 가 빠져 무효)", "DELIVER 4모달", "ep1 (12:20 재기동)", "09-21 경"],
            ["yeon 6,7", "E-LoRA arm C 시드 902 (옛 체크아웃, 판정 때 코드 버전 각주)", "DELIVER 4모달", "ep7 — 61.92", "09-20 22시경"],
            ["yeon 4,5", "E13 확정 시드4 — E1 시드4·C3 시드4 와 같은 서버·코드·시드의 E13 넷째 페어(헤드라인을 E1·E13 중에서 고르는 판정의 검정력 맞춤)", "DELIVER 4모달", "ep1 (09-15 20:21 기동)", "09-21 오전"],
            ["hpca100 0", "E1M 200ep 풀 런(시드 3407, MUSES test 제출 후보) — 디스크 사망 후 ep38 에서 이어 받기", "MUSES 3모달", "ep39 — 최고 80.40", "09-17 오후"],
            ["hpca100 1 / 2 / 3", "MCubeS 같은 코드 기준선 B0Mc 시드 3407 · 0828 / E1Mc 시드 0828 — 판별 불가였던 MCubeS 판정을 같은 서버·코드 쌍으로 다시 한다", "MCubeS 4모달", "ep1~3 (12:25 기동, val 상위 1개만 저장)", "09-16 오전"],
            ["lecun 0 / 1", "MCubeS B0Mc · E1Mc 시드 0827 쌍 (대기) — 데이터 복사 검증 후 새 체크아웃으로 기동", "MCubeS 4모달", "—", "09-16"],
        ]),
        P(B("이번 갱신에서 표에서 내린 런: "),
          T("hpca100 E7 시드 902·903(완주, 공식 재채점 대기) · E1M 시드3(ep35 에서 디스크 사망, ep35 val-best 로 판정) · yeon 옛 체크아웃 E1·C3 시드4(중단) · lecun CAFuser(완주).")),
