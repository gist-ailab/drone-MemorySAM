...
def sec_cards():
    return [
        ...
        h2("4.2 카드 표 (2026-09-16 16:40 KST 기준)"),
        ...
        table([
            ...
            ["E13Mc / E1Mc MCubeS 이식", "E13·E1 이 세 번째 벤치(편광·NIR 4센서)에서도 오르는가 · 레시피 그대로, 시드 3개씩", "완결 — 단 E1Mc 두 시드는 무효", "—", "🟡 같은 코드 재판정 진행 중(09-16) — 같은 서버·같은 코드 쌍 두 개가 +0.38 과 −0.26 으로 갈려 2페어 평균 +0.06(게이트 +0.5 미달). 셋째 페어 09-16 밤. 앞선 yeon 결과는 탭 코드 부재로 철회됐다"],
...
def sec_plan():
    return [
        ...
        h2("6.1 진행 중 (서버별)"),
        P(B("네 서버 실측(2026-09-16 15:20 KST). "),
          T("E1 확정 3시드가 끝났고, 남은 것은 같은 코드 기준선 두 개(09-17)와 넷째 페어들이다. hpca100 은 MCubeS 세 런을 마치고 다음 세 런으로 넘어갔다. "),
        ...
        table([
            ...
            ["hpca100 1 / 2", "MCubeS 셋째 페어 B0Mc · E1Mc 시드 0827", "MCubeS 4모달", "ep131 / ep129 — 최고 58.00 / 58.94", "09-16 21시경"],
                    ]),
        P(B("이번 갱신에서 표에서 내린 런: "),
          T("E1 확정 시드3(완주·재채점 완료) · MCubeS B0Mc 3407·0828 과 E1Mc 0828(200ep 완주, 6.2 참조) · jarvis 조건별 평가 네 건(완료).")),
        ...
        table([
            ...
            ["MCubeS 같은 코드·같은 서버 쌍 (09-16)", "마지막 epoch 기준 — 시드 3407: 기준선 58.30 대 4탭 58.68(+0.38) · 시드 0828: 58.30 대 58.04(−0.26)", "2페어 평균 +0.06 으로 사전 등록 게이트(+0.5) 미달이고 부호도 갈린다. 셋째 페어는 09-16 밤 완주"],
            ...
            ["🔴 MCubeS E1Mc 판정 철회 (09-15 07:30)", "yeon 체크아웃(378864f)에 TAPS 코드가 없어 E1Mc 두 시드·yeon E1 시드4 가 TAPS 없이 돌았음(학습 파라미터 수가 기준선과 같음)", "E1Mc 게이트 통과 무효 · MCubeS 는 E1·E13 모두 판별 불가 · 같은 코드 기준선 B0Mc 로 재판정 · 기동 검증에 파라미터 수 대조 추가"],
            ...
            ["MCubeS E13Mc 정정 (09-15)", "기준선을 마지막 epoch(56.97 / 56.61 / 57.58)로 맞춰 다시 계산 — 평균 +0.40", "게이트 +0.5 근소 미달 유지. 전날 −0.62 는 척도 혼합의 산물"],
        ...
        table([
            ...
            ["4", "MCubeS 이식 판정(E13Mc 16:53 · E1Mc 09-15 06:40) → DELIVER 판정에 따라 불필요한 계열 정리", "세 벤치 공통 상승 여부", "hpca100 · yeon", "마지막 epoch 3시드 평균 Δ vs 같은 시드 C3-off ≥ +0.5"],
