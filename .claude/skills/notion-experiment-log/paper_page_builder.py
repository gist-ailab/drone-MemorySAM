...
def sec_cards():
    return [
        ...
        table([
            ...
            ["E1 중간층 4탭 읽기", "기하 센서의 클래스 근거가 중간층에 있다 · 블록 6/12/18/24 를 FPN 레벨별로 투입", "40ep 두 시드 · 200ep 확정 시드1·2 완결, 시드3 진행 중(09-16 02시)", "확정 24클래스 55.26 / 55.85 (2시드 평균 55.55)", "판정 대기 — 같은 코드 기준선 902·903(09-17) 과의 매칭 쌍이 1차 근거. 24클래스로 E13 확정 3시드(55.33)와 동급. 조건별(시드1): 다섯 조건 24클래스 Δ +0.06~+1.19, 얇은 객체 4클래스는 다섯 조건 모두 카드 중 최고"],
...
def sec_plan():
    return [
        ...
        table([
            ["항목", "결과", "판정"],
            ["DELIVER 조건별 평가 E1·E13 확정 시드1 (09-15)", "cloud·fog·night·rain·sun 다섯 조건 test(조건당 약 380장). 24클래스 Δ vs 시드 821 기준선 — E1 +0.58 / +0.83 / +1.19 / +0.56 / +0.06, E13 +0.14 / +0.79 / +1.18 / +0.27 / +0.35. 첫 시도는 TAPS 코드가 없는 옛 체크아웃이라 로드가 어긋나 무효, 새 체크아웃에서 재실행(로드 누락·초과 0)", "두 카드 모두 −0.5 미만 조건 없음(악조건 조건 통과 재료). 한 시드씩이고 기준선 코드 버전이 달라 같은 코드 기준선과의 조건별 비교가 1차 근거"],
