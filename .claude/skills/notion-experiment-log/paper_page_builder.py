...
def sec_cards():
    return [
        ...
        h2("4.2 카드 표 (2026-09-15 10:10 KST 기준)"),
...
def sec_plan():
    return [
        ...
        h2("6.1 진행 중 (서버별)"),
        P(B("다섯 서버 전수 실측(2026-09-15 04:40 KST, 06:50 갱신). "), T("yeon 4·5 는 조건별 평가가 끝나 비어 있던 사이 다른 사용자가 가져갔다(12:10 확인). 빈 GPU 는 lecun 0~2 뿐이다. jarvis 0·3 에는 E1 시드2 재채점 뒤 E-LoRA arm C 시드 903 이 자동으로 이어진다. "),
        ...
        table([
            ...
            ["jarvis 0,3 / 2,4", "E1 확정 런 시드2 / 시드3 — 중간층 4탭 읽기 단독 카드의 200ep 확정", "DELIVER 4모달", "ep192 / ep130 — 최고 68.76 / 68.53", "09-15 06:55 / 09-16 02:20 · 완주 판정 뒤 legal 재채점이 자동으로 이어진다"],
            ["jarvis 1,7 / 5,6", "C3-only 200ep 시드 902 / 903 — 확정 게이트의 같은 시드 기준선(E1 확정에서 TAPS 만 뺀 구성)", "DELIVER 4모달", "ep18 — 64.48 / 65.01", "09-17 12시경"],
            ["bengio 0~7", "E-LoRA A(센서별 r16)·B(완전공유 r16) × 시드 902·903 — 공유가 센서별을 앞선 1페어 결과의 3시드 확인", "DELIVER 4모달", "ep8~10 — 61.3~63.3", "09-18 전후"],
            ["hpca100 0~3", "🔴 정지 — 공유 볼륨이 가득 차(09-15 09:24~09:40) E1M 200ep 풀 런·E1M 시드3·MCubeS 같은 코드 기준선 B0Mc 3시드·E1Mc 2시드가 디스크 공간 부족으로 멈춤. E7 시드 902·903 은 그 전에 완주", "MUSES 3모달 · MCubeS 4모달", "E1M 풀 런 ep35(최고 80.40) · E1M 시드3 ep35(80.86) · E7 두 시드 완주(79.79 / 80.45)", "공간 정리 동의 후 재기동 — E1M 풀 런은 ep35 부터 이어 받기"],
            ["yeon 0,1 / 2,3 / 6,7", "E1 확정 시드4(🔴 TAPS 미적용 — yeon 코드에 TAPS 없음) / C3-only 시드4 / E-LoRA arm C 시드 902", "DELIVER 4모달", "ep4 전후", "E1·C3 시드4 는 사용자 동의 후 새 체크아웃에서 재기동 예정 · arm C 는 계속(코드 버전 각주)"],
            ["lecun 0,1,2", "CAFuser Swin-T 대조군 재현 — 학습 완주(266,667 iter, 09-15 04:26)", "DELIVER CLDE", "종료 평가 mIoU 66.04 (스플릿 확인 전, 공개 val 68.12)", "학습 끝. 체크포인트별 test 스윕은 이 세션이 띄운 것이 아니며 판정에 쓰지 않음"],
        ]),
        P(B("이번 갱신에서 표에서 내린 종결 런: "),
          T("E13 확정 시드3 · E-LoRA arm C(재채점 완료) · MCubeS hpca100 4런 (모두 판정·기록 완료, 6.2 참조).")),
