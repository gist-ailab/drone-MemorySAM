...
def sec_cards():
    return [
        ...
        h2("4.2 카드 표 (2026-09-14 13:00 KST 기준)"),
        ...
        table([
            ...
            ["🥇 E13 = E1 + E3 결합", "중간층 읽기(특징 접점) + 센서별 prototype(손실)의 결합. 두 축이 직교한다는 가설", "40ep 세 시드 완결 → 200ep 확정 시드1·시드2 완주, 시드3 진행 중(19:35)", "40ep +1.29 / +0.69 / +0.76 (평균 +0.91) · 확정 시드1 55.13(+0.45) · 시드2 55.25(+0.57)", "✅ 스크린 3/3 통과. 확정 두 시드의 24클래스가 0.12 안에 붙는다(25클래스 차 1.81 은 RailTrack 68.45 대 20.49 한 클래스). 시드2·3 매칭 분모가 없어 3페어 판정 보류"],
            ...
            ["E-LoRA arm A / B / C", "센서별 LoRA 용량이 정말 필요한가 · 센서별 r16(6.29M) / 완전공유 r16(1.57M) / 공유8+잔차8(3.93M)", "A·B 완결, C 진행 중(21:45)", "A 54.70 · B 55.04 (24클래스 절대값)", "24클래스로는 파라미터 1/4 인 완전공유(B)가 센서별(A)을 앞선다. 세 갈래 판정은 C 재채점 후"],
            ["E13Mc / E1Mc MCubeS 이식", "E13·E1 이 세 번째 벤치(편광·NIR 4센서)에서도 오르는가 · 레시피 그대로, 시드 3개씩", "E13Mc 3시드 완결 · E1Mc 1/3", "—", "❌ E13Mc 미달 — 마지막 epoch 평균 57.45 로 같은 시드 C3-off 58.07 대비 −0.62(게이트 +0.5). 카드에 가장 유리한 정의(val 최고 대 val 최고)로도 +0.15. E1Mc 시드3407 은 58.68(+0.75), 나머지 두 시드 09-15"],
...
def sec_plan():
    return [
        ...
        h2("6.1 진행 중 (서버별)"),
        P(B("다섯 서버 전수 실측(2026-09-14 12:53 KST, 17:00 갱신). "), T("빈 GPU 는 jarvis 2장(12:55 부터)과 hpca100 4장(16:53 부터)이며 다음 카드는 미정이다. bengio 4장은 06:55 에 비어 있다가 10:41 에 다른 사용자에게 넘어갔다. "),
        ...
        table([
            ...
            ["jarvis 0,3", "E1 확정 런 시드2 — 중간층 4탭 읽기 단독 카드의 200ep 확정, 3페어 중 둘째", "DELIVER 4모달", "ep142 — 최고 68.76@134", "09-15 07시 · 완주 후 legal 재채점"],
            ["jarvis 2,4", "E1 확정 런 시드3 — 3페어 중 셋째", "DELIVER 4모달", "ep82 — 최고 68.53@70 (09-13 약 5시간 공백 뒤 이어 돌리기)", "09-16 02:40"],
            ["jarvis 5,6", "E13 확정 런 시드3 — 1순위 카드(E1+E3 결합)의 3페어 중 셋째", "DELIVER 4모달", "ep170 — 최고 68.15@90", "09-14 19:35 · 완주 판정 뒤 legal 재채점이 자동으로 이어진다"],
            ["yeon 6,7", "E-LoRA arm C = 공유 r8 + 센서별 잔차 r8(3.93M). 센서별(A)과 완전공유(B)의 절충", "DELIVER 4모달", "ep186 — 최고 67.99@56", "09-14 21:45 · 세 갈래 판정"],
            ["hpca100 0~3", "(유휴) MCubeS E13Mc 3시드 · E1Mc 1시드 완주 — 다음 카드 미정", "—", "16:53 완주", "E13Mc 게이트 미달(§4 카드 표)"],
            ["yeon 4,5", "E1Mc 시드 20260827 · 20260828 — MCubeS 이식 대조", "MCubeS 4센서", "ep84 — 57.26 / 57.85", "09-15 06:40"],
            ["lecun 0,1,2", "CAFuser Swin-T 대조군 재현(외부 기준선)", "DELIVER CLDE", "iter 236,399/266,667", "09-15 02시 · 공개값 val 68.12 / test 55.80"],
        ]),
        P(B("이번 갱신에서 표에서 내린 종결 런: "),
          T("E13 확정 시드1·시드2 · E1 확정 시드1 · E14 · E15 · E13s3 · E-LoRA arm A·B · P52 두 시드 · E13M 시드2 (모두 판정 완료, 6.2 참조).")),
        ...
        table([
            ["#", "실험", "왜", "자원", "게이트"],
            ["1", "E13·E1 확정 3페어 완결(E13 시드3 19:35 · E1 시드2 09-15 07시 · 시드3 09-16 02:40) → 🔴 먼저 시드2·3 매칭 분모 정의(시드 821 고정 / 5시드 평균 / 같은 시드 C3-only 200ep 신규) 결정", "1순위 카드의 확정 판정. 지금은 분모가 시드 821 하나라 시드2·3 Δ 가 시드 어긋난 비교", "jarvis (완주 즉시 자동 재채점)", "3페어 mean Δtest ≥ +1.0 그리고 24클래스 mean Δ ≥ +0.5"],
            ["2", "🔴 빈 GPU 배정 — jarvis 1·7(지금) · hpca100 0~3(16:53) · jarvis 5·6(19:35 이후)", "비우면 즉시 뺏긴다(bengio 4장 09-14 상실)", "카드 선정 = 생각정리 세션, 점유 동의 = user", "—"],
