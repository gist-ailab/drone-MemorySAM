...
def sec_cards():
    return [
        ...
        h2("4.2 카드 표 (2026-09-15 08:10 KST 기준)"),
        ...
        table([
            ...
            ["E1 중간층 4탭 읽기", "기하 센서의 클래스 근거가 중간층에 있다 · 블록 6/12/18/24 를 FPN 레벨별로 투입", "40ep 두 시드 · 200ep 확정 시드1·2 완결, 시드3 진행 중(09-16 02시)", "확정 24클래스 55.26 / 55.85 (2시드 평균 55.55)", "판정 대기 — 같은 코드 기준선 902·903(09-17) 과의 매칭 쌍이 1차 근거. 24클래스로 E13 확정 3시드(55.33)와 동급"],
            ...
            ["🥇 E13 = E1 + E3 결합", "중간층 읽기(특징 접점) + 센서별 prototype(손실)의 결합. 두 축이 직교한다는 가설", "40ep 세 시드 · 200ep 확정 세 시드 완결, 매칭 기준선 902·903 진행 중", "확정 3시드 24클래스 평균 55.33 · 5시드 기준선(같은 선택 규칙) 54.66 대비 +0.67", "🟡 잠정 통과 — 같은 선택 규칙의 5시드 기준선 대비 Δtest +1.31 · Δ24 +0.67 로 두 조건을 넘는다. 확정은 같은 시드 기준선(09-17) 완주 후. MCubeS 에서는 게이트 근소 미달"],
...
def sec_plan():
    return [
        ...
        h2("6.1 진행 중 (서버별)"),
        P(B("다섯 서버 전수 실측(2026-09-15 04:40 KST, 06:50 갱신). "), T("빈 GPU 는 yeon 2장(4·5, E1Mc 완주)이며 다음 카드는 미정이다. jarvis 0·3 에는 E1 시드2 재채점 뒤 E-LoRA arm C 시드 903 이 자동으로 이어진다. "),
