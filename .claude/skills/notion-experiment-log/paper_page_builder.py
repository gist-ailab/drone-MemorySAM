...
def sec_cards():
    return [
        ...
        table([
            ...
            ["MUSES 페어 보강 E7 시드 902·903 · E1M 시드 903 · E1M 200ep", "E7 기준선이 한 시드뿐이라 시드2·3 페어가 성립하지 않았음 · 풀 런은 test 서버 제출 후보", "E7 두 시드·E1M 시드3 공식 재채점 완료(09-15) · 풀 런은 ep38 에서 이어 받아 진행 중", "E1M − E7 페어 평균 +0.52(3페어)", "E7 79.61 / 80.28, E1M 시드3 80.69(ep35) — 판정 대기"],
...
def sec_plan():
    return [
        ...
        table([
            ["항목", "결과", "판정"],
            ["MUSES 시드 페어 보강 공식 재채점 (09-15)", "E7 기준선 시드 902·903 = 79.61 / 80.28, E1M 시드3(ep35) = 80.69. 페어 차이 E1M − E7 = +0.34 / +0.80 / +0.41(평균 +0.52), E13M − E7 = +0.72 / +1.53(평균 +1.13). 다른 서버에서 채점해 E1M 시드2 를 대조군으로 다시 채점했고 차이는 +0.002", "판정 대기 — E1M 은 세 페어 모두 양수, E13M 은 두 페어 모두 +0.5 초과"],
