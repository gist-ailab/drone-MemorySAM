...
def sec_summary():
    return [
        ...
        CO([B("지금 하는 것(2026-09-07~) "),
            T("일일 사이클 실험 카드: 카드 1개 = 변수 1개 = 하루, 40ep 스크린 후 통과 카드만 200ep×3페어 확정. 기준선 B0 legal test 53.78. E1(중간층 4탭 읽기) 트레이너 val ep30 66.88 vs B0 65.13, E2(전 선형층 LoRA r32) legal test 54.50 = B0 +0.72(회색지대, 판정 보류; RailTrack +15.8·Wall −6.4), E7/E7c(PhysAug off/on) 둘 다 완주해 공식 페어 재채점 중, E3·E4·E1M 진행 중, E7(MUSES PhysAug-off 공정 기준선) 완주. 병행: DGFusion·CAFuser 공식 설정 직접 재학습(재현 수치 확보 후 한계 분석).")], "🔬", "orange_background"),
...
def sec_cards():
    return [
        ...
        table([
            ...
            ["E1 중간층 4탭 읽기", "기하 센서의 클래스 근거가 중간층에 있음", "블록 6/12/18/24 출력을 SimpleFPN 레벨별로 투입하면 depth·lidar 의존 클래스가 회복", "MODEL.TAPS per_modal [6,12,18,24]", "ep38/40 (bengio GPU6,7), 완주 20:28 KST 전후", "트레이너 val ep5~35: 60.98/63.56/66.30/66.57/66.43/66.88/67.25(카드 중 최고) vs B0 58.71/63.17/62.18/63.91/64.03/65.13/64.67(ep40 65.40)", "보류 — 완주 후 legal test 1회로 판정 (양성 방향)"],
            ...
            ["E3 센서별 클래스 prototype", "user 가설 직접 검증", "클래스 정체성을 센서마다 독립으로 지지하면 RGB가 헷갈리는 클래스를 lidar·depth 근거가 붙잡음", "P46.C3_PROTO.SRC permodal + AGREE_LAMBDA", "ep30/40 (bengio GPU4,5), 완주 ETA 23:45 KST", "ep5~30: 59.35/63.72/62.23/65.44/65.97/65.51 (best 65.97@25, B0 ep25 64.03)", "보류"],
            ...
            ["E1M MUSES 4탭 이식", "E1의 MUSES 공통 상승 검증", "중간층 4탭이 MUSES에서도 오르면 공통 한 단계", "E7(PhysAug-off) + MODEL.TAPS on", "ep10/40 (hpca100 GPU3), ETA 09-09 04시 KST", "ep5 71.73 · ep10 75.55 vs E7 73.84 · 75.39 (−2.11 → +0.16, warmup 15ep라 ep15·20이 실질 비교점)", "E7 매칭 페어, 공식 val Δ ≥ +1.0"],
...
def sec_plan():
    return [
        ...
        table([
            ...
            ["bengio 6,7", "E1 중간층 4탭 읽기", "DELIVER 4모달", "ep38/40, 트레이너 val 67.25@35(카드 중 최고)", "20:28 KST 완주 → val-best legal 재채점"],
            ["bengio 4,5", "E3 센서별 prototype", "DELIVER 4모달", "ep30/40, 65.51@30 (best 65.97@25)", "23:45 KST 완주"],
            ["bengio 1-3", "E4 혼동 쌍 margin(auto)", "DELIVER 4모달", "ep30/40, 65.79@25 (best 65.92@15)", "22:30 KST 완주. E4b(명시 쌍) config 추가, 미기동"],
            ["hpca100 1,3 → GPU1 비어 있음", "E2 전 선형층 LoRA r32", "DELIVER 4모달", "완주 + legal test 54.50 / val 68.38 (Δtest +0.72 회색지대)", "판정 보류(생각정리 세션). GPU1 다음 카드 대기"],
            ["hpca100 3", "E1M MUSES 4탭 이식(E7 매칭 페어)", "MUSES 3모달", "ep10/40, 75.55@10 (E7 75.39)", "09-09 04시 KST 완주 예정"],
        ...
        table([
            ["#", "실험", "왜", "자원", "게이트"],
            ["1", "E1 legal 재채점 → 통과 시 200ep×3페어 확정 런", "궤적 양성, 카드 프로그램 첫 통과 후보", "yeon 2장 × 3", "Δtest ≥ +1.0, 3페어 mean ≥ +1.0"],
            ["2", "E2 legal 재채점 · E4b(RailTrack 명시 쌍) 스크린 · E3·E4 완주 재채점", "2일차 판정 완결", "bengio/hpca100 해방분", "Δtest ≥ +1.0"],
