...
def sec_cards():
    return [
        ...
        table([
            ...
            ["E1 중간층 4탭 읽기", "기하 센서의 클래스 근거가 중간층에 있음", "블록 6/12/18/24 출력을 SimpleFPN 레벨별로 투입하면 depth·lidar 의존 클래스가 회복", "MODEL.TAPS per_modal [6,12,18,24]", "완주 + legal 재채점 완결 (bengio)", "트레이너 val ep5~35: 60.98/63.56/66.30/66.57/66.43/66.88/67.25(카드 중 최고) vs B0 58.71/63.17/62.18/63.91/64.03/65.13/64.67(ep40 65.40)", "✅ 스크린 통과 — legal test 54.85 / val 66.90 = B0 +1.07 / +1.93 (val-best ep35, 생각정리 세션 확정). 클래스별: RailTrack +13.13·TrafficLight +5.01·SideWalk +3.09, Wall +0.26·Static +0.25·Water −1.71. val→test 전이율 55%. 다음 = 200ep×3페어 확정 런"],
            ...
            ["E3 센서별 클래스 prototype", "user 가설 직접 검증", "클래스 정체성을 센서마다 독립으로 지지하면 RGB가 헷갈리는 클래스를 lidar·depth 근거가 붙잡음", "P46.C3_PROTO.SRC permodal + AGREE_LAMBDA", "완주 + legal test 재채점 완결 (bengio)", "legal test 55.18 = B0 +1.40 (val-best ep25, 트레이너 val 65.97; legal val 미측정). 로그 Best Test 54.99@35는 test-best라 무효", "✅ 카드 넷 중 최고 Δtest — 통과선 +1.0 초과(판정 = 생각정리 세션). val 순위(E2>E1>E3>E4)와 test 순위(E3>E1>E2>E4) 역전. 후속 E3b(일치 항 0.1) jarvis 기동"],
            ["E4 혼동 쌍 margin", "붕괴가 특정 쌍으로 흡수됨", "검증셋 혼동행렬 top-k 쌍에 prototype/로짓 margin", "CONFUSION_PAIRS auto_k5", "완주 + legal 재채점 완결 (bengio)", "legal test 53.75 / val 65.86 = B0 −0.03 / +0.89 (val-best ep15). 클래스별: RailTrack +13.73·Ground +10.88·Water +5.86 / Pole −8.87·TwoWheeler −5.10·GroundRail −4.51·RoadLine −3.74", "❌ 폐기(생각정리 세션 확정) — 표적 쌍은 올랐으나 얇은 구조물 클래스가 넓게 손실. 명시 쌍판 E4b(bengio 양도로 ep14에서 중단, ckpt 보존)"],
            ["E7 MUSES PhysAug-off", "공정선 정합", "헤드라인 교체용 기준선(게이트 없음)", "PHYSAUG off", "완주 + 공식 재채점 완결 (hpca100)", "공식 native val 80.0756 (letterbox 1024 80.2872), 트레이너 80.29@40", "✅ PhysAug-off 기준선 확정 — E7c(on) 79.9417 대비 +0.134. 이후 MUSES 카드 기준선 = E7"],
            ["E7c MUSES PhysAug-on 대조군", "PhysAug 효과 크기 격리", "—", "E7과 PHYSAUG만 상이", "완주 + 공식 재채점 완결 (hpca100)", "공식 native val 79.9417 vs E7 80.0756 (Δ −0.134), 트레이너 80.18@40", "PhysAug on의 이득 없음 → off로 통일 확정(공정성 정정 완료)"],
            ["E1M MUSES 4탭 이식", "E1의 MUSES 공통 상승 검증", "중간층 4탭이 MUSES에서도 오르면 공통 한 단계", "E7(PhysAug-off) + MODEL.TAPS on", "완주 + 공식 재채점 완결 (hpca100 GPU3, /tmp 우회 재개)", "공식 native val 80.416 vs E7 80.0756 = Δ +0.340 (letterbox +0.349). 트레이너 val-best 80.64@35, 궤적 Δ(E1M−E7) ep10 이후 7지점 연속 양수(+0.16~+0.95). 조건별 fog/day 85.43 ~ rain/day 67.24", "🟡 게이트(+1.0) 문면상 미달이나 폐기 아님 — 중립~미소 양성, 채택 저지 사유 없음(생각정리 세션 판정). 근거: 공식 +0.34·letterbox +0.35·트레이너 +0.35 세 축 일치, 8지점 중 7지점 양수(PhysAug의 4:4 진동과 다름), 같은 4탭 축이 DELIVER에서 +1.07 통과, 단일 아키텍처 원칙상 MUSES는 무손해면 충분"],
            ["E12 / E2s2 (hpca100)", "E1+E2 결합 / E2 시드2 페어", "결합이 E1·E2 이득을 더하는가 / E2 +0.72의 재현", "TAPS on + LORA_TARGETS 전 선형층 / E2 + seed 20260902", "ep15/40 · ep15/40 (/tmp 우회 가동)", "E12 ep5 61.04·ep10 61.32 / E2s2 61.13·61.86 (ep5 시드 흔들림 1.41 — 축 비교 금지)", "09-10 08시 · 07시 KST 완주 예정"],
            ["E3b (jarvis GPU1,2,4)", "E3 + 센서 간 prototype 일치 항", "일치 항(AGREE_LAMBDA 0.1)을 켜면 Wall/Water가 회복되는가", "E3 대비 AGREE_LAMBDA 0.0→0.1만", "기동 (09-09), 완주 20시 KST 예정", "—", "Δtest ≥ +1.0, 예측 RailTrack 유지 + Wall/Water 회복"],
            ["B0s2 / E1s2 / E4b (bengio)", "시드2 기준선 / E1 시드2 페어 / RailTrack 명시 쌍 margin", "E1 통과의 재현 · 명시 쌍이 RailTrack을 더 올리는가", "seed 20260902 / E1 + seed 20260902 / PAIRS 명시", "⏸ 중단(bengio 후배 양도, 09-09 04:18) — ep5 / ep5 / ep10 ckpt 보존, AUTO_RESUME 재개 대기", "중단 시 ep5: B0s2 59.71 · E1s2 61.75 · E4b ep10 62.83", "재개 슬롯 대기(jarvis 유휴 7장 후보)"],
...
def sec_plan():
    return [
        ...
        table([
            ...
            ["hpca100 3", "E1M MUSES 4탭 이식(E7 매칭 페어)", "MUSES 3모달", "완주 + 공식 재채점 완결: 80.416 vs E7 80.0756 (+0.340) — 게이트 문면상 미달이나 폐기 아님(중립~미소 양성)", "GPU3 해방"],
            ...
            ["jarvis (중단)", "DGFusion Swin-T 재학습(공식 설정 bs8·LR 1e-4·200k)", "DELIVER CLDE", "⏸ NaN 발산 4회(iter 86,572·87,370·87,842·86,572 재현 — 특정 배치 원인 유력, CE·contrastive NaN·mask/dice 0 동일 형태) — 80k ckpt = 재현 정본: val 66.54(공개 +0.03) / test 55.68(공개 56.71 −1.03)", "생각정리 세션 판정: 200k 완주 포기, 80k ckpt를 정본으로 기록(공식 설정을 바꾸면 재현이 아님). jarvis GPU5·7은 감시 공백 중 타 사용자에게 상실. 평가 함정: DATASETS.TEST_SEMANTIC이 실제 키"],
        ...
        table([
            ["#", "실험", "왜", "자원", "게이트"],
            ["1", "✅ E3(+1.40)·E1(+1.07) 스크린 통과 → 200ep × 3페어 확정 런 착수 + 시드2 페어(B0s2·E1s2) 재개", "카드 프로그램 첫 통과 2건. bengio 양도로 중단된 시드2 페어는 다른 서버에서 AUTO_RESUME", "jarvis 유휴 7장 / yeon(P52·E-LoRA 완주 09-12~13 후)", "3페어 mean ≥ +1.0"],
            ["2", "E2 회색지대(+0.72) 재판정 = E2s2·E12(09-10 07~08시) / E3b(일치 항, 09-09 20시) legal 재채점 / E4b 재개", "2~3일차 판정 완결", "hpca100 · jarvis", "Δtest ≥ +1.0 (페어 mean)"],
            ...
            ["4", "E1M(MUSES 4탭) ✅ 완주 +0.34(폐기 아님, 중립~미소 양성) · E12(E1+E2) · E2s2 진행 중", "공통 상승·결합·재현", "hpca100", "단일 아키텍처 원칙: DELIVER 통과 + MUSES 무손해면 채택 검토"],
            ...
            ["8", "E-LoRA 세 팔(A per-modal / B 공유 / C 공유+잔차) 완주 판정 → E-BB A2 상위 절반 FT → E8 copy-paste → E5/E6(하향)", "구조 사다리 S2~S4 · LoRA 구조 ablation", "yeon·jarvis 200ep 진행 중", "A1 ≥ A0 + 1.0 / A2 ≥ A1 + 0.5 / C ≥ A − 0.3"],
