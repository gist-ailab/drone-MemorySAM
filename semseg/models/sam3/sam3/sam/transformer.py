...
class Attention(nn.Module):
    ...
    def forward(self, q: Tensor, k: Tensor, v: Tensor) -> Tensor:
        ...
        # [RBMA] additive reliability bias on pre-softmax attention logits.
        # When `self._rbma_attn_bias` is set (float tensor broadcastable to
        # (B, nheads, L_q, S_k)), use the math/mem-efficient SDPA path with
        # attn_mask=bias (flash kernel rejects arbitrary float masks). Set only on
        # cross-attn modules by semseg/models/sam3/sam3_lora_rbma.py (RBMA).
        _rbma_bias = getattr(self, "_rbma_attn_bias", None)
        if _rbma_bias is not None:
            with torch.backends.cuda.sdp_kernel(
                enable_flash=False, enable_math=True, enable_mem_efficient=True
            ):
                out = F.scaled_dot_product_attention(
                    q, k, v, attn_mask=_rbma_bias, dropout_p=dropout_p
                )
        elif self.use_fa3:
