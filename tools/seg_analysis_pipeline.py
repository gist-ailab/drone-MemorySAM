...
Dimensions (사용자 분석항목 1-4 매핑은 tools/README_seg_analysis.md 참조):
  D1 per-class / per-domain metrics   -> tools/eval_per_domain.py + tools/analyze_per_domain.py   [항목4]
  D2 per-modality encoder info        -> tools/viz_features.py (R2) + tools/module_diagnostics.py (B) [항목2]
  D2N per-modality feature statistics -> tools/feature_stats.py (full-testset numeric)            [항목2]
  D3 adapter / LoRA / router health   -> tools/adapter_health.py (static, always) + module_diagnostics (C/E/F)
  D3B per-modality adapter adaptation -> tools/modal_adaptation.py (adapter on/off A-B)           [항목1]
  D4 post-fusion feature info         -> tools/viz_features.py (R2/R3/R5) + module_diagnostics (D) [항목3]
  D5 module A/B ablation              -> tools/module_ablation.py (toggle 전후 ΔmIoU+피쳐 변화)    [항목3]
  (cross-model: tools/compare_models.py — 파이프라인 밖, N개 모델 D1 산출물 비교)                  [항목4]
...
def main():
    ...
    ap.add_argument('--ablation-toggles',
                    default='rbma_off,router_off,ctd_off,sdc_off,temp_off,cons_off,amf_uniform')
    ...
    # ---- D2N: full-testset per-modal feature statistics [항목2] ----
    if 'D2N' in stages:
        if not cap['forward_ok']:
            skip('D2N_feature_stats', 'forward failed in capability probe')
        elif not cap['hooks']['_last_per_modal_feats']:
            skip('D2N_feature_stats', f'_last_per_modal_feats not live (family={cap["family"]})')
        else:
            run('D2N_feature_stats',
                [py, str(TOOLS / 'feature_stats.py'), '--cfg', args.cfg,
                 '--model_path', args.model_path,
                 *(['--dataset-root', args.dataset_root] if args.dataset_root else []),
                 '--conditions', args.conditions, '--max-imgs', str(args.max_imgs),
    ...
    # ---- D3B: per-modality adapter adaptation (adapter on/off A-B) [항목1] ----
    if 'D3B' in stages:
        if not cap['forward_ok']:
            skip('D3B_modal_adaptation', 'forward failed in capability probe')
        elif not cap['hooks']['_last_per_modal_feats']:
            skip('D3B_modal_adaptation', f'per-modal hooks not live (family={cap["family"]})')
        else:
            run('D3B_modal_adaptation',
                [py, str(TOOLS / 'modal_adaptation.py'), '--cfg', args.cfg,
                 '--model_path', args.model_path,
                 *(['--dataset-root', args.dataset_root] if args.dataset_root else []),
    ...
    # ---- D5: module A/B ablation [항목3] ----
    if 'D5' in stages:
        if not cap['forward_ok']:
            skip('D5_module_ablation', 'forward failed in capability probe')
        else:
            run('D5_module_ablation',
                [py, str(TOOLS / 'module_ablation.py'), '--cfg', args.cfg,
                 '--model_path', args.model_path,
                 *(['--dataset-root', args.dataset_root] if args.dataset_root else []),
                 '--conditions', args.conditions,
                 '--max-imgs', str(min(args.max_imgs, 40)),
    ...
    lines = [
        ...
        *[f"| {s} | {r['status']} | {r.get('note', '')} |" for s, r in results.items()],
        ...
        "- `feature_stats.{json,md,_pca.png}` — full-testset per-modal 수치 통계 (D2N, 항목2)",
        "- `modal_adaptation.{json,md}` — adapter on/off per-modal 적응도 (D3B, 항목1)",
        "- `module_ablation.{json,md}` — 모듈 toggle 전후 비교 (D5, 항목3)",
